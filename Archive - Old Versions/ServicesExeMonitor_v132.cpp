...
self.activity_panel_w = rightW;
    ...
    // Services page
    {
        ...
        // Split left panel horizontally: bottom half shows selected service description
        int pageH = leftH;
        int topNeeded = y2 + 2*(btnH + btnGap) + btnH + inPad;
        int topH = std::max(pageH / 2, topNeeded);

        int infoW = std::max(S(80), lw - 2*inPad);
        int infoLabelY = topH + inPad;
        int infoEditY = infoLabelY + labelH + S(6);
        int infoEditH = pageH - infoEditY - inPad;
        if (infoEditH < S(48)) infoEditH = S(48);
    ...
    // EXEs page
    {
        ...
        // Split left panel horizontally: bottom half shows selected EXE info
        int pageH = leftH;
        int topNeeded = y2 + 2*(btnH + btnGap) + btnH + inPad;
        int topH = std::max(pageH / 2, topNeeded);

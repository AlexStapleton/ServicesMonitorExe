...
static void apply_filter_now(App& self, HWND hwnd, int kind);
...
// Debounce so typing doesn’t rebuild the ListView on every keystroke
...
// Empty-state labels occupy the list area
if (self.svc_empty) SetWindowPos(self.svc_empty, NULL, cx, y + searchH + gap, cw, ch - searchH - gap, SWP_NOZORDER);
if (self.exe_empty) SetWindowPos(self.exe_empty, NULL, cx, y + searchH + gap, cw, ch - searchH - gap, SWP_NOZORDER);
...
if (wParam == TIMER_SEARCH_DEBOUNCE) {
    ...
    if (self.filter_pending[KIND_SVC]) { self.filter_pending[KIND_SVC] = false; apply_filter_now(self, hwnd, KIND_SVC); }
    if (self.filter_pending[KIND_EXE]) { self.filter_pending[KIND_EXE] = false; apply_filter_now(self, hwnd, KIND_EXE); }
    return 0;
}
...
// Persist column widths/order when user drags headers

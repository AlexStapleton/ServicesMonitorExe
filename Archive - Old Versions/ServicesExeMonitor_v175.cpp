...
#define IDC_PREF_FLAP_COUNT       8010
#define IDC_PREF_FLAP_MINUTES     8011
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (wcscmp(it->last_status, ns) != 0) {
                ...
                if (kind == KIND_SVC) {
                    ...
                    // Reset window if old
                    if (it->flap_window_start_wall == 0 || (wall_ts - it->flap_window_start_wall) > (time_t)(self.flap_disable_window_minutes > 0 ? (self.flap_disable_window_minutes * 60) : 60) {
                    ...
                    if (!it->flap_flagged && it->flap_restarts >= (uint16_t)(self.flap_disable_count > 0 ? self.flap_disable_count : 1)) {
                        it->flap_flagged = true;                        log_linef(self, L"%s: detected frequent restarts (%u in ~%d min)", it->name.c_str(), (unsigned)it->flap_restarts, self.flap_disable_window_minutes);
...
// ----------------------------
static bool prefs_apply(App& self, HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close, HWND c_dark, HWND c_flap, HWND e_flap_count, HWND e_flap_mins);
...
static bool prefs_apply(App& self, HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close, HWND c_dark, HWND c_flap, HWND e_flap_count, HWND e_flap_mins) {
    ...
    GetWindowTextW(e_flap_mins, b6, 64);
    ...
    if (flap_count < 1 || flap_count > 100) { msgbox_err(dlg, L"Preferences", L"Flap restart threshold must be between 1 and 100."); return false; }
    if (flap_mins < 1 || flap_mins > 120) { msgbox_err(dlg, L"Preferences", L"Flap window (minutes) must be between 1 and 120."); return false; }
    ...
    bool dark_changed = false;
    {
        ...
        BOOL flap_checked = (SendMessageW(c_flap, BM_GETCHECK, 0, 0) == BST_CHECKED);
        self.auto_disable_flapping = flap_checked ? true : false;
        self.flap_disable_count = flap_count;
        self.flap_disable_window_minutes = flap_mins;
...
static LRESULT CALLBACK PrefsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    static HWND e_flap_count, e_flap_mins;
    ...
    case WM_CREATE: {
        ...
        CreateWindowW(L"STATIC", L"Disable after this many restarts:", WS_CHILD | WS_VISIBLE,
            S(32), S(248), S(220), S(20), dlg, NULL, NULL, NULL);
        e_flap_count = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
            S(260), S(246), S(60), S(24), dlg, (HMENU)IDC_PREF_FLAP_COUNT, NULL, NULL);

        CreateWindowW(L"STATIC", L"Within this many minutes:", WS_CHILD | WS_VISIBLE,
            S(32), S(278), S(220), S(20), dlg, NULL, NULL, NULL);
        e_flap_mins = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
            S(260), S(276), S(60), S(24), dlg, (HMENU)IDC_PREF_FLAP_MINUTES, NULL, NULL);
        wchar_t buf[64];
        ...
        SetWindowTextW(e_flap_mins, buf);
        ...
        EnableWindow(e_flap_mins, self.auto_disable_flapping ? TRUE : FALSE);
...
case WM_COMMAND: {
        ...
        if (id == IDC_PREF_AUTO_DISABLE_FLAP && code == BN_CLICKED) {
            BOOL chk = (SendMessageW(c_flap, BM_GETCHECK, 0, 0) == BST_CHECKED);
            EnableWindow(e_flap_count, chk);
            EnableWindow(e_flap_mins, chk);
            return 0;
        }
...
if (id == IDC_TOP_QUIT && code == BN_CLICKED) {
    ...
    return 0;
        ...
        if (id == IDC_PREF_APPLY) { prefs_apply(self, dlg, e_ui, e_mon, e_cd, e_sw, c_tray, c_close, c_dark, c_flap, e_flap_count, e_flap_mins); return 0; }
        if (id == IDOK) {
            if (prefs_apply(self, dlg, e_ui, e_mon, e_cd, e_sw, c_tray, c_close, c_dark, c_flap, e_flap_count, e_flap_mins)) DestroyWindow(dlg);
            return 0;
        }
        return 0;
    }
...
self.force_quit = false;
    self.auto_disable_flapping = false;
    self.flap_disable_count = 3;
    self.flap_disable_window_minutes = 2;

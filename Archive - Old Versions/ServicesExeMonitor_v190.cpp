            ...
            if (wcscmp(it->last_status, ns) != 0) {
                ...
                if (kind == KIND_SVC) {
                    ...
                    if (!it->flap_flagged && it->flap_restarts >= (uint32_t)(self.flap_disable_count > 0 ? self.flap_disable_count : 3)) {
                        ...
                        // Auto-disable (opt-in) — mark pending in the model and perform SCM calls outside the lock.
                        if (self.auto_disable_flapping && !it->flap_auto_disable_attempted) {
                            ...
                            it->status_gen++; // UI-affecting change even if status text doesn't change
                            it->flap_auto_disabled = false;
                            it->flap_auto_disable_err = 0;

// ServicesExeMonitor-modernized.c
...
//
// Build (MinGW-w64):
//   gcc -municode -mwindows -O2 -Wall -Wextra ServicesExeMonitor-modernized.c -o ServicesExeMonitor.exe ^
...
#include <process.h>   // _beginthreadex (MSVCRT)   // _beginthreadex
...
typedef struct Item {
    ...
    time_t last_update_wall;
    uint64_t last_autostop_mono_ms;
    int autostop_count;
} Item;
...
static void list_init(ItemList* L) { memset(L, 0, sizeof(*L)); }
...
static Item* list_find(ItemList* L, const wchar_t* name) {
    for (size_t i = 0; i < L->n; i++) {
        if (L->v[i] && _wcsicmp(L->v[i]->name, name) == 0) return L->v[i];
    }
    return NULL;
}
...
static bool list_push(ItemList* L, Item* it) {
    if (L->n == L->cap) {
        ...
        Item** nv = (Item**)realloc(L->v, nc * sizeof(Item*));
        if (!nv) return false;
        L->v = nv;
        L->cap = nc;
    }
...
static void list_remove_at(ItemList* L, size_t idx) {
    if (idx >= L->n) return;
    Item* it = L->v[idx];
    if (it) { free(it->name); free(it); }
    memmove(&L->v[idx], &L->v[idx + 1], (L->n - idx - 1) * sizeof(Item*));
    L->n--;
}

static bool list_remove_name(ItemList* L, const wchar_t* name) {
    for (size_t i = 0; i < L->n; i++) {
        if (L->v[i] && _wcsicmp(L->v[i]->name, name) == 0) {
            list_remove_at(L, i);
            return true;
        }
    }
...
typedef struct DispMap {
    wchar_t* disp_casefold;
    wchar_t* key;
} DispMap;

typedef struct DispCache {
    DispMap* v;
    size_t n, cap;
} DispCache;
...
static bool dispcache_push(DispCache* D, const wchar_t* disp_lower, const wchar_t* key) {
    ...
    if (D->n == D->cap) {
        size_t nc = (D->cap == 0) ? 256 : (D->cap * 2);
        DispMap* nv = (DispMap*)realloc(D->v, nc * sizeof(DispMap));
        if (!nv) return false;
        D->v = nv; D->cap = nc;
    }

    wchar_t* d = wcsdup_heap(disp_lower);
    wchar_t* k = wcsdup_heap(key);
    if (!d || !k) { free(d); free(k); return false; }

    D->v[D->n].disp_casefold = d;
    D->v[D->n].key = k;
    D->n++;
    return true;
}
...
static const wchar_t* dispcache_lookup(DispCache* D, const wchar_t* disp) {
    ...
    wchar_t tmp[512];
    StringCchCopyW(tmp, 512, disp);
    CharLowerBuffW(tmp, (DWORD)wcslen(tmp));
    for (size_t i = 0; i < D->n; i++) {
        if (D->v[i].disp_casefold && _wcsicmp(D->v[i].disp_casefold, tmp) == 0) return D->v[i].key;
    }
    return NULL;
}
...
static void tray_add(HINSTANCE hInst) {
    ...
    StringCchCopyW(g_app.nid.szTip, ARRAYSIZE(g_app.nid.szTip), APP_TITLE);
...
static bool stop_service_and_wait(const wchar_t* svc_name, int wait_s, wchar_t* out_result, size_t cch) {
    ...
    SERVICE_STATUS_PROCESS ssp;
    DWORD bytes = 0;
    uint64_t deadline = now_mono_ms() + (uint64_t)wait_s * 1000ULL;
...
static void lv_set_row(HWND lv, Item* it) {
    if (!lv || !it || !it->name) return;

    int idx = lv_find_item_by_ptr(lv, it);
    if (idx < 0) idx = lv_find_item_by_name(lv, it->name);
    ...
    if (idx < 0) {
        ...
        ListView_SetItemText(lv, row, 1, (LPWSTR)it->name);
...
static bool parse_setting(const wchar_t* key, const wchar_t* val) {
    ...
    } else if (_wcsicmp(key, L"monitor_interval_s") == 0) {
        double v = _wtof(val);
        if (v >= 0.2) { g_app.monitor_interval_s = v; return true; }
    } else if (_wcsicmp(key, L"autostop_cooldown_s") == 0) {
        double v = _wtof(val);
        if (v >= 0) { g_app.autostop_cooldown_s = v; return true; }
    } else if (_wcsicmp(key, L"stop_wait_s") == 0) {
        int v = (int)_wtof(val);
        if (v >= 0) { g_app.stop_wait_s = v; return true; }
    }
    return false;
}
...
static void load_config(void) {
    ...
    while (line) {
        ...
        Item* exist = list_find(&g_app.items[kind], norm);
        ...
        log_linef(L"Monitoring restored (%s): %s (status: %s)",
                  (kind == KIND_SVC) ? L"service" : L"exe", it->name, it->last_status);
        restored++;
...
static void save_config_now(void) {
    ...
    double autostop_cooldown_s = 15.0;
    int stop_wait_s = 10;
    int tray_enabled = 1;
    int close_to_tray = 0;
    ...
    monitor_interval_s = g_app.monitor_interval_s;
    autostop_cooldown_s = g_app.autostop_cooldown_s;
    stop_wait_s = g_app.stop_wait_s;
    tray_enabled = g_app.tray_enabled ? 1 : 0;
    close_to_tray = g_app.close_to_tray ? 1 : 0;

    ns = g_app.items[KIND_SVC].n;
    ne = g_app.items[KIND_EXE].n;
    ...
    for (size_t i = 0; i < ns; i++) {
        Item* it = g_app.items[KIND_SVC].v[i];
        svcs[i].name = (it && it->name) ? wcsdup_heap(it->name) : NULL;
        svcs[i].auto_stop = (it && it->auto_stop) ? 1 : 0;
    }
    for (size_t i = 0; i < ne; i++) {
        Item* it = g_app.items[KIND_EXE].v[i];
        exes[i].name = (it && it->name) ? wcsdup_heap(it->name) : NULL;
        exes[i].auto_stop = (it && it->auto_stop) ? 1 : 0;
    }
    LeaveCriticalSection(&g_app.lock);

    wchar_t tmp[MAX_PATH];
    StringCchPrintfW(tmp, MAX_PATH, L"%s.tmp", g_app.cfg_path);
    ...
    int n = snprintf(header, sizeof(header),
        ...
        "SETTING|monitor_interval_s|%.6g\n"
        "SETTING|autostop_cooldown_s|%.6g\n"
        "SETTING|stop_wait_s|%d\n"
        "SETTING|tray_enabled|%d\n"
        "SETTING|close_to_tray|%d\n"
        "# Lines:\n"
        "#   SVC|<service_key_name>|<auto_stop 0/1>\n"
        "#   EXE|<exe_name>|<auto_stop 0/1>\n",
        ui_refresh_ms, monitor_interval_s, autostop_cooldown_s, stop_wait_s,
    ...
    fclose(f);
    MoveFileExW(tmp, g_app.cfg_path, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH);
...
// Action worker queue
// ----------------------------
static void action_enqueue(int kind, const wchar_t* name, const wchar_t* reason, int wait_s) {
    Action* a = (Action*)calloc(1, sizeof(Action));
    if (!a) return;
    a->kind = kind;
    a->name = wcsdup_heap(name);
    a->reason = wcsdup_heap(reason);
    a->wait_s = wait_s;
    a->next = NULL;

    EnterCriticalSection(&g_app.action_lock);
    if (!g_app.action_tail) {
        g_app.action_head = g_app.action_tail = a;
    } else {
        g_app.action_tail->next = a;
        g_app.action_tail = a;
    }
    LeaveCriticalSection(&g_app.action_lock);

    SetEvent(g_app.action_event);
}

static Action* action_dequeue(void) {
    EnterCriticalSection(&g_app.action_lock);
    Action* a = g_app.action_head;
    if (a) {
        g_app.action_head = a->next;
        if (!g_app.action_head) g_app.action_tail = NULL;
    }
    LeaveCriticalSection(&g_app.action_lock);
    return a;
}

static unsigned __stdcall action_thread_proc(void* _) {
    (void)_;
    HANDLE waits[2] = { g_app.shutdown_event, g_app.action_event };
    ...
    for (;;) {
        ...
        for (;;) {
            Action* a = action_dequeue();
            if (!a) break;

            if (a->kind == KIND_SVC) {
                wchar_t result[256];
                stop_service_and_wait(a->name, a->wait_s, result, 256);
                wchar_t line[700];
                StringCchPrintfW(line, 700, L"%s: %s \u2192 %s", a->name, a->reason, result);
                post_log(line);
            } else {
                int terminated = 0, failed = 0;
                bool ok = terminate_all_by_name_lower(a->name, &terminated, &failed);
                wchar_t result[256];
                ...
                if (!ok) {
                    ...
                    if (terminated == 0 && failed == 0) {
                        wchar_t cmd[512];
                        StringCchPrintfW(cmd, 512, L"taskkill /IM \"%s\" /F >NUL 2>NUL", a->name);
                        _wsystem(cmd);
                        terminated = 1;
                    }
                    if (a->wait_s > 0) {
                        uint64_t deadline = now_mono_ms() + (uint64_t)a->wait_s * 1000ULL;
                        while (now_mono_ms() < deadline) {
                            if (process_count_by_name_lower(a->name) == 0) break;
                            Sleep(150);
                        }
                    }
                ...
                wchar_t line[800];
                StringCchPrintfW(line, 800, L"%s: %s \u2192 %s", a->name, a->reason, result);
                post_log(line);
            }
...
static unsigned __stdcall monitor_thread_proc(void* _) {
    (void)_;
    ...
    for (;;) {
        if (WaitForSingleObject(g_app.shutdown_event, 0) == WAIT_OBJECT_0) break;
        ...
        monitor_interval_s = g_app.monitor_interval_s;
        cooldown_s = g_app.autostop_cooldown_s;
        stop_wait_s = g_app.stop_wait_s;
        ...
        bool ok_alloc = mons_ensure_snap(&snap, ns, ne) && mons_ensure_scratch(&sc, ns, ne);
        if (!ok_alloc) {
            LeaveCriticalSection(&g_app.lock);
            // If we can't allocate snapshot/scratch buffers, skip this cycle.
            post_log(L\"Monitor: out of memory (skipping cycle)");
            DWORD wait_ms = (DWORD)std::max(50.0, monitor_interval_s * 1000.0);
            if (WaitForSingleObject(g_app.shutdown_event, wait_ms) == WAIT_OBJECT_0) break;
            continue;
        }
        ...
        if (gen_s != snap.gen_s || ns != snap.ns) {
            ...
            for (size_t i = 0; i < ns; i++) {
                Item* it = g_app.items[KIND_SVC].v[i];
                snap.svc_names[i] = (it && it->name) ? wcsdup_heap(it->name) : NULL;
            }
        ...
        if (gen_e != snap.gen_e || ne != snap.ne) {
            ...
            for (size_t i = 0; i < ne; i++) {
                Item* it = g_app.items[KIND_EXE].v[i];
                snap.exe_names[i] = (it && it->name) ? wcsdup_heap(it->name) : NULL;
            }
        ...
        double cd_ms = cooldown_s * 1000.0;
        ...
        if (ns && snap.svc_names && sc.svc_status) {
            ...
            for (size_t i = 0; i < ns; i++) {
                ...
                if (_wcsicmp(sc.svc_status[i], L"unknown") == 0) {
                    wchar_t st[32];
                    if (query_service_status_fast(snap.svc_names[i], st, 32)) StringCchCopyW(sc.svc_status[i], 32, st);
                    ...
                    if (_wcsicmp(sc.svc_status[i], L"stopped") != 0) {
                        if ((double)(mono_now - snap.svc_lastreq[i]) >= cd_ms) {
                            ...
                            if (it) {
                                ...
                                action_enqueue(KIND_SVC, snap.svc_names[i], reason, stop_wait_s);
                            } else {
                                LeaveCriticalSection(&g_app.lock);
                            }
                        }
        ...
        if (ne && snap.exe_names && sc.exe_counts && sc.exe_status) {
            ...
            for (size_t i = 0; i < ne; i++) {
                ...
                if (snap.exe_auto[i] && _wcsicmp(status, L"stopped") != 0) {
                    if ((double)(mono_now - snap.exe_lastreq[i]) >= cd_ms) {
                        EnterCriticalSection(&g_app.lock);
                        Item* it = list_find(&g_app.items[KIND_EXE], snap.exe_names[i]);
                        if (it) {
                            it->last_autostop_mono_ms = mono_now;
                            it->autostop_count++;
                            int k = it->autostop_count;
                            LeaveCriticalSection(&g_app.lock);
                            ...
                            action_enqueue(KIND_EXE, snap.exe_names[i], reason, stop_wait_s);
                        } else {
                            LeaveCriticalSection(&g_app.lock);
                        }
                    }
...
static bool prefs_apply(HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close) {
    ...
    if (mon < 0.2) { msgbox_err(dlg, L"Preferences", L"Monitor Interval must be at least 0.2 s."); return false; }
    if (cd < 0) { msgbox_err(dlg, L"Preferences", L"Autostop Cooldown cannot be negative."); return false; }
    if (sw < 0) { msgbox_err(dlg, L"Preferences", L"Stop Wait cannot be negative."); return false; }

    g_app.ui_refresh_ms = ui;
    g_app.monitor_interval_s = mon;
    g_app.autostop_cooldown_s = cd;
    g_app.stop_wait_s = sw;
...
if (close_checked) tray_checked = TRUE; // close-to-tray requires tray icon

g_app.tray_enabled = tray_checked ? true : false;
g_app.close_to_tray = (g_app.tray_enabled && close_checked) ? true : false;
...
tray_sync(GetModuleHandleW(NULL));
    ...
    log_linef(L"Settings updated: UI %dms, Monitor %.3gs, Cooldown %.3gs, StopWait %ds, Tray %d, CloseToTray %d",
              g_app.ui_refresh_ms, g_app.monitor_interval_s, g_app.autostop_cooldown_s, g_app.stop_wait_s, g_app.tray_enabled ? 1 : 0, g_app.close_to_tray ? 1 : 0);
...
static LRESULT CALLBACK PrefsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowW(L"STATIC", L"Monitor Interval (s):", WS_CHILD | WS_VISIBLE,
        ...
        CreateWindowW(L"STATIC", L"Autostop Cooldown (s):", WS_CHILD | WS_VISIBLE,
        ...
        CreateWindowW(L"STATIC", L"Stop Wait (s):", WS_CHILD | WS_VISIBLE,
...
c_close = CreateWindowW(L"BUTTON", L"Close to tray (hide on X)", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
    12, 176, 240, 22, dlg, (HMENU)IDC_PREF_CLOSE_TO_TRAY, NULL, NULL);
        ...
        StringCchPrintfW(buf, 64, L"%.6g", g_app.monitor_interval_s);
        SetWindowTextW(e_mon, buf);
        StringCchPrintfW(buf, 64, L"%.6g", g_app.autostop_cooldown_s);
        SetWindowTextW(e_cd, buf);
        StringCchPrintfW(buf, 64, L"%d", g_app.stop_wait_s);
...
static LRESULT CALLBACK HelpWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        const wchar_t* help =
            ...
            L"Stop Wait (s)\r\n"
            L"- After requesting stop/terminate, wait up to this many seconds to confirm it stopped.\r\n"
            L"- 0 = fire-and-forget.\r\n\r\n"
...
static void add_item(int kind) {
    ...
    EnterCriticalSection(&g_app.lock);
    Item* exist = list_find(&g_app.items[kind], name);
    LeaveCriticalSection(&g_app.lock);
    if (exist) {
...
static void remove_selected(int kind) {
    ...
    EnterCriticalSection(&g_app.lock);
    bool removed = list_remove_name(&g_app.items[kind], name);
...
static void stop_selected(int kind) {
    ...
    action_enqueue(kind, name, L"manual stop", g_app.stop_wait_s);
}
...
tray_sync(GetModuleHandleW(NULL));
        ...
        g_app.shutdown_event = CreateEventW(NULL, TRUE, FALSE, NULL);
        g_app.action_event = CreateEventW(NULL, FALSE, FALSE, NULL);
        InitializeCriticalSection(&g_app.action_lock);

        g_app.action_thread = (HANDLE)_beginthreadex(NULL, 0, action_thread_proc, NULL, 0, NULL);
        g_app.monitor_thread = (HANDLE)_beginthreadex(NULL, 0, monitor_thread_proc, NULL, 0, NULL);
...
case WM_CLOSE:
    ...
    case WM_DESTROY:
        ...
        if (g_app.shutdown_event) SetEvent(g_app.shutdown_event);

        if (g_app.monitor_thread) {
            WaitForSingleObject(g_app.monitor_thread, 2000);
            CloseHandle(g_app.monitor_thread);
        }
        if (g_app.action_thread) {
            SetEvent(g_app.action_event);
            WaitForSingleObject(g_app.action_thread, 2000);
            CloseHandle(g_app.action_thread);
        }
        ...
        EnterCriticalSection(&g_app.action_lock);
        Action* a = g_app.action_head;
        g_app.action_head = g_app.action_tail = NULL;
        LeaveCriticalSection(&g_app.action_lock);
        while (a) {
            Action* n = a->next;
            free(a->name); free(a->reason); free(a);
            a = n;
        }

        if (g_app.action_event) CloseHandle(g_app.action_event);
        if (g_app.shutdown_event) CloseHandle(g_app.shutdown_event);

        DeleteCriticalSection(&g_app.action_lock);
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int show) {
    ...
    g_app.monitor_interval_s = 1.0;
    g_app.autostop_cooldown_s = 15.0;
    g_app.stop_wait_s = 10;

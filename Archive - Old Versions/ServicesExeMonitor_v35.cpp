...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  static HWND lb_profiles, e_name, b_add, b_rename, b_del, b_up, b_down, st_active, b_close_btn;
  ...
  auto do_layout = [&]() {
    ...
    const int maxScroll = 0; (void)maxScroll; s_scrollY = 0;
    const int yOff = 0;

    auto MW = [&](HWND h, int x, int y, int w, int hgt, BOOL repaint = TRUE) {
      if (!h) return;
      ::MoveWindow(h, x, y + yOff, w, hgt, repaint);
    };
    ...
    if (b_close_btn) {
      MoveWindow(b_close_btn, cw - m - closeW, viewH - m - btnH, closeW, btnH, TRUE);
    }
  ...
  case WM_CREATE: {
    ...
    b_close_btn = CreateWindowW(L"BUTTON", L"Close", WS_CHILD | WS_VISIBLE,
...
static void build_ui(App& self, HWND hwnd) {
  ...
  self.activity = CreateWindowExW(0, L"LISTBOX", L"",
    WS_CHILD | WS_VISIBLE | WS_HSCROLL | LBS_NOINTEGRALHEIGHT,

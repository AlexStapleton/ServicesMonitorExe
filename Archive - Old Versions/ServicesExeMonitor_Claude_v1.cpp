...
//   g++ -std=c++20 -municode -mwindows -O2 -Wall -Wextra ServicesExeMonitor_Claude_v1.cpp app_res.o -o ServicesExeMonitor.exe ^
...
// --------------------------------------------------
constexpr long  kMaxConfigBytes      = 5 * 1024 * 1024;   // 5 MB config file limit
constexpr DWORD kScmPollMinMs        = 100;                // Min SCM wait-hint clamp
constexpr DWORD kScmPollMaxMs        = 2000;               // Max SCM wait-hint clamp
constexpr DWORD kExePollFallbackMs   = 250;                // Fallback poll for EXE start/stop
constexpr DWORD kTaskkillTimeoutMs   = 5000;               // Timeout for taskkill fallback
constexpr int   kStatusDispBuf       = 128;                // Status display string buffer (wchar_t)
constexpr DWORD kForceTermWaitMs     = 2000;               // Wait after TerminateProcess before giving up
// --------------------------------------------------
...
static constexpr int ki(ItemKind k) {
...
// WM_APP + 2 formerly WM_APP_STATUS — removed (dead code)
constexpr UINT WM_APP_REQUEST_SAVE     = WM_APP + 3;
constexpr UINT WM_APP_RESTART_UI_TIMER = WM_APP + 4;
// WM_APP + 5 formerly WM_APP_STATUS_BULK — removed (dead code)
...
// Function-like macros (must remain as #define)
...
// --------------------------------------------------
// Control IDs — main window (5000-6999)
// --------------------------------------------------
constexpr int IDC_LEFT_TABS      = 5001;
constexpr int IDC_CENTER_TABS    = 5002;
constexpr int IDC_TOP_SETTINGS   = 5003;
constexpr int IDC_TOP_QUIT       = 5004;
constexpr int IDC_TOP_EXIT_TO_TRAY = 5005;

// Left pages — services
constexpr int IDC_SVC_ADD_EDIT   = 5101;
constexpr int IDC_SVC_ADD_BTN    = 5102;
constexpr int IDC_SVC_STOP_BTN   = 5103;
constexpr int IDC_SVC_REMOVE_BTN = 5104;
constexpr int IDC_SVC_START_BTN  = 5105;

// Left pages — executables
constexpr int IDC_EXE_ADD_EDIT   = 5201;
constexpr int IDC_EXE_ADD_BTN    = 5202;
constexpr int IDC_EXE_STOP_BTN   = 5203;
constexpr int IDC_EXE_REMOVE_BTN = 5204;
constexpr int IDC_EXE_BROWSE_BTN = 5205;
constexpr int IDC_EXE_START_BTN  = 5206;
...
// --------------------------------------------------
// Control IDs — shared / splitter (7000-7199)
// --------------------------------------------------
constexpr int IDC_ACTIVITY        = 7001;
constexpr int IDC_SPLITTER        = 7002;
constexpr int IDC_LEFT_HSPLIT_SVC = 7003;
constexpr int IDC_LEFT_HSPLIT_EXE = 7004;
constexpr int TRAY_UID            = 1;

// --------------------------------------------------
// Control IDs — dialogs (7200-8999)
// --------------------------------------------------
...
constexpr int ACTIVITY_MIN_W  = 200;
constexpr int CENTER_MIN_W    = 260;
...
// Main window controls: 5000-6999
// Dialog/tools controls: 7200-8999
static_assert(IDC_LV_SVC != IDC_LV_EXE, "Listview IDs must be distinct");
static_assert(IDC_HOTKEYS_LIST >= 7200 && IDC_HOTKEYS_LIST < 9000, "Hotkeys dialog IDs must live in dialog ID band");
static_assert(IDC_HOTKEYS_COPY >= 7200 && IDC_HOTKEYS_COPY < 9000, "Hotkeys dialog IDs must live in dialog ID band");
...
struct StatusStr; // forward declaration (defined near MonScratch)
...
static void post_status_bulk(App& self, ItemKind kind, const std::vector<std::wstring>& names, const std::vector<StatusStr>& statuses, size_t n, time_t wall_ts);
...
// Lowercase a wide string in-place (uses Win32 CharLowerBuffW for locale-aware folding).
static void str_lower_inplace(std::wstring& s) {
    if (!s.empty()) CharLowerBuffW(s.data(), (DWORD)s.size());
}

static void str_lower_inplace(wchar_t* s) {
...
// wcsdup_heap removed — all callers now use std::wstring.
...
// Cached DLL handles and proc addresses for dwmapi.dll and uxtheme.dll.
// These DLLs are loaded once on first use and kept for the process lifetime,
// avoiding repeated LoadLibrary/FreeLibrary on every theme or titlebar call.
static struct {
    bool inited = false;
    HMODULE dwm = NULL;
    HMODULE ux  = NULL;
...
static void dll_cache_init() {
    if (g_dll_cache.inited) return;
    g_dll_cache.inited = true;
    g_dll_cache.dwm = LoadLibraryW(L"dwmapi.dll");
    g_dll_cache.ux  = LoadLibraryW(L"uxtheme.dll");
    if (g_dll_cache.dwm) {
...
static void toggle_autostop_selected(App& self, ItemKind kind);
static bool cmd_set_autostop_uid(App& self, uint32_t uid, bool enabled, std::wstring* out_name_opt = nullptr);
static void show_item_details(App& self, ItemKind kind);
...
//   - Backslashes before a quote (or end of arg) are doubled
//   - Embedded quotes are escaped as \"
static std::wstring escape_argv(const wchar_t* arg) {
    std::wstring out;
    out.push_back(L'"');
...
// ----------------------------
struct Item {
    ItemKind kind = ItemKind::Svc;       // Svc / Exe
    uint32_t uid = 0;                   // stable ID for UI lParam mapping
    std::wstring name;                   // owning
    std::wstring name_lower;             // cached lowercase name (set once on creation; avoids per-filter allocations)
    ...
    wchar_t  ui_cache_status_disp[kStatusDispBuf]{};   // e.g. "* running" or "running (Profile)"
...
// dispcache_lookup: find service key name by lowercased display name.
...
struct Action {
    ...
    DWORD pid = 0;
    unique_handle hproc; // duplicated handle; RAII ensures cleanup
};

// ----------------------------
// Sub-structs (break up the App god-object into focused groups)
// ----------------------------

// Visual theme: dark/light mode, cached colours and GDI brushes.
struct ThemeState {
...
// Profile system: named config snapshots activated by watched EXE presence.
struct ProfileState {
    ...
    int active = -1;                       // -1 = default, otherwise index into `profiles`
    ...
    std::shared_ptr<const std::vector<std::vector<uint64_t>>> watch_hashes_sp; // hashed watch keys (lower-case)
...
// System tray: icon, notification state, minimize-to-tray behaviour.
struct TrayState {
    bool enabled = false;
    bool close_to_tray = false;
    bool added = false;
...
// Config settings: tuning knobs and config file path.
...
// Threading: worker threads, action queue, condition variables, save coalescing.
struct ThreadState {
    std::jthread monitor_thread;
    std::jthread action_thread;

    // Condition variable to wake the monitor thread early (e.g. when config changes).
    // Uses App::mtx as its associated mutex.
    ...
    //   - Never wait on action_cv / lock action_mtx while holding App::mtx.
    //   - All enqueue operations MUST happen without holding App::mtx.
    //
...
//   - self.threads.action_mtx + self.threads.action_cv (action queue, in ThreadState)
//
...
// RAII guard for suppress_lv_notify (prevents re-entrant LVN_ITEMCHANGED processing).
// Use this instead of manually saving/restoring the bool, to guarantee exception-safety
// and avoid forgetting to restore the flag on early returns.
...
static void item_cache_name_lower(Item* it) {
    if (!it) return;
...
static void action_enqueue(App& self, ItemKind kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms);
static void update_statusbar(App& self);
static void apply_filter_now(App& self, ItemKind kind);
static void capture_columns_now(App& self, ItemKind kind);
static void apply_saved_columns(App& self, ItemKind kind);
...
static bool parse_setting(App& self, const wchar_t* key, const wchar_t* val) {
    ...
    if (_wcsicmp(key, L"ui_refresh_ms") == 0) {
        int v = 0; parse_int(&v);
        if (v >= 200) { self.cfg.ui_refresh_ms = v; return true; }
        return false;
    }
...
// ----------------------------
// ----------------------------
// Name index for batch process/service matching (moved early for forward declarations)
// ----------------------------
struct NameIdx {
    const wchar_t* name;
    size_t idx;
...
static LRESULT theme_customdraw_listview(App& self, NMLVCUSTOMDRAW* lcd) {
    if (!self.theme.dark_mode || !lcd) return CDRF_DODEFAULT;
...
static bool theme_draw_owner_button(App& self, const DRAWITEMSTRUCT* dis) {
    ...
    // Background
    HBRUSH br = CreateSolidBrush(pressed ? RGB(55,55,55) : self.theme.col_panel);
    FillRect(hdc, &rc, br);
    DeleteObject(br);
...
static bool theme_draw_owner_tab(App& self, const DRAWITEMSTRUCT* dis) {
    ...
    HBRUSH br = CreateSolidBrush(selected ? RGB(55,55,55) : self.theme.col_panel);
    FillRect(hdc, &rc, br);
    DeleteObject(br);
...
static bool theme_draw_owner_menu(App& self, const DRAWITEMSTRUCT* dis) {
    ...
    HBRUSH br = CreateSolidBrush(selected ? RGB(55,55,55) : self.theme.col_bg);
    FillRect(hdc, &rc, br);
    DeleteObject(br);
...
#define EM_SETBKGNDCOLOR (WM_USER + 67)
    ...
    if (lstrcmpiW(cls, L"EDIT") == 0) {
        // Standard EDIT controls are themed via WM_CTLCOLOREDIT.
        // Apply DarkMode_CFD for dark scrollbars on multiline edit controls.
        if (self.theme.dark_mode) {
            dll_cache_init();
            if (g_dll_cache.SetWindowTheme)
                g_dll_cache.SetWindowTheme(child, L"DarkMode_CFD", nullptr);
            ...
            dll_cache_init();
            if (g_dll_cache.SetWindowTheme)
                g_dll_cache.SetWindowTheme(child, L"Explorer", nullptr);
        }
        InvalidateRect(child, NULL, TRUE);
        return;
    }
    if (lstrcmpiW(cls, L"ListBox") == 0) {
        // Apply DarkMode_CFD for dark scrollbars on listbox controls.
        if (self.theme.dark_mode) {
            dll_cache_init();
            if (g_dll_cache.SetWindowTheme)
                g_dll_cache.SetWindowTheme(child, L"DarkMode_CFD", nullptr);
...
static void load_config(App& self) {
    ...
    if (sz <= 0 || sz > kMaxConfigBytes) { fclose(f); return; }
...
static void save_config_now(App& self) {
    ...
    int svc_cw[4], exe_cw[4], svc_co[4], exe_co[4];
    for (int i = 0; i < 4; i++) {
...
"SETTING|exe_colorder|%d,%d,%d,%d\n"
        ...
        lsy_svc,
        lsy_exe,
        svc_cw[0], svc_cw[1], svc_cw[2], svc_cw[3],
        exe_cw[0], exe_cw[1], exe_cw[2], exe_cw[3],
        svc_co[0], svc_co[1], svc_co[2], svc_co[3],
        exe_co[0], exe_co[1], exe_co[2], exe_co[3]
    );
    fwrite(header, 1, (size_t)n, f);

    std::vector<char> utf8_buf;  // reusable buffer for UTF-8 conversion
    auto write_wline = [&](const wchar_t* wline) {
        int blen = WideCharToMultiByte(CP_UTF8, 0, wline, -1, NULL, 0, NULL, NULL);
        if (blen <= 0) return;
        utf8_buf.resize((size_t)blen);
        WideCharToMultiByte(CP_UTF8, 0, wline, -1, utf8_buf.data(), blen, NULL, NULL);
        fwrite(utf8_buf.data(), 1, (size_t)(blen - 1), f);
    };
        ...
        if (a.kind == ItemKind::Svc) {
            ...
            if (!ok) {
                ...
                if (terminated == 0 && failed == 0) {
                    // Use CreateProcessW instead of _wsystem to avoid command injection.
                    wchar_t cmdline[512];
                    StringCchPrintfW(cmdline, 512, L"taskkill.exe /IM \"%s\" /F", a.name.c_str());
                    STARTUPINFOW si{};
                    si.cb = sizeof(si);
                    ...
                    if (CreateProcessW(NULL, cmdline, NULL, NULL, FALSE,
                        ...
                        WaitForSingleObject(pi.hProcess, kTaskkillTimeoutMs);
                ...
                if (a.wait_ms > 0 && !term_handles.empty()) {
                    ...
                    // Fallback: no handles (taskkill path) — poll briefly.
...
// ----------------------------
// NameIdx and binsearch_nameidx are defined earlier (near forward declarations).


// Fixed-size wide-string buffer for service/exe status text.
...
// Wait for up to total_ms, but wake immediately if stop is requested or mon_cv is signalled.
...
// --------------------------------------------------
// Legacy app() macro removed — all functions use `App& self` directly.
...
static LRESULT CALLBACK DarkButtonSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam,
    ...
    switch (msg) {
    case WM_ERASEBKGND:
        if (self && self->theme.dark_mode) return 1;
        break;
    case WM_ENABLE:
    ...
    case WM_STYLECHANGED:
        ...
        break;
    case WM_PAINT: {
        if (!self || !self->theme.dark_mode) break;
        ...
        COLORREF bg = pressed ? RGB(55,55,55) : self->theme.col_panel;
...
// ----------------------------
// Log messages posted to the UI thread as heap-allocated std::wstring (cleaned up by receiver).
static void post_log(App& self, const wchar_t* text) {
...
// StatusMsg removed — status updates now use generation-based post_status_bulk (no heap churn).
...
static bool build_running_exe_hashes_lower(std::vector<uint64_t>& out_hashes,
    ...
    do {
        if (pe.szExeFile[0]) {
            ...
            if (idx_sorted && idx_n > 0 && out_counts) {
                size_t orig = 0;
...
static bool item_matches_filter(const Item* it, const std::wstring& flt_lower) {
    ...
    // Match against status (small fixed buffer; must still lowercase per-call)
    wchar_t st_lower[32];
    StringCchCopyW(st_lower, 32, it->last_status);
    str_lower_inplace(st_lower);
    if (wcsstr(st_lower, flt_lower.c_str()) != nullptr) return true;
...
// Shared NM_CUSTOMDRAW dispatch for dark-mode custom drawing.
// Used by the main window and all dialog WndProcs.
static LRESULT theme_handle_customdraw(App& self, NMHDR* nh, LPARAM lParam, bool& handled) {
    handled = false;
    ...
    if (lstrcmpiW(ccls, STATUSCLASSNAMEW) == 0) {
        ...
        if (cd->dwDrawStage == CDDS_ITEMPREPAINT) {
            SetTextColor(cd->hdc, self.theme.col_text);
            SetBkColor(cd->hdc, self.theme.col_panel);
            handled = true;
            return CDRF_NEWFONT;
        }
    }
...
static void prof_fill_content_list(App& self, HWND lb, int prof_idx, ItemKind kind) {
    ...
    std::vector<ItemRow> rows;
    {
        ModelLockGuard lk(self);
        if (prof_idx >= 0 && prof_idx < (int)self.prof.profiles.size()) {
            rows = self.prof.profiles[(size_t)prof_idx].cfg.items[ki(kind)];
        }
    }
...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    int tab = 0;
    if (pst->tab_right) {
        tab = TabCtrl_GetCurSel(pst->tab_right);
        if (tab < 0) tab = 0;
    }
    ...
    if (!pst->tab_right) return;

    DW(pst->tab_right, xR, yTop, rightW, yBot - yTop);

    // Page area inside the tab
    RECT pr{ 0, 0, rightW, yBot - yTop };
    TabCtrl_AdjustRect(pst->tab_right, FALSE, &pr);
    int px = xR + pr.left;
    int py = yTop + pr.top;
...
pst->tab_right = CreateWindowExW(0, WC_TABCONTROLW, L"", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
    280, 10, 520, 560, dlg, (HMENU)IDC_PROFILES_TAB, NULL, NULL);
if (pst->tab_right) {
    TCITEMW ti{};
    ti.mask = TCIF_TEXT;
    ti.pszText = (LPWSTR)L"Activation";
    TabCtrl_InsertItem(pst->tab_right, 0, &ti);
    ti.pszText = (LPWSTR)L"Start Items";
    TabCtrl_InsertItem(pst->tab_right, 1, &ti);
    ti.pszText = (LPWSTR)L"Services";
    TabCtrl_InsertItem(pst->tab_right, 2, &ti);
    ti.pszText = (LPWSTR)L"EXEs";
    TabCtrl_InsertItem(pst->tab_right, 3, &ti);
    TabCtrl_SetCurSel(pst->tab_right, 0);
}
     ...
     660, 1040, 80, 28, dlg, (HMENU)IDCANCEL, NULL, NULL);
        ...
        if (f) {
            ...
            if (pst->tab_right) SendMessageW(pst->tab_right, WM_SETFONT, (WPARAM)f, TRUE);
        }
    ...
    case WM_NOTIFY: {
        auto* nh = reinterpret_cast<NMHDR*>(lParam);
        if (nh && nh->hwndFrom == pst->tab_right && nh->code == TCN_SELCHANGE) {
    ...
    case WM_COMMAND: {
        ...
        if (id == IDC_PCONT_SVC_TOGGLE) {
            ...
            if (changed) {
                request_save_debounced(self);
                prof_fill_content_list(self, pst->lb_svc, psel, ItemKind::Svc);
                if (isel >= 0) SendMessageW(pst->lb_svc, LB_SETCURSEL, (WPARAM)isel, 0);
                refresh_for_selection();
                if (apply_now) apply_active_profile_cfg_inplace(self);
            }
            return 0;
        ...
        if (id == IDC_PCONT_EXE_TOGGLE) {
            ...
            if (changed) {
                request_save_debounced(self);
                prof_fill_content_list(self, pst->lb_exe, psel, ItemKind::Exe);
                if (isel >= 0) SendMessageW(pst->lb_exe, LB_SETCURSEL, (WPARAM)isel, 0);
                refresh_for_selection();
                if (apply_now) apply_active_profile_cfg_inplace(self);
            }
            return 0;
...
static LRESULT CALLBACK SplitterProc(HWND sw, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        COLORREF hilite = self.theme.dark_mode ? RGB(80, 80, 80) : GetSysColor(COLOR_3DHILIGHT);
...
apply_saved_columns(self, ItemKind::Exe);
    ...
    // Image lists are created and attached by theme_compute() + theme_apply_all_controls(),
    // which runs after build_ui. No need to duplicate that work here.
...
static void capture_columns_now(App& self, ItemKind kind) {
    ...
    for (int i = 0; i < ncol; i++) {
        self.cols_w[k][i] = ListView_GetColumnWidth(lv, i);
    }
    int order[8]{0};
...
static void add_item(App& self, ItemKind kind) {
    const int k = ki(kind);
    HWND page = (kind == ItemKind::Svc) ? self.left_page_svc : self.left_page_exe;
    int editId = (kind == ItemKind::Svc) ? IDC_SVC_ADD_EDIT : IDC_EXE_ADD_EDIT;
...
static void start_selected(App& self, ItemKind kind) {
    ...
    {
        ModelLockGuard lk(self);
        wait_ms = self.cfg.stop_wait_ms; // reuse existing wait window
    }
...
if (itp->ui_cache_status_disp[0] == 0 || itp->ui_cache_last_text[0] == 0) {
    ...
    if (itp->ui_cache_status_disp[0] == 0) {
        if (itp->last_status[0]) StringCchCopyW(itp->ui_cache_status_disp, kStatusDispBuf, itp->last_status);
        ...
        if (lvh) {
            ...
            if (ListView_SubItemHitTest(lvh, &hti) != -1) {
                ...
                if (on_checkbox && hti.iItem >= 0) {
                    ...
                    if (uid) {
                        // NM_CLICK can arrive before the ListView updates the checkbox state image,
                        // so ListView_GetCheckState() may still return the OLD value here.
                        // Handle checkbox clicks manually: invert the current state, update the UI,
                        // then sync the model by UID.
...
// Checkbox toggle (auto-stop)
            if ((lv->uNewState ^ lv->uOldState) & LVIS_STATEIMAGEMASK) {
                if (lv->iItem >= 0) {
                    ...
                    // Derive the new checkbox value directly from the notification state image.
                    // Using ListView_GetCheckState() here can read a stale value during LVN_ITEMCHANGED
                    // in some owner-data/notification timing paths, causing the toggle to not stick.
                    const UINT state_img = ((UINT)lv->uNewState & LVIS_STATEIMAGEMASK) >> 12; // 1=unchecked, 2=checked
    ...
    if (nh->code == LVN_KEYDOWN) {
        ...
        if (kd->wVKey == VK_SPACE) {
            // Keyboard parity with context menu: toggle auto-stop on selected row.
...
static void post_model_dirty(App& self) {
    ...
    // Wake the monitor thread so it picks up config/list changes promptly.
...
self.tray.taskbar_restart_msg = RegisterWindowMessageW(L"TaskbarCreated");
        ...
        self.threads.action_thread = std::jthread(action_thread_main, &self);
    ...
    case WM_SETTINGCHANGE: {
        // Detect Windows dark/light mode system change.
        // When the user changes "Colors" in Windows Settings, Windows broadcasts
        // WM_SETTINGCHANGE with lParam = "ImmersiveColorSet".
        if (lParam && wcscmp((const wchar_t*)lParam, L"ImmersiveColorSet") == 0) {
            // Query the system preference from the registry
            HKEY hk = NULL;
            bool sys_dark = false;
...
if (nh && nh->code == NM_CUSTOMDRAW) {
    ...
    ItemKind kind = (sel == 0) ? ItemKind::Svc : ItemKind::Exe;
    const int kind_i = ki(kind);
...
switch (id) {
        case IDC_SVC_ADD_BTN: add_item(self, ItemKind::Svc); return 0;
        case IDC_EXE_ADD_BTN: add_item(self, ItemKind::Exe); return 0;
        case IDC_SVC_REMOVE_BTN: remove_selected(self, ItemKind::Svc); return 0;
        case IDC_EXE_REMOVE_BTN: remove_selected(self, ItemKind::Exe); return 0;
        case IDC_SVC_STOP_BTN:  stop_selected(self, ItemKind::Svc); return 0;
        case IDC_SVC_START_BTN: start_selected(self, ItemKind::Svc); return 0;
        case IDC_EXE_STOP_BTN:  stop_selected(self, ItemKind::Exe); return 0;
        case IDC_EXE_START_BTN: start_selected(self, ItemKind::Exe); return 0;
        }
        return 0;
    }
    ...
    // WM_APP_STATUS removed — status updates now use generation-based WM_APP_STATUS_GEN above.
...
if (wParam == TIMER_SEARCH_DEBOUNCE) {
    ...
    case WM_DESTROY:
        ...
        if (self.threads.action_thread.joinable()) self.threads.action_thread.request_stop();
        self.threads.action_cv.notify_all();

        if (self.threads.monitor_thread.joinable()) self.threads.monitor_thread.join();
        if (self.threads.action_thread.joinable()) self.threads.action_thread.join();

        save_config_now(self);

        // Drain any pending WM_APP_LOG messages so their heap-allocated std::wstring is freed.
        {
            MSG drain_msg;
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int show) {
    ...
    // cfg defaults are set in ConfigSettings struct definition
    ...
    // prof.active and prof.have_default_cfg use struct defaults (-1, false)
    self.prof.watch_keys_sp = std::make_shared<std::vector<std::vector<std::wstring>>>();
    self.prof.watch_hashes_sp = std::make_shared<std::vector<std::vector<uint64_t>>>();

    self.tray.enabled = true;   // show tray icon by default (overrides struct default)

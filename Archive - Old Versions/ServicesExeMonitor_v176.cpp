...
static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) {
    ...
    if (lv == self.lv_svc) {
        if (it->flap_auto_disable_pending) {
            StringCchCatW(status_disp, 64, L" [AUTO-DISABLE PENDING]");
        } else if (it->flap_auto_disabled) {
            StringCchCatW(status_disp, 64, L" [AUTO-DISABLED]");
        } else if (it->flap_auto_disable_attempted && it->flap_auto_disable_err) {
            wchar_t eb[32];
            StringCchPrintfW(eb, 32, L" [AUTO-DISABLE FAILED e%lu]", (unsigned long)it->flap_auto_disable_err);

...
static void tray_add(HINSTANCE hInst) {
    ...
    app().nid.uCallbackMessage = WM_APP_TRAYICON;

    HICON ico = load_app_icon(hInst, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON));
    app().nid.hIcon = ico ? ico : LoadIconW(NULL, IDI_APPLICATION);
    StringCchCopyW(app().nid.szTip, ARRAYSIZE(app().nid.szTip), app().app_title.c_str());
...
static void tray_sync(HINSTANCE hInst) {
    ...
    if (!app().tray_added) tray_add(hInst);
}
...
static void tray_show_menu(HWND hwnd) {
    ...
    UINT ctt = app().close_to_tray ? MF_CHECKED : 0;
...
static void post_log(const wchar_t* text) {
    ...
    if (!PostMessageW(app().hwnd, WM_APP_LOG, 0, (LPARAM)m)) {
        free(m->text);
        free(m);
    }
}
...
static void post_status(int kind, const wchar_t* name, const wchar_t* status, time_t wall_ts) {
    ...
    if (!PostMessageW(app().hwnd, WM_APP_STATUS, 0, (LPARAM)m)) {
        free(m->name);
        free(m);
    }
}
...
static void post_status_bulk(int kind, wchar_t** names, wchar_t (*statuses)[32], size_t n, time_t wall_ts) {
    ...
    if (!PostMessageW(app().hwnd, WM_APP_STATUS_BULK, 0, (LPARAM)m)) {
        free(m);
    }
}

static void request_save_debounced(void) {
    PostMessageW(app().hwnd, WM_APP_REQUEST_SAVE, 0, 0);
}
...
static void resolve_service_name(const wchar_t* raw, wchar_t* out, size_t cch) {
    ...
    const wchar_t* key = dispcache_lookup(&app().disp_cache, out);
    if (key) { StringCchCopyW(out, cch, key); return; }

    enum_services_build_disp_cache(&app().disp_cache);
    key = dispcache_lookup(&app().disp_cache, out);
    if (key) { StringCchCopyW(out, cch, key); return; }
}
...
static void lv_set_row(HWND lv, Item* it) {
    ...
    if (idx < 0) {
        ...
        app().suppress_lv_notify = true;
        ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE);
        app().suppress_lv_notify = false;
    } else {
...
static void log_linef(const wchar_t* fmt, ...) {
    ...
    activity_append(app().activity, line);
}
...
static bool parse_setting(const wchar_t* key, const wchar_t* val) {
    ...
    if (_wcsicmp(key, L"ui_refresh_ms") == 0) {
        int v = 0;
        parse_int(&v);
        if (v >= 200) { app().ui_refresh_ms = v; return true; }
        return false;
    }

    if (_wcsicmp(key, L"monitor_interval_ms") == 0) {
        int v = 0; parse_int(&v);
        if (v >= 200) { app().monitor_interval_ms = v; return true; }
        return false;
    }
    if (_wcsicmp(key, L"monitor_interval_s") == 0) {
        int ms = 0;
        if (parse_seconds_to_ms(&ms) && ms >= 200) { app().monitor_interval_ms = ms; return true; }
        return false;
    }

    if (_wcsicmp(key, L"autostop_cooldown_ms") == 0) {
        int v = 0; parse_int(&v);
        if (v >= 0) { app().autostop_cooldown_ms = v; return true; }
        return false;
    }
    if (_wcsicmp(key, L"autostop_cooldown_s") == 0) {
        int ms = 0;
        if (parse_seconds_to_ms(&ms)) { app().autostop_cooldown_ms = ms; return true; }
        return false;
    }

    if (_wcsicmp(key, L"stop_wait_ms") == 0) {
        int v = 0; parse_int(&v);
        if (v >= 0) { app().stop_wait_ms = v; return true; }
        return false;
    }
    if (_wcsicmp(key, L"stop_wait_s") == 0) {
        int ms = 0;
        if (parse_seconds_to_ms(&ms)) { app().stop_wait_ms = ms; return true; }
        return false;
    }
    ...
    if (_wcsicmp(key, L"tray_enabled") == 0) {
        int v = 0; parse_int(&v);
        app().tray_enabled = (v != 0);
        if (!app().tray_enabled) app().close_to_tray = false;
        return true;
    }
    if (_wcsicmp(key, L"close_to_tray") == 0) {
        int v = 0; parse_int(&v);
        app().close_to_tray = (v != 0);
        if (!app().tray_enabled) app().close_to_tray = false;
        return true;
    }
...
static void load_config(void) {
    ...
    while (line) {
        ...
        Item* exist = NULL;
        {
            ...
            exist = list_find(&app().items[kind], norm);
        }
        ...
        {
            std::lock_guard<std::mutex> lk(app().mtx);
            list_push(&app().items[kind], it);
            app().items_gen[kind]++;
        }

        lv_set_row(kind == KIND_SVC ? app().lv_svc : app().lv_exe, it);
    ...
    PostMessageW(app().hwnd, WM_APP_RESTART_UI_TIMER, 0, 0);
}
...
static void save_config_now(void) {
    ...
    // Snapshot settings + counts under lock.
    {
        ...
        ui_refresh_ms = app().ui_refresh_ms;
        monitor_interval_ms = app().monitor_interval_ms;
        autostop_cooldown_ms = app().autostop_cooldown_ms;
        stop_wait_ms = app().stop_wait_ms;
        tray_enabled = app().tray_enabled ? 1 : 0;
        close_to_tray = app().close_to_tray ? 1 : 0;

        ns = app().items[KIND_SVC].v.size();
        ne = app().items[KIND_EXE].v.size();

        StringCchCopyW(cfg_path, MAX_PATH, app().cfg_path);
    }
    ...
    // Copy monitored items under lock (best-effort if list changed while allocating).
    {
        ...
        size_t cur_ns = app().items[KIND_SVC].v.size();
        size_t cur_ne = app().items[KIND_EXE].v.size();

        size_t copy_ns = (cur_ns < ns) ? cur_ns : ns;
        size_t copy_ne = (cur_ne < ne) ? cur_ne : ne;

        for (size_t i = 0; i < copy_ns; i++) {
            Item* it = app().items[KIND_SVC].v[i].get();
        ...
        for (size_t i = 0; i < copy_ne; i++) {
            Item* it = app().items[KIND_EXE].v[i].get();
...
static void action_enqueue(int kind, const wchar_t* name, const wchar_t* reason, int wait_ms) {
    ...
    {
        std::lock_guard<std::mutex> lk(app().action_mtx);
        app().action_q.emplace_back(std::move(a));
    }
    app().action_cv.notify_one();
}
...
static void action_thread_main(std::stop_token st) {
    for (;;) {
        Action a;
        {
            std::unique_lock<std::mutex> lk(app().action_mtx);
            app().action_cv.wait(lk, [&] { return st.stop_requested() || !app().action_q.empty(); });
            if (st.stop_requested()) break;

            a = std::move(app().action_q.front());
            app().action_q.pop_front();
        }
...
static void monitor_thread_main(std::stop_token st) {
    ...
    for (;;) {
        ...
        // Snapshot pointers + cheap fields under lock.
        {
            ...
            monitor_interval_ms = app().monitor_interval_ms;
            cooldown_ms = app().autostop_cooldown_ms;
            stop_wait_ms = app().stop_wait_ms;

            ns = app().items[KIND_SVC].v.size();
            ne = app().items[KIND_EXE].v.size();
            gen_s = app().items_gen[KIND_SVC];
            gen_e = app().items_gen[KIND_EXE];
                ...
                if (snap.svc_auto[i] && !should_skip_autostop_for_service_status(sc.svc_status[i])) {
                    if (_wcsicmp(sc.svc_status[i], L"stopped") != 0) {
                        if ((mono_now - snap.svc_lastreq[i]) >= cd_ms) {
                            ...
                            {
                                std::lock_guard<std::mutex> lk(app().mtx);
                                Item* it = list_find(&app().items[KIND_SVC], snap.svc_names[i]);
                                if (it) {
            ...
            for (size_t i = 0; i < ne; i++) {
                ...
                if (snap.exe_auto[i] && _wcsicmp(status, L"stopped") != 0) {
                    if ((mono_now - snap.exe_lastreq[i]) >= cd_ms) {
                        ...
                        {
                            std::lock_guard<std::mutex> lk(app().mtx);
                            Item* it = list_find(&app().items[KIND_EXE], snap.exe_names[i]);
                            if (it) {
                                it->last_autostop_mono_ms = mono_now;
                                it->autostop_count++;
                                k = it->autostop_count;
...
static bool prefs_apply(HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close) {
...

    {
        ...
        app().ui_refresh_ms = ui;
        app().monitor_interval_ms = mon;
        app().autostop_cooldown_ms = cd;
        app().stop_wait_ms = sw;
        ...
        app().tray_enabled = tray_en;
        app().close_to_tray = tray_en && close_en;
...
tray_sync(GetModuleHandleW(NULL));
    ...
    log_linef(L"Settings updated: UI %dms, Monitor %dms, Cooldown %dms, StopWait %dms, Tray %d, CloseToTray %d",
              app().ui_refresh_ms, app().monitor_interval_ms, app().autostop_cooldown_ms, app().stop_wait_ms, app().tray_enabled ? 1 : 0, app().close_to_tray ? 1 : 0);
...
c_close = CreateWindowW(L"BUTTON", L"Close to tray (hide on X)", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
    12, 176, 240, 22, dlg, (HMENU)IDC_PREF_CLOSE_TO_TRAY, NULL, NULL);
        wchar_t buf[64];
        StringCchPrintfW(buf, 64, L"%d", app().ui_refresh_ms);
        SetWindowTextW(e_ui, buf);
        StringCchPrintfW(buf, 64, L"%d", app().monitor_interval_ms);
        SetWindowTextW(e_mon, buf);
        StringCchPrintfW(buf, 64, L"%d", app().autostop_cooldown_ms);
        SetWindowTextW(e_cd, buf);
        StringCchPrintfW(buf, 64, L"%d", app().stop_wait_ms);
        SetWindowTextW(e_sw, buf);

SendMessageW(c_tray, BM_SETCHECK, app().tray_enabled ? BST_CHECKED : BST_UNCHECKED, 0);
SendMessageW(c_close, BM_SETCHECK, app().close_to_tray ? BST_CHECKED : BST_UNCHECKED, 0);


        wchar_t cfgline[MAX_PATH + 64];
        StringCchPrintfW(cfgline, MAX_PATH + 64, L"Config file: %s", app().cfg_path);
        ...
        if (app().ui_font) apply_font_recursive(dlg, app().ui_font);
        return 0;
    }
    case WM_COMMAND: {
...
static LRESULT CALLBACK SplitterProc(HWND sw, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        app().splitter_drag_start_x = pt.x;
        app().splitter_drag_start_w = (app().activity_panel_w > 0) ? app().activity_panel_w : 360;
        return 0;
    }
    ...
    case WM_MOUSEMOVE:
        if (app().splitter_dragging) {
            ...
            int dx = pt.x - app().splitter_drag_start_x;
            int desired = app().splitter_drag_start_w - dx; // drag right => narrower Activity panel
            app().activity_panel_w = desired;
...
static void build_ui(HWND hwnd) {
    ...
    app().left_tabs = make_tab(hwnd, IDC_LEFT_TABS);
    app().center_tabs = make_tab(hwnd, IDC_CENTER_TABS);

    tab_add(app().left_tabs, 0, L"Services");
    tab_add(app().left_tabs, 1, L"EXEs");
    tab_add(app().center_tabs, 0, L"Services");
    tab_add(app().center_tabs, 1, L"EXEs");

    app().left_page_svc = make_page(hwnd);
    app().left_page_exe = make_page(hwnd);
    app().center_page_svc = make_page(hwnd);
    app().center_page_exe = make_page(hwnd);

    // Left: Services controls (same functionality, cleaner labels)
    CreateWindowW(L"STATIC", L"Add Windows Service", WS_CHILD | WS_VISIBLE,
        10, 10, 220, 18, app().left_page_svc, NULL, NULL, NULL);
    CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        10, 34, 220, 24, app().left_page_svc, (HMENU)IDC_SVC_ADD_EDIT, NULL, NULL);
    CreateWindowW(L"BUTTON", L"Add", WS_CHILD | WS_VISIBLE,
        10, 64, 220, 28, app().left_page_svc, (HMENU)IDC_SVC_ADD_BTN, NULL, NULL);
    CreateWindowW(L"BUTTON", L"Stop selected", WS_CHILD | WS_VISIBLE,
        10, 110, 220, 28, app().left_page_svc, (HMENU)IDC_SVC_STOP_BTN, NULL, NULL);
    CreateWindowW(L"BUTTON", L"Remove selected", WS_CHILD | WS_VISIBLE,
        10, 144, 220, 28, app().left_page_svc, (HMENU)IDC_SVC_REMOVE_BTN, NULL, NULL);

    // Left: EXE controls
    CreateWindowW(L"STATIC", L"Add Executable (.exe)", WS_CHILD | WS_VISIBLE,
        10, 10, 220, 18, app().left_page_exe, NULL, NULL, NULL);
    CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        10, 34, 220, 24, app().left_page_exe, (HMENU)IDC_EXE_ADD_EDIT, NULL, NULL);
    CreateWindowW(L"BUTTON", L"Add", WS_CHILD | WS_VISIBLE,
        10, 64, 220, 28, app().left_page_exe, (HMENU)IDC_EXE_ADD_BTN, NULL, NULL);
    CreateWindowW(L"BUTTON", L"Stop selected", WS_CHILD | WS_VISIBLE,
        10, 110, 220, 28, app().left_page_exe, (HMENU)IDC_EXE_STOP_BTN, NULL, NULL);
    CreateWindowW(L"BUTTON", L"Remove selected", WS_CHILD | WS_VISIBLE,
        10, 144, 220, 28, app().left_page_exe, (HMENU)IDC_EXE_REMOVE_BTN, NULL, NULL);

    // Center: listviews
    app().lv_svc = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"",
        WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS,
        0, 0, 100, 100, app().center_page_svc, (HMENU)IDC_LV_SVC, GetModuleHandleW(NULL), NULL);

    app().lv_exe = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"",
        WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS,
        0, 0, 100, 100, app().center_page_exe, (HMENU)IDC_LV_EXE, GetModuleHandleW(NULL), NULL);

    lv_set_columns(app().lv_svc, L"Auto Stop", L"Service Name", L"Status", L"Last Update");
    lv_set_columns(app().lv_exe, L"Auto Stop", L"Executable", L"Status", L"Last Update");

    // Explorer theming for modern look (safe if unavailable)
    try_set_explorer_theme(app().lv_svc);
    try_set_explorer_theme(app().lv_exe);
    try_set_explorer_theme(app().left_tabs);
    try_set_explorer_theme(app().center_tabs);

    // Right: activity
    // Draggable splitter between center and Activity
    app().splitter = CreateWindowExW(0, SPLITTER_CLASS, L"",
        WS_CHILD | WS_VISIBLE,
        0, 0, SPLITTER_WIDTH, 100, hwnd, (HMENU)IDC_SPLITTER, GetModuleHandleW(NULL), NULL);

    app().activity_label = CreateWindowW(L"STATIC", L"Activity", WS_CHILD | WS_VISIBLE,
        0, 0, 100, 18, hwnd, NULL, NULL, NULL);

    app().activity = CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"",
    ...
    app().statusbar = CreateWindowExW(0, STATUSCLASSNAMEW, L"",
        WS_CHILD | WS_VISIBLE | SBARS_SIZEGRIP,
        0, 0, 0, 0, hwnd, (HMENU)IDC_STATUSBAR, GetModuleHandleW(NULL), NULL);

    TabCtrl_SetCurSel(app().left_tabs, 0);
    TabCtrl_SetCurSel(app().center_tabs, 0);
    show_only(app().left_page_svc, app().left_page_exe, true);
    show_only(app().center_page_svc, app().center_page_exe, true);

    // Apply modern font once everything exists
    if (app().ui_font) apply_font_recursive(hwnd, app().ui_font);
}
...
static void update_statusbar(void) {
    ...
    wchar_t ts[64];
    fmt_time_local(ts, 64, app().last_any_status_wall);
...
static void layout(HWND hwnd) {
    ...
    if (app().statusbar) {
        ...
        RECT sr; GetWindowRect(app().statusbar, &sr);
        sbH = (sr.bottom - sr.top);
        SetWindowPos(app().statusbar, NULL, 0, H - sbH, W, sbH, SWP_NOZORDER);
        H -= sbH;
    }
...
int rightW = (app().activity_panel_w > 0) ? app().activity_panel_w : 360;
...
app().activity_panel_w = rightW;

    int pad = 8;
    int tabH = 28;

    SetWindowPos(app().left_tabs, NULL, pad, pad, leftW - 2*pad, tabH, SWP_NOZORDER);
    int leftX = pad;
    int leftY = pad + tabH;
    int leftH = H - leftY - pad;
    SetWindowPos(app().left_page_svc, NULL, leftX, leftY, leftW - 2*pad, leftH, SWP_NOZORDER);
    SetWindowPos(app().left_page_exe, NULL, leftX, leftY, leftW - 2*pad, leftH, SWP_NOZORDER);

    int cx = leftW + pad;
    SetWindowPos(app().center_tabs, NULL, cx, pad, centerW - 2*pad, tabH, SWP_NOZORDER);
    int cy = pad + tabH;
    int ch = H - cy - pad;
    SetWindowPos(app().center_page_svc, NULL, cx, cy, centerW - 2*pad, ch, SWP_NOZORDER);
    SetWindowPos(app().center_page_exe, NULL, cx, cy, centerW - 2*pad, ch, SWP_NOZORDER);

    SetWindowPos(app().lv_svc, NULL, 0, 0, centerW - 2*pad, ch, SWP_NOZORDER);
    SetWindowPos(app().lv_exe, NULL, 0, 0, centerW - 2*pad, ch, SWP_NOZORDER);

    int rx = leftW + centerW + pad;

    if (app().splitter) {
        int sx = leftW + centerW - (SPLITTER_WIDTH / 2);
        SetWindowPos(app().splitter, NULL, sx, pad, SPLITTER_WIDTH, H - 2*pad, SWP_NOZORDER);
    }
    int labelH = 18;
    HWND label = app().activity_label;
    if (label) SetWindowPos(label, NULL, rx, pad, rightW - 2*pad, labelH, SWP_NOZORDER);

    SetWindowPos(app().activity, NULL, rx, pad + labelH + 6, rightW - 2*pad, H - (pad + labelH + 6) - pad, SWP_NOZORDER);

    // Mark for autosize on next UI tick (less churn during resize drags)
    app().lv_needs_layout[KIND_SVC] = true;
    app().lv_needs_layout[KIND_EXE] = true;
}
...
static void add_item(int kind) {
    HWND page = (kind == KIND_SVC) ? app().left_page_svc : app().left_page_exe;
    ...
    trim_ws_inplace(raw);
    if (!raw[0]) {
        msgbox_warn(app().hwnd, L"Add", L"Name is required.");
        return;
    }
    ...
    if (kind == KIND_SVC) {
        ...
        if (!query_service_status_fast(name, st, 32)) {
            ...
            StringCchPrintfW(err, 900,
                ...
                raw, name);
            msgbox_err(app().hwnd, L"Add service", err);
            return;
        }
    } else {
        normalize_exe_input(raw, name, 512);
        if (!name[0]) {
            msgbox_err(app().hwnd, L"Add EXE", L"Executable name is invalid.");
            return;
        }
    }
    ...
    {
        std::lock_guard<std::mutex> lk(app().mtx);
        exist = list_find(&app().items[kind], name);
    }
    if (exist) {
        msgbox_info(app().hwnd, L"Add", L"Already monitored.");
        return;
    }
    ...
    HWND lv = (kind == KIND_SVC) ? app().lv_svc : app().lv_exe;
    lv_set_row(lv, it);
...
static void remove_selected(int kind) {
    ...
    {
        std::lock_guard<std::mutex> lk(app().mtx);
        removed = list_remove_name(&app().items[kind], name);
        if (removed) app().items_gen[kind]++;
    }
    if (!removed) return;
...
static void stop_selected(int kind) {
    ...
    if (!lv_get_selected_name(lv, name, 512)) {
        msgbox_warn(app().hwnd, L"Stop", L"Select a row first.");
        return;
    }
...
static void toggle_autostop_by_row(int kind, int row) {
    ...
    {
        std::lock_guard<std::mutex> lk(app().mtx);
        Item* it = list_find(&app().items[kind], name);
...
static void sync_tabs_to(int sel) {
    ...
    set_tab_sel_if(app().left_tabs, sel);
    set_tab_sel_if(app().center_tabs, sel);

    // Keep both sides in lock-step so the left actions always correspond
    // to the visible center list.
    show_only(app().left_page_svc, app().left_page_exe, (sel == 0));
    show_only(app().center_page_svc, app().center_page_exe, (sel == 0));
}
static void restart_ui_timer(HWND hwnd) {
    KillTimer(hwnd, TIMER_UI_REFRESH);
    int ms = std::max(200, app().ui_refresh_ms);
    SetTimer(hwnd, TIMER_UI_REFRESH, (UINT)ms, NULL);
}

static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    
if (app().taskbar_restart_msg && msg == app().taskbar_restart_msg) {
...
app().taskbar_restart_msg = RegisterWindowMessageW(L"TaskbarCreated");



        // Allow Explorer to deliver tray callback messages to an elevated window
        allow_uipi_message(hwnd, WM_APP_TRAYICON);
        if (app().taskbar_restart_msg) allow_uipi_message(hwnd, app().taskbar_restart_msg);

        if (!enum_services_build_disp_cache(&app().disp_cache)) {
...
tray_sync(GetModuleHandleW(NULL));
        ...
        app().action_thread = std::jthread(action_thread_main);
        app().monitor_thread = std::jthread(monitor_thread_main);
...
if (id == IDM_TRAY_CLOSE_TO_TRAY) {
    ...
    if (app().close_to_tray) app().tray_enabled = true;
    request_save_debounced();
    return 0;
}
...
switch (id) {
    ...
    case WM_APP_STATUS_BULK: {
        ...
        if (m) {
...

            {
                ...
                Item* it = list_find(&app().items[m->kind], name);
                if (it) {
                    StringCchCopyW(it->last_status, 32, e[i].status);
                    it->last_update_wall = m->wall_ts;
                    lv_set_row(m->kind == KIND_SVC ? app().lv_svc : app().lv_exe, it);
                }
                }
            }

            app().last_any_status_wall = m->wall_ts;
            app().lv_needs_layout[m->kind] = true;
    ...
    case WM_APP_STATUS: {
        ...
        if (m && m->name) {
            {
                std::lock_guard<std::mutex> lk(app().mtx);
                Item* it = list_find(&app().items[m->kind], m->name);
                if (it) {
    ...
    case WM_TIMER:
        ...
        if (wParam == TIMER_UI_REFRESH) {
            ...
            if (app().lv_needs_layout[KIND_SVC]) {
                lv_autosize_status_last(app().lv_svc);
                lv_apply_name_fill(app().lv_svc);
                app().lv_needs_layout[KIND_SVC] = false;
            }
            if (app().lv_needs_layout[KIND_EXE]) {
                lv_autosize_status_last(app().lv_exe);
                lv_apply_name_fill(app().lv_exe);
                app().lv_needs_layout[KIND_EXE] = false;
            }
            update_statusbar();
            return 0;
        }
...
case WM_CLOSE:
        if (!app().force_quit && app().close_to_tray && app().tray_enabled) {
            hide_main_window();
            return 0;
        }
        DestroyWindow(hwnd);
    ...
    case WM_DESTROY:
        ...
        if (app().monitor_thread.joinable()) app().monitor_thread.request_stop();
        if (app().action_thread.joinable()) app().action_thread.request_stop();
        app().action_cv.notify_all();

        if (app().monitor_thread.joinable()) app().monitor_thread.join();
        if (app().action_thread.joinable()) app().action_thread.join();
        ...
        dispcache_free(&app().disp_cache);
        list_free(&app().items[KIND_SVC]);
        list_free(&app().items[KIND_EXE]);

        if (app().ui_font && app().ui_font != (HFONT)GetStockObject(DEFAULT_GUI_FONT)) {
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int show) {
    ...
    list_init(&app().items[KIND_SVC]);
    list_init(&app().items[KIND_EXE]);


    app().items_gen[KIND_SVC] = 1;
    app().items_gen[KIND_EXE] = 1;
    app().ui_refresh_ms = 1000;
    app().monitor_interval_ms = 1000;
    app().autostop_cooldown_ms = 15000;
    app().stop_wait_ms = 10000;

app().tray_enabled = true;   // show tray icon by default
app().close_to_tray = false; // X closes normally unless enabled
app().tray_added = false;
app().force_quit = false;
app().taskbar_restart_msg = 0;

    app().activity_panel_w = 360;
    app().last_any_status_wall = 0;

    compute_default_cfg_path(app().cfg_path, MAX_PATH);

    app().ui_font = create_ui_font();
    app().app_title = exe_stem_title();
    ...
    HWND hwnd = CreateWindowW(
        wc.lpszClassName, app().app_title.c_str(),

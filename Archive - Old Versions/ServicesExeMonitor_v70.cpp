...
static LRESULT theme_customdraw_tab(App& self, NMCUSTOMDRAW* cd) {
    ...
    case CDDS_PREPAINT:
        // Tell the control we will custom-draw each tab item.
        return CDRF_NOTIFYITEMDRAW;
    case CDDS_ITEMPREPAINT: {
        HDC hdc = cd->hdc;
        if (hdc) {
            // Paint the item background ourselves; many builds ignore DarkMode_Explorer for tabs.
...
static LRESULT theme_customdraw_header(App& self, NMCUSTOMDRAW* cd) {
    ...
    switch (cd->dwDrawStage) {
    case CDDS_PREPAINT:
        // Per-item drawing for each header section.
        return CDRF_NOTIFYITEMDRAW;
    case CDDS_ITEMPREPAINT: {
        HDC hdc = cd->hdc;
        if (hdc) {
            // Paint header background; without this the header stays light.
            FillRect(hdc, &cd->rc, self.br_panel);
            SetBkMode(hdc, TRANSPARENT);
            SetTextColor(hdc, self.col_text);
        }
        // Let the control draw the text, but with our colors.
        return CDRF_NEWFONT;
    }
    default:
        return CDRF_DODEFAULT;
...
static void theme_apply_to_control(App& self, HWND child) {
    ...
    if (lstrcmpiW(cls, L"Button") == 0) {
        // Push buttons do NOT reliably render dark with SetWindowTheme alone.
        // We switch them to owner-draw in dark mode so they match the rest of the UI.
        LONG_PTR st = GetWindowLongPtrW(child, GWL_STYLE);
        const bool is_push = (st & (BS_PUSHBUTTON | BS_DEFPUSHBUTTON)) != 0;
        const bool is_checkish = (st & (BS_AUTOCHECKBOX | BS_CHECKBOX | BS_AUTORADIOBUTTON | BS_RADIOBUTTON | BS_GROUPBOX)) != 0;

        if (is_push && !is_checkish) {
            if (self.dark_mode) st |= BS_OWNERDRAW;
            else st &= ~((LONG_PTR)BS_OWNERDRAW);
            SetWindowLongPtrW(child, GWL_STYLE, st);
            SetWindowPos(child, NULL, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED);
        }

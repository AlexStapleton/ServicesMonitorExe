...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    auto do_layout = [&]() {
        ...
        if (st_profiles_hdr) MoveWindow(st_profiles_hdr, xL, y, leftW, labelH, TRUE);
        y += labelH + g2;
        ...
        MoveWindow(lb_profiles, xL, y, leftW, listH, TRUE);

        int yName = y + listH + g2;
        if (st_name_hdr) MoveWindow(st_name_hdr, xL, yName, leftW, labelH, TRUE);
        yName += labelH + g2;
        MoveWindow(e_name, xL, yName, leftW, editH, TRUE);

        int yBtn = yName + editH + g2;
        // row 1
        MoveWindow(b_add,    xL,                 yBtn, dpi_scale(s_prof_dpi, 60), btnH, TRUE);
        MoveWindow(b_rename, xL + dpi_scale(s_prof_dpi, 66), yBtn, dpi_scale(s_prof_dpi, 70), btnH, TRUE);
        MoveWindow(b_del,    xL + dpi_scale(s_prof_dpi, 142), yBtn, dpi_scale(s_prof_dpi, 70), btnH, TRUE);
        MoveWindow(b_up,     xL + leftW - btnW_sm, yBtn, btnW_sm, btnH, TRUE);

        // row 2 (down)
        int yBtn2 = yBtn + btnH + g2;
        MoveWindow(b_down, xL + leftW - btnW_sm, yBtn2, btnW_sm, btnH, TRUE);

        // Close button bottom-right
        if (b_close) {
            MoveWindow(b_close, cw - m - closeW, ch - m - btnH, closeW, btnH, TRUE);
        }
        ...
        int yR = m;
        if (st_watch_hdr) MoveWindow(st_watch_hdr, mainX, yR, mainW, labelH, TRUE);
        yR += labelH + g2;

        int watchBtnW = btnW;
        int watchListW = std::max(dpi_scale(s_prof_dpi, 220), mainW - g - watchBtnW);
        int watchListH = dpi_scale(s_prof_dpi, 230);
        MoveWindow(lb_watch, mainX, yR, watchListW, watchListH, TRUE);

        int xWB = mainX + watchListW + g;
        MoveWindow(b_watch_add, xWB, yR, watchBtnW, btnH, TRUE);
        MoveWindow(b_watch_del, xWB, yR + btnH + g2, watchBtnW, btnH, TRUE);

        int yAddWatch = yR + watchListH + g2;
        if (st_watch_add_hdr) MoveWindow(st_watch_add_hdr, mainX, yAddWatch, watchListW, labelH, TRUE);
        yAddWatch += labelH + g2;

        int editW = watchListW;
        int addBtnW = watchBtnW;
        MoveWindow(e_watch, mainX, yAddWatch, editW, editH, TRUE);

        // Place Add WATCH button aligned with the edit (keep "Remove selected" under it)
        MoveWindow(b_watch_add, xWB, yAddWatch, addBtnW, btnH, TRUE);
        MoveWindow(b_watch_del, xWB, yAddWatch + btnH + g2, addBtnW, btnH, TRUE);
        ...
        if (st_start_hdr) MoveWindow(st_start_hdr, xStart, startY, startW, startLabelH, TRUE);
        ...
        MoveWindow(lb_start_exe, xStart, yStartBody, startW, startListH, TRUE);

        int yStartAdd = yStartBody + startListH + g2;
        if (st_start_add_hdr) MoveWindow(st_start_add_hdr, xStart, yStartAdd, startW, labelH, TRUE);
        yStartAdd += labelH + g2;

        MoveWindow(e_start_exe, xStart, yStartAdd, startEditW, editH, TRUE);
        int xSB = xStart + startEditW + g;
        MoveWindow(b_start_exe_add, xSB, yStartAdd, startBtnW, btnH, TRUE);
        MoveWindow(b_start_exe_del, xSB, yStartAdd + btnH + g2, startBtnW, btnH, TRUE);

        // ---- Profile contents (services / exes) in main area ----
        int yC = yContents;
        if (st_contents_hdr) MoveWindow(st_contents_hdr, mainX, yC, mainW, labelH, TRUE);
        yC += labelH + g2;
        ...
        if (st_active) MoveWindow(st_active, mainX, activeY, mainW, labelH, TRUE);
        if (st_note) MoveWindow(st_note, mainX, noteY, mainW, noteH, TRUE);
        ...
        // Services column
        if (st_svc_hdr) MoveWindow(st_svc_hdr, xSvc, yC, (mainW - g) / 2, labelH, TRUE);
        MoveWindow(lb_svc, xSvc, listTop, (mainW - g) / 2, listH2, TRUE);

        int ySvcCtrl = listTop + listH2 + g2;
        MoveWindow(e_svc, xSvc, ySvcCtrl, (mainW - g) / 2, editH, TRUE);
        ySvcCtrl += editH + g2;
        MoveWindow(chk_svc, xSvc, ySvcCtrl, (mainW - g) / 2, chkH, TRUE);
        ySvcCtrl += chkH + g2;
        MoveWindow(b_svc_add, xSvc, ySvcCtrl, (mainW - g) / 2, btnH, TRUE);
        ySvcCtrl += btnH + g2;
        MoveWindow(b_svc_toggle, xSvc, ySvcCtrl, (mainW - g) / 2, btnH, TRUE);
        ySvcCtrl += btnH + g2;
        MoveWindow(b_svc_del, xSvc, ySvcCtrl, (mainW - g) / 2, btnH, TRUE);

        // EXEs column
        if (st_exe_hdr) MoveWindow(st_exe_hdr, xExe, yC, colW, labelH, TRUE);
        MoveWindow(lb_exe, xExe, listTop, colW, listH2, TRUE);

        int yExeCtrl = listTop + listH2 + g2;
        MoveWindow(e_exe, xExe, yExeCtrl, colW, editH, TRUE);
        yExeCtrl += editH + g2;
        MoveWindow(chk_exe, xExe, yExeCtrl, colW, chkH, TRUE);
        yExeCtrl += chkH + g2;
        MoveWindow(b_exe_add, xExe, yExeCtrl, colW, btnH, TRUE);
        yExeCtrl += btnH + g2;
        MoveWindow(b_exe_toggle, xExe, yExeCtrl, colW, btnH, TRUE);
        yExeCtrl += btnH + g2;
        MoveWindow(b_exe_del, xExe, yExeCtrl, colW, btnH, TRUE);
    };

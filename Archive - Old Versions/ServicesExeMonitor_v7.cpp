...
static void load_config(App& self) {
    ...
    if (restored > 0 || settings_loaded) request_save_debounced(self);
...
static void save_config_now(App& self) {
    ...
    // Snapshot settings + counts under lock.
    {
        ...
        autostop_cooldown_ms = self.autostop_cooldown_ms;
        stop_wait_ms = self.stop_wait_ms;
        tray_enabled = self.tray_enabled ? 1 : 0;
        close_to_tray = self.close_to_tray ? 1 : 0;
    ...
    // Copy monitored items under lock (best-effort if list changed while allocating).
    {
        ...
        size_t cur_ns = self.items[KIND_SVC].v.size();
        size_t cur_ne = self.items[KIND_EXE].v.size();

...
static RECT calc_window_rect_for_client(UINT dpi, DWORD style, DWORD exstyle, int client_w, int client_h) {
    RECT r{ 0, 0, client_w, client_h };
    HMODULE user32 = GetModuleHandleW(L"user32.dll");
    if (user32) {
        PFN_AdjustWindowRectExForDpi p = get_proc<PFN_AdjustWindowRectExForDpi>(user32, "AdjustWindowRectExForDpi");
        if (p) {
            p(&r, style, FALSE, exstyle, dpi);
            return r;
        }
    }
...
[[maybe_unused]] static std::wstring started_exe_lower_from_target(const wchar_t* target) {
...
[[maybe_unused]] static int cmp_nameidx(const void* a, const void* b) {
...
[[maybe_unused]] static bool batch_query_service_states(SC_HANDLE scm_in, std::vector<BYTE>& buf, const NameIdx* idx_sorted, size_t idx_n,
...
static LRESULT CALLBACK PrefsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Config file path (use a read-only edit so long paths aren't cut off and can be copied)
        CreateWindowW(L"STATIC", L"Config file:", WS_CHILD | WS_VISIBLE,
            S(12), S(230), S(80), S(20), dlg, NULL, NULL, NULL);

        RECT rc_client{};
        GetClientRect(dlg, &rc_client);
        int cfg_x = S(12) + S(80) + S(8);
        int cfg_w = (rc_client.right - S(12)) - cfg_x;
        if (cfg_w < S(120)) cfg_w = S(120);
        HWND e_cfg = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", self.cfg_path,
            WS_CHILD | WS_VISIBLE | ES_READONLY | ES_AUTOHSCROLL,
            cfg_x, S(228), cfg_w, S(24), dlg, (HMENU)IDC_PREF_CFG_PATH, NULL, NULL);
        // Show the filename/end of the path by default
        int cfg_len = GetWindowTextLengthW(e_cfg);
        SendMessageW(e_cfg, EM_SETSEL, cfg_len, cfg_len);
        SendMessageW(e_cfg, EM_SCROLLCARET, 0, 0);
...
case WM_COMMAND: {
    ...
    case WM_DPICHANGED: {
        ...
        if (new_dpi && new_dpi != s_prefs_dpi) {
            ...
            // Keep the config path scrolled to the end (filename visible)
            HWND e_cfg = GetDlgItem(dlg, IDC_PREF_CFG_PATH);
            // Also re-fit the config path box to the current client width (non-client sizes can differ across DPI)
            if (e_cfg) {
                RECT rc{};
                GetClientRect(dlg, &rc);
                auto S = [&](int v96) -> int { return dpi_scale(s_prefs_dpi, v96); };
                int left = S(12);
                int label_w = S(80);
                int gap = S(8);
                int x = left + label_w + gap;
                int w = (rc.right - left) - x;
                if (w < S(120)) w = S(120);
                SetWindowPos(e_cfg, NULL, x, S(228), w, S(24), SWP_NOZORDER | SWP_NOACTIVATE);
            }
            if (e_cfg) {
                int len = GetWindowTextLengthW(e_cfg);
                SendMessageW(e_cfg, EM_SETSEL, len, len);
                SendMessageW(e_cfg, EM_SCROLLCARET, 0, 0);
            }
        }
        return 0;
...
static void open_prefs_dialog(App& self, HWND parent) {
    ...
    // Make the *client area* DPI-scaled, then expand to include non-client borders.
    int client_w = dpi_scale(dpi, 510);
    int client_h = dpi_scale(dpi, 310);
    DWORD style = WS_CAPTION | WS_POPUP | WS_SYSMENU;
    DWORD exstyle = WS_EX_DLGMODALFRAME;
    RECT wr = calc_window_rect_for_client(dpi, style, exstyle, client_w, client_h);
    ...
    HWND dlg = CreateWindowExW(exstyle, wc.lpszClassName, L"Preferences",
        style,
...
static UINT s_prof_dpi = 96;
    ...
    if (show_activation) {
        ...
        const int ctrlH = editH + g2 + chkH + g2 + (btnH * 3) + (g2 * 2);
        int listH = std::max(dpi_scale(s_prof_dpi, 180), remainingH - ctrlH);

...
static void save_config_now(void) {
    ...
    // Copy monitored items under lock (best-effort if list changed while allocating).
    {
        ...
        size_t cur_ns = g_app.items[KIND_SVC].n;
        size_t cur_ne = g_app.items[KIND_EXE].n;
...
static void action_thread_main(std::stop_token st) {
    for (;;) {
        ...
        if (a.kind == KIND_SVC) {
            wchar_t result[256];
            stop_service_and_wait(a.name.c_str(), a.wait_s, result, 256);
...
static void stop_selected(int kind) {
    ...
    {
        std::lock_guard<std::mutex> lk(g_app.mtx);
        wait_s = g_app.stop_wait_s;
    }
    action_enqueue(kind, name, L"manual stop", wait_s);
}

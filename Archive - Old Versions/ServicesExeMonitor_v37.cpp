...
if (idx >= 0 && idx < (int)self.profiles.size()) {
    ...
    // Record these so we can stop them when leaving the profile.
...
static LRESULT CALLBACK PrefsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowW(L"STATIC", cfgline, WS_CHILD | WS_VISIBLE,
            S(12), S(228), S(470), S(20), dlg, NULL, NULL, NULL);
...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    // Start EXEs list (stopped on deactivation)
    static HWND lb_start_exe, e_start_exe, b_start_exe_add, b_start_exe_del;
    static HWND b_close;
    // Static labels we want to reposition on resize
    static HWND st_profiles_hdr, st_name_hdr;
    static HWND st_watch_hdr, st_watch_add_hdr;
    static HWND st_contents_hdr, st_svc_hdr, st_exe_hdr;
    static HWND st_start_hdr, st_start_add_hdr;
    static HWND st_note;

    static UINT s_prof_dpi = 96;
    static HFONT s_prof_font = NULL;
    auto do_layout = [&]() {
        if (!IsWindow(lb_profiles)) return;
        ...
        // --- Metrics (font-aware) ---
        int lineH = dpi_scale(s_prof_dpi, 16);
        {
            HDC hdc = GetDC(dlg);
            HFONT old = (HFONT)SelectObject(hdc, s_prof_font ? s_prof_font : (HFONT)GetStockObject(DEFAULT_GUI_FONT));
            TEXTMETRICW tm{};
            if (GetTextMetricsW(hdc, &tm)) lineH = (int)(tm.tmHeight + tm.tmExternalLeading);
            SelectObject(hdc, old);
            ReleaseDC(dlg, hdc);
        }
        ...
        const int labelH  = lineH + dpi_scale(s_prof_dpi, 6);
        const int headerH = lineH + dpi_scale(s_prof_dpi, 10);
        const int editH   = std::max(dpi_scale(s_prof_dpi, 26), lineH + dpi_scale(s_prof_dpi, 10));
        const int chkH    = std::max(dpi_scale(s_prof_dpi, 24), lineH + dpi_scale(s_prof_dpi, 10));
        const int btnH    = std::max(dpi_scale(s_prof_dpi, 28), lineH + dpi_scale(s_prof_dpi, 12));

        const int btnW_sm = dpi_scale(s_prof_dpi, 34);
        const int closeW  = dpi_scale(s_prof_dpi, 90);

        // Bottom strip (Close button)
        const int bottom_strip = btnH + g;
        const int content_top  = m;
        const int content_bottom = std::max(m, ch - m - bottom_strip);

        // --- Layout helpers (DeferWindowPos reduces flicker & partial updates) ---
        HDWP hdwp = BeginDeferWindowPos(48);
        auto DW = [&](HWND h, int x, int y, int w, int hgt) {
            if (!h || !hdwp) return;
            hdwp = DeferWindowPos(hdwp, h, NULL, x, y, w, hgt, SWP_NOZORDER | SWP_NOACTIVATE);
        };
        auto DWS = [&](HWND h, int x, int y, int w, int hgt) { // same as DW, but ok with null handle
            if (!h) return;
            DW(h, x, y, w, hgt);
        };

        // Responsive breakpoint: if too narrow for two columns, stack vertically.
        const int minRightW = dpi_scale(s_prof_dpi, 520);
        const int minLeftW  = dpi_scale(s_prof_dpi, 320);
        const int preferLeftW = dpi_scale(s_prof_dpi, 340);

        bool two_col = (cw >= (m + preferLeftW + g + minRightW + m));

        // Close button anchored bottom-right
        if (b_close) {
        ...
        // -------- Left column: Profiles list + name + buttons --------
        auto layout_left = [&](int x, int w, int yTop, int yBot) {
            int yy = yTop;

            DWS(st_profiles_hdr, x, yy, w, headerH); yy += headerH + g2;

            // Profiles list gets a solid minimum height so it never "collapses" and hides text.
            const int minList = std::max(dpi_scale(s_prof_dpi, 160), lineH * 8);
            int listH = std::max(minList, (yBot - yy) / 2);
            // Clamp so we leave room for name + buttons
            const int needBelow = headerH + editH + (btnH * 2) + (g2 * 6) + labelH;
            listH = std::min(listH, std::max(minList, yBot - yy - needBelow));
            listH = std::max(minList, listH);

            DW(lb_profiles, x, yy, w, listH); yy += listH + g;

            DWS(st_name_hdr, x, yy, w, headerH); yy += headerH + g2;
            DW(e_name, x, yy, w, editH); yy += editH + g2;

            // Buttons: Add / Rename / Delete + Up/Down on right edge
            ...
            DW(b_up, x + w - btnW_sm, yy, btnW_sm, btnH);
            yy += btnH + g2;
            DW(b_down, x + w - btnW_sm, yy, btnW_sm, btnH);
            yy += btnH + g2;

            // Active note/status (wrap-friendly)
            DWS(st_active, x, yy, w, labelH * 2);
            yy += (labelH * 2) + g2;
        ...
        auto layout_watch_and_start = [&](int x, int w, int yTop, int yBot) {
            int yy = yTop;

            const int btnW = dpi_scale(s_prof_dpi, 110);
            const int btnW2 = dpi_scale(s_prof_dpi, 110);

            // WATCH LIST
            DWS(st_watch_hdr, x, yy, w, headerH); yy += headerH + g2;

            const int minWatch = std::max(dpi_scale(s_prof_dpi, 150), lineH * 6);
            // Reserve space for add row (header + edit + buttons)
            const int watchAddNeed = headerH + editH + g2;
            int watchListH = std::max(minWatch, std::min((yBot - yy - watchAddNeed - g) / 2, yBot - yy - watchAddNeed - g));
            watchListH = std::max(minWatch, watchListH);

            DW(lb_watch, x, yy, w, watchListH);
            yy += watchListH + g;

            // Add/Remove watch row
            DWS(st_watch_add_hdr, x, yy, w, headerH); yy += headerH + g2;

            int editW = std::max(dpi_scale(s_prof_dpi, 260), w - (btnW + btnW2 + g2 * 2));
            int bx = x + editW + g2;
            DW(e_watch, x, yy, editW, editH);
            DW(b_watch_add, bx, yy, btnW, btnH);
            DW(b_watch_del, bx + btnW + g2, yy, btnW2, btnH);
            yy += editH + g;

            // START EXES
            DWS(st_start_hdr, x, yy, w, headerH); yy += headerH + g2;

            const int minStart = std::max(dpi_scale(s_prof_dpi, 140), lineH * 5);
            const int startAddNeed = headerH + editH + g2;
            int startListH = std::max(minStart, yBot - yy - startAddNeed - g);
            startListH = std::max(minStart, startListH);

            DW(lb_start_exe, x, yy, w, startListH);
            yy += startListH + g;

            DWS(st_start_add_hdr, x, yy, w, headerH); yy += headerH + g2;

            editW = std::max(dpi_scale(s_prof_dpi, 260), w - (btnW + btnW2 + g2 * 2));
            bx = x + editW + g2;
            DW(e_start_exe, x, yy, editW, editH);
            DW(b_start_exe_add, bx, yy, btnW, btnH);
            DW(b_start_exe_del, bx + btnW + g2, yy, btnW2, btnH);
            yy += editH + g;

            return yy;
        };

        auto layout_contents = [&](int x, int w, int yTop, int yBot) {
            int yy = yTop;

            DWS(st_contents_hdr, x, yy, w, headerH); yy += headerH + g2;

            // Note text: give it multiple lines so it doesn't clip
            const int noteH = std::max(dpi_scale(s_prof_dpi, 72), lineH * 4);
            DWS(st_note, x, yy, w, noteH); yy += noteH + g;

            // Two sub-columns: services / exes
            const int colGap = g;
            const int colW = (w - colGap) / 2;
            int xSvc = x;
            int xExe = x + colW + colGap;

            // Headers
            DWS(st_svc_hdr, xSvc, yy, colW, headerH);
            DWS(st_exe_hdr, xExe, yy, colW, headerH);
            yy += headerH + g2;

            // Lists (share remaining height; keep minimum)
            const int minList = std::max(dpi_scale(s_prof_dpi, 160), lineH * 6);
            const int ctrlBlock = (editH + chkH + btnH * 3 + g2 * 6);
            int listH = std::max(minList, yBot - yy - ctrlBlock);
            listH = std::max(minList, listH);

            DW(lb_svc, xSvc, yy, colW, listH);
            DW(lb_exe, xExe, yy, colW, listH);
            yy += listH + g2;

            // Controls blocks (keep both columns aligned)
            const int yCtrlStart = yy;

            // Service controls
            int ySvcCtrl = yCtrlStart;
            DW(e_svc, xSvc, ySvcCtrl, colW, editH); ySvcCtrl += editH + g2;
            DW(chk_svc, xSvc, ySvcCtrl, colW, chkH); ySvcCtrl += chkH + g2;
            DW(b_svc_add, xSvc, ySvcCtrl, colW, btnH); ySvcCtrl += btnH + g2;
            DW(b_svc_toggle, xSvc, ySvcCtrl, colW, btnH); ySvcCtrl += btnH + g2;
            DW(b_svc_del, xSvc, ySvcCtrl, colW, btnH);

            // EXE controls (aligned to the same starting y)
            int yExeCtrl = yCtrlStart;
            DW(e_exe, xExe, yExeCtrl, colW, editH); yExeCtrl += editH + g2;
            DW(chk_exe, xExe, yExeCtrl, colW, chkH); yExeCtrl += chkH + g2;
            DW(b_exe_add, xExe, yExeCtrl, colW, btnH); yExeCtrl += btnH + g2;
            DW(b_exe_toggle, xExe, yExeCtrl, colW, btnH); yExeCtrl += btnH + g2;
            DW(b_exe_del, xExe, yExeCtrl, colW, btnH);

            // Advance yy to bottom of controls area for any callers that care
            yy = std::max(ySvcCtrl + btnH, yExeCtrl + btnH);

            return std::max(yy, yExeCtrl + btnH);
        };

        if (two_col) {
            int xL = m;
            int leftW = std::min(std::max(minLeftW, preferLeftW), std::max(minLeftW, (cw - 2 * m - g - minRightW)));
            int xR = xL + leftW + g;
            int rightW = cw - m - xR;

            // Split right side: top = watch + start, bottom = contents
            int yTop = y;
            int yBot = content_bottom;

            int rightTopH = (int)((yBot - yTop) * 0.45);
            const int minTop = std::max(dpi_scale(s_prof_dpi, 260), lineH * 12);
            const int minBot = std::max(dpi_scale(s_prof_dpi, 320), lineH * 14);
            rightTopH = std::max(minTop, std::min(rightTopH, (yBot - yTop) - minBot));
            int yMid = yTop + rightTopH;

            layout_left(xL, leftW, yTop, yBot);
            layout_watch_and_start(xR, rightW, yTop, yMid - g);
            layout_contents(xR, rightW, yMid + g, yBot);
        } else {
            // Single column: stack sections vertically using full width
            int x = m;
            int w = std::max(1, cw - 2 * m);
            int yTop = y;
            int yBot = content_bottom;

            int yy = yTop;
            yy = layout_left(x, w, yy, yBot);
            yy += g;

            // Watch + Start block
            int blockH = std::max(dpi_scale(s_prof_dpi, 420), (yBot - yy) / 2);
            blockH = std::min(blockH, yBot - yy - dpi_scale(s_prof_dpi, 300));
            int yMid = yy + blockH;

            yy = layout_watch_and_start(x, w, yy, yMid);
            yy += g;
            layout_contents(x, w, yy, yBot);
        }
    ...
    case WM_CREATE: {
        ...
        st_watch_hdr = CreateWindowW(L"STATIC", L"Activation (WATCH EXEs) — profile activates if ANY are running", WS_CHILD | WS_VISIBLE,
        ...
        st_contents_hdr = CreateWindowW(L"STATIC", L"Profile contents (editable even when profile is NOT active)", WS_CHILD | WS_VISIBLE,
        ...
        st_start_add_hdr = CreateWindowW(L"STATIC", L"Add start exe (name or full path):", WS_CHILD | WS_VISIBLE,
    ...
    case WM_GETMINMAXINFO: {
        ...
        if (p) {
            const int minW = dpi_scale(s_prof_dpi, 760);

...
// If multiple profiles match, the first match (config order) wins.
{
    ...
    for (size_t i = 0; i < watches.size(); i++) {
        ...
        for (const auto& exe_raw : ws) {
            if (exe_raw.empty()) continue;

            // Normalize watch entry to basename + lower-case (handles full paths too)
            std::wstring key = exe_watch_key_lower(exe_raw);

...
static UINT get_dpi_for_hwnd(HWND hwnd) {
    HMODULE user32 = GetModuleHandleW(L"user32.dll");
    if (user32) {
        PFN_GetDpiForWindow pW = (PFN_GetDpiForWindow)GetProcAddress(user32, "GetDpiForWindow");
        if (pW && hwnd) return pW(hwnd);
        PFN_GetDpiForSystem pS = (PFN_GetDpiForSystem)GetProcAddress(user32, "GetDpiForSystem");
        if (pS) return pS();
    }
    return 96;
}

static int sm_for_dpi(int idx, UINT dpi) {
    HMODULE user32 = GetModuleHandleW(L"user32.dll");
    if (user32) {
        PFN_GetSystemMetricsForDpi p = (PFN_GetSystemMetricsForDpi)GetProcAddress(user32, "GetSystemMetricsForDpi");
...
static HFONT create_ui_font_for_dpi(UINT dpi) {
    ...
    HMODULE user32 = GetModuleHandleW(L"user32.dll");
    if (user32) {
        PFN_SystemParametersInfoForDpi p = (PFN_SystemParametersInfoForDpi)GetProcAddress(user32, "SystemParametersInfoForDpi");
        if (p) {
...
self.activity_panel_w = rightW;
    ...
    // Services page
    {
        ...
        if (bRem)  SetWindowPos(bRem,  NULL, inPad, y2 + btnH + S(10), fullW, btnH, SWP_NOZORDER);
    }

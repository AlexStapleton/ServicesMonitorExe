...
#define IDC_DIAG_EDIT       5901
#define IDC_DIAG_COPY_BTN   5902
#define IDC_DIAG_CLOSE_BTN  5903
...
static void update_left_info_pane(App& self, int kind, Item* it);
...
static void toggle_autostop_selected(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; Item* sel = lv_get_selected_item_ptr(self, kind); if (!sel) { msgbox_warn(self.hwnd, L"Auto-stop", L"Select a row first."); return; } bool now = false;;
static void snapshot_active_from_runtime_locked(App& self); [[maybe_unused]] static void snapshot_active_from_runtime(App& self); static void apply_profile_index(App& self, int idx); static void action_enqueue(App& self, int kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms); static void update_statusbar(App& self);
static void apply_filter_now(App& self, int kind);
static void capture_columns_now(App& self, int kind);
static void apply_saved_columns(App& self, int kind);
static LRESULT CALLBACK SearchEditSubclassProc(HWND, UINT, WPARAM, LPARAM, UINT_PTR, DWORD_PTR);
...
#endif // disabled broken duplicate block near forward decls
...
// Process helpers
// ----------------------------
#if 0 // disabled duplicate normalize_exe_input (keep earlier)
static void normalize_exe_input(const wchar_t* raw, wchar_t* out, size_t cch_out) {
    out[0] = 0;
    if (!raw) return;
...
#endif // disabled duplicate normalize_exe_input (keep earlier)

#if 0 // disabled duplicate resolve_exe_launch_path (keep earlier)
static void resolve_exe_launch_path(const wchar_t* raw, wchar_t* out, size_t cch_out) {
    out[0] = 0;
    if (!raw) return;
...
#endif // disabled duplicate resolve_exe_launch_path (keep earlier)
...
// ----------------------------
#if 0 // disabled duplicate parse_setting (keep earlier)
static bool parse_setting(App& self, const wchar_t* key, const wchar_t* val) {
    if (!key || !val) return false;
...
#endif // disabled duplicate parse_setting (keep earlier)
...
Service / EXE table
  ...
  Ctrl+C    Copy selected name
        ...
        theme_compute_for_window(self, dlg);(self, dlg);
        HFONT f = theme_ensure_font_for_dpi(self, dpi);
        if (f) apply_font_recursive(dlg, f);
        theme_apply_to_window(self, dlg);
        return 0;
    ...
    App* selfp = (App*)GetWindowLongPtrW(dlg, GWLP_USERDATA);
    if (!selfp) return DefWindowProcW(dlg, msg, wParam, lParam);
    App& self = *selfp;
...
static std::wstring build_diagnostics_text(App& self) {
    ...
    auto appendf = [&](const wchar_t* fmt, ...) {
        ...
        va_list ap;
        va_start(ap, fmt);
        _vsnwprintf(buf, (int)(sizeof(buf)/sizeof(buf[0])) - 1, fmt, ap);
        buf[(sizeof(buf)/sizeof(buf[0])) - 1] = 0;
        va_end(ap);
        out.append(buf);
        out.push_back(L'\n');
    ...
    const std::wstring title = exe_stem_title();
    appendf(L"ServicesExeMonitor Diagnostics");
    appendf(L"Build: %s (%s %s)", title.c_str(), widen_ascii(__DATE__).c_str(), widen_ascii(__TIME__).c_str());

    appendf(L"");
    appendf(L"Settings");
    appendf(L"  UI refresh: %d ms", self.ui_refresh_ms);
    appendf(L"  Monitoring tick: %d ms", self.monitor_interval_ms);
    appendf(L"  Auto-stop cooldown: %d ms", self.autostop_cooldown_ms);
    appendf(L"  Stop wait: %d ms", self.stop_wait_ms);

    size_t qdepth = 0;
    {
        std::lock_guard<std::mutex> lk(self.action_mtx);
        qdepth = self.action_q.size();
    }

    appendf(L"");
    appendf(L"Runtime");
    appendf(L"  Actions queued: %zu", qdepth);
    appendf(L"  Services tracked: %zu", self.items[KIND_SVC].v.size());
    appendf(L"  EXEs tracked: %zu", self.items[KIND_EXE].v.size());

    // Per-item last errors (primarily services)
    const uint64_t now = now_mono_ms();
    size_t err_count = 0;
    appendf(L"");
    appendf(L"Last errors per item (non-zero)");
    // Services
    ...
    if (err_count == 0) {
        ...
        for (auto& up : self.items[KIND_SVC].v) {
            ...
            const bool supp = (it->svc_stop_suppress_until_ms && it->svc_stop_suppress_until_ms > now);
            appendf(L"  %s  last_stop_err=%lu  streak=%u  suppressed=%s",
                    it->name.c_str(),
                    (unsigned long)it->svc_last_stop_err,
                    (unsigned)it->svc_stop_fail_streak,
                    supp ? L"yes" : L"no");
            shown++;
            if (shown >= 40) {
                appendf(L"  ... (%zu more)", (size_t)(err_count - shown));
                break;
            }
        }
    }
    ...
    appendf(L"");
    appendf(L"Notes");
    appendf(L"  - 'Actions queued' is the current pending action queue depth.");
    appendf(L"  - 'last_stop_err' is the last Win32 error returned by a service stop attempt.");
    return out;
}
...
static LRESULT CALLBACK DiagnosticsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    static HWND s_copy = NULL;
    static HWND s_close = NULL;
    ...
    case WM_CREATE: {
        const UINT dpi = get_dpi_for_hwnd(dlg);
        HFONT f = theme_ensure_font_for_dpi(self, dpi);

        s_edit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_MULTILINE | ES_READONLY | WS_VSCROLL | ES_AUTOVSCROLL,
            0, 0, 10, 10, dlg, (HMENU)IDC_DIAG_EDIT, NULL, NULL);

        s_copy = CreateWindowExW(0, L"BUTTON", L"Copy diagnostics",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP,
            0, 0, 10, 10, dlg, (HMENU)IDC_DIAG_COPY_BTN, NULL, NULL);

        s_close = CreateWindowExW(0, L"BUTTON", L"Close",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP,
            0, 0, 10, 10, dlg, (HMENU)IDC_DIAG_CLOSE_BTN, NULL, NULL);

        SendMessageW(s_edit, WM_SETFONT, (WPARAM)f, TRUE);
        SendMessageW(s_copy, WM_SETFONT, (WPARAM)f, TRUE);
        SendMessageW(s_close, WM_SETFONT, (WPARAM)f, TRUE);

        theme_apply_to_control(self, s_edit);
        theme_apply_to_control(self, s_copy);
        theme_apply_to_control(self, s_close);

        const std::wstring diag = build_diagnostics_text(self);
        SetWindowTextW(s_edit, diag.c_str());
        return 0;
    }
    case WM_SIZE: {
        RECT rc; GetClientRect(dlg, &rc);
        const UINT dpi = get_dpi_for_hwnd(dlg);
        const int pad = dpi_scale(dpi, 10);
        const int btnH = dpi_scale(dpi, 28);
        const int gap = dpi_scale(dpi, 8);
        const int btnW = dpi_scale(dpi, 150);
        ...
        int editH = h - btnH - gap;
        SetWindowPos(s_edit, NULL, x, y, w, editH, SWP_NOZORDER);

        int by = y + editH + gap;
        SetWindowPos(s_copy, NULL, x + (w - btnW*2 - gap), by, btnW, btnH, SWP_NOZORDER);
        SetWindowPos(s_close, NULL, x + (w - btnW), by, btnW, btnH, SWP_NOZORDER);
        return 0;
    }
    case WM_CTLCOLORSTATIC:
    case WM_CTLCOLOREDIT:
    ...
    case WM_COMMAND: {
        int id = LOWORD(wParam);
        if (id == IDC_DIAG_CLOSE_BTN || id == IDCANCEL) { DestroyWindow(dlg); return 0; }
        if (id == IDC_DIAG_COPY_BTN) {
            const std::wstring diag = build_diagnostics_text(self);
            clipboard_set_text(dlg, diag.c_str());
            return 0;
        }
        return 0;
    }
...
static void open_diagnostics_dialog(App& self, HWND parent) {
    ...
    wc.hInstance = GetModuleHandleW(NULL);
    wc.lpszClassName = L"ServicesExeMonitor_DiagnosticsWnd";
    wc.hCursor = LoadCursorW(NULL, IDC_ARROW);
    wc.hbrBackground = NULL;
    RegisterClassW(&wc);

    const UINT dpi = get_dpi_for_hwnd(parent);
    const int w = dpi_scale(dpi, 760);
    const int h = dpi_scale(dpi, 520);
    ...
    MSG m;
    while (IsWindow(dlg) && GetMessageW(&m, NULL, 0, 0)) {
        // Allow global accelerators even while dialog is open
        if (self.accel && self.hwnd && TranslateAcceleratorW(self.hwnd, self.accel, &m)) continue;
...
static void update_statusbar(App& self) {
    ...
    wchar_t text[256];
    size_t actions_q = 0; { std::lock_guard<std::mutex> lk(self.action_mtx); actions_q = self.action_q.size(); }
...
if ((nh->idFrom == IDC_LV_SVC || nh->idFrom == IDC_LV_EXE) && nh->code == LVN_KEYDOWN) {
    ...
    if (kd) {
        ...
        // Ctrl+F focuses the center search box for this kind (Services/EXEs)
        if ((GetKeyState(VK_CONTROL) & 0x8000) && kd->wVKey == 'F') {
            HWND edit = (kind == KIND_SVC) ? self.svc_search : self.exe_search;
            if (edit) {
                SetFocus(edit);
                SendMessageW(edit, EM_SETSEL, 0, -1);
                return 0;
        ...
        // Esc clears the search for this kind even when focus is in the list
        if (kd->wVKey == VK_ESCAPE) {
            HWND edit = (kind == KIND_SVC) ? self.svc_search : self.exe_search;
            if (edit) {
                SetWindowTextW(edit, L"");
                apply_filter_now(self, kind);
            }
            return 0;
        }
        ...
        // Space toggles the checkbox natively; LVN_ITEMCHANGED will handle applying the change.
}
}
...
self.taskbar_restart_msg = 0;
    ...
    // Ctrl+1 / Ctrl+2 -> switch tabs (Services / EXEs)
    ...
    app_instance.accel = CreateAcceleratorTableW(accels, 6);

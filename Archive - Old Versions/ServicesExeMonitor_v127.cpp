...
static LRESULT CALLBACK HotkeysWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (msg == WM_CREATE) {
        ...
        if (!cs || !cs->lpCreateParams) return -1;
        SetWindowLongPtrW(dlg, GWLP_USERDATA, (LONG_PTR)cs->lpCreateParams);

        App& self = *(App*)cs->lpCreateParams;
        s_dpi = get_dpi_for_hwnd(dlg);
        self.dpi = s_dpi;

        auto S = [&](int v96) -> int { return dpi_scale(s_dpi, v96); };

        CreateWindowW(L"STATIC", L"Keyboard shortcuts (hotkeys)", WS_CHILD | WS_VISIBLE,
            S(12), S(10), S(520), S(20), dlg, NULL, NULL, NULL);
        ...
        const wchar_t* help =
            L"Global\r\n"
            L"  Ctrl+F    Focus the search box for the active tab (Services/EXEs)\r\n"
            L"\r\n"
            L"Search box\r\n"
            L"  Esc       Clear search immediately\r\n"
            L"\r\n"
            L"Service / EXE table\r\n"
            L"  Enter     Show details\r\n"
            L"  Del       Remove selected row\r\n"
            L"  Space     Toggle Auto-stop checkbox on selected row\r\n"
            L"  Ctrl+C    Copy selected item name\r\n"
            L"\r\n"
            L"Tips\r\n"
            L"  Right-click a row for actions (Stop / Remove / Toggle auto-stop / Copy name).\r\n"
            L"  Right-click the column header to Reset columns.\r\n";

        SetWindowTextW(s_edit, help);

        theme_compute_for_window(self, dlg);
        HFONT f = theme_ensure_font_for_dpi(self, s_dpi);
        if (f) apply_font_recursive(dlg, f);
        theme_apply_to_window(self, dlg);
        return 0;
    }
...
self.taskbar_restart_msg = 0;
    ...
    app_instance.accel = CreateAcceleratorTableW(accels, 1);

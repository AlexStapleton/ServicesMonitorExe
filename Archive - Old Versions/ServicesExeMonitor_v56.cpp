...
static bool shell_open_and_track_process(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) {
    ...
    if (!ShellExecuteExW(&sei)) {
        ...
        if (out_result && cch) StringCchPrintfW(out_result, cch, L"ShellExecuteEx failed (%lu).", (unsigned long)e);
        return false;
    }

    out_sp.started_target = target;

    out_sp.exe_lower = started_exe_lower_from_target(target);
    ...
    if (sei.hProcess) {
        ...
        // Common when opening documents/URLs that reuse an existing process.

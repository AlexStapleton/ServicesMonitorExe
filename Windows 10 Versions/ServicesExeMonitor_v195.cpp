...
static void unregister_item_locked(App& self, ItemKind kind, Item* it) {
    ...
    // UI row cache map
    // If this item is currently visible in the owner-data view, null out row pointers
    // so LVN_GETDISPINFO never touches a freed Item* (keeps GETDISPINFO lock-free).
    {
        auto frow = self.lv_row_of_item[k].find(it);
        if (frow != self.lv_row_of_item[k].end()) {
            const int row = frow->second;
            if (row >= 0 && row < (int)self.view_items[k].size()) self.view_items[k][row] = nullptr;
            if (row >= 0 && row < (int)self.view_uids[k].size())  self.view_uids[k][row]  = 0;
        }
    }
...
static void toggle_autostop_selected(App& self, int kind) {
    ...
    (void)cmd_set_autostop_uid(self, sel->uid, !cur);
}
...
// Row checkbox toggle
static bool cmd_set_autostop_uid(App& self, uint32_t uid, bool enabled, std::wstring* out_name_opt) {
...

    {
        ...
        if (changed) {
            const ItemKind kind = (it->kind == KIND_SVC) ? ItemKind::Svc : ItemKind::Exe;
            kind_i = kind_index(kind);

            // Refresh UI caches while holding the lock so LVN_GETDISPINFO can be lock-free.
            ui_seed_ownerdata_cache_locked(self, kind, it);

            auto frow = self.lv_row_of_item[kind_i].find(it);
            if (frow != self.lv_row_of_item[kind_i].end()) row = frow->second;
        }
    }
    ...
    if (changed && !name_for_log.empty()) {
        ...
        // Immediate repaint for responsiveness (owner-data ListView uses ui_cache_*).
        HWND lv = NULL;
        if (kind_i == kind_index(ItemKind::Svc)) lv = self.lv_svc;
        else if (kind_i == kind_index(ItemKind::Exe)) lv = self.lv_exe;

        if (lv) {
            if (row >= 0) {
                // Owner-data + LVS_EX_CHECKBOXES: ListView may cache the state image; update it explicitly.
                const bool prev_suppress = self.suppress_lv_notify;
                self.suppress_lv_notify = true;
                ListView_SetItemState(lv, row, INDEXTOSTATEIMAGEMASK(enabled ? 2 : 1), LVIS_STATEIMAGEMASK);
                self.suppress_lv_notify = prev_suppress;
...
static bool handle_main_listview_notify(App& self, HWND hwnd, NMHDR* nh, LPARAM lParam, LRESULT& out) {
    ...
    if (nh->code == LVN_GETDISPINFO) {
            NMLVDISPINFO* di = (NMLVDISPINFO*)nh;
            ...
            // Lock-free: view_items/view_uids are rebuilt together on the UI thread.
            if (row >= 0 && row < (int)self.view_items[kind_i].size()) {
                itp = self.view_items[kind_i][row];
                if (itp) uid = itp->uid;
            }
            if (!uid && row >= 0 && row < (int)self.view_uids[kind_i].size()) {
                uid = self.view_uids[kind_i][row];
            }

            // If we don't have a live Item* (rare; e.g. during a rebuild), return blanks.
            if (!uid || !itp) {
                if (di->item.mask & LVIF_PARAM) di->item.lParam = (LPARAM)uid;

                if (di->item.mask & LVIF_STATE) {
                    di->item.stateMask = LVIS_STATEIMAGEMASK;
                    di->item.state = INDEXTOSTATEIMAGEMASK(1);
                }
            ...
            if ((di->item.mask & LVIF_TEXT) && di->item.pszText) {
                ...
                switch (di->item.iSubItem) {
                    ...
                    // ui_cache_last_text is a fixed wchar_t buffer, not a std::wstring
    ...
    if (nh->code == LVN_ITEMCHANGED) {
        ...
        if (lv && (lv->uChanged & LVIF_STATE)) {
            ...
            if ((lv->uNewState ^ lv->uOldState) & LVIS_STATEIMAGEMASK) {
                // Prefer lParam identity when available (we set it in LVN_GETDISPINFO via LVIF_PARAM).
                uint32_t uid = (uint32_t)lv->lParam;
                if (!uid) {
                    ModelLockGuard lk(self);
                    if (lv->iItem >= 0 && lv->iItem < (int)self.view_uids[kind_i].size()) uid = self.view_uids[kind_i][lv->iItem];
                }

                // For owner-data ListViews, ListView_GetCheckState is unreliable.
                // Decode the state image from uNewState: 1=unchecked, 2=checked.
                const int img = (int)((lv->uNewState & LVIS_STATEIMAGEMASK) >> 12);
                if (uid && img != 0) {
                    const bool enabled = (img == 2);

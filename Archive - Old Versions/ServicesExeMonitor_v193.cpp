...
static void item_set_status(Item* it, const wchar_t* s);
static void item_enqueue_dirty(App& self, int kind, Item* it);
...
#define IDC_PREF_AUTO_DISABLE_FLAP 8009
#define IDC_PREF_FLAP_COUNT_EDIT 8010
#define IDC_PREF_FLAP_WINDOW_MIN_EDIT 8011
...
static void lv_set_columns5(HWND lv, const wchar_t* c0, const wchar_t* c1, const wchar_t* c2, const wchar_t* c3, const wchar_t* c4);
...
static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], const DWORD* pids_opt, size_t n, time_t wall_ts);
...
struct Item {
    ...
    std::wstring name_lower;              // cached lowercase name for fast filter matching
    ...
    wchar_t  last_status_lower[32]{};     // cached lowercase last_status for fast filter matching
    time_t   last_update_wall = 0;



    // Service-only: last observed service PID (dwProcessId when running). Helps detect fast restarts where status may stay "running".
    DWORD    svc_last_pid = 0;

    // UI-side cache to avoid repainting unchanged rows
    wchar_t  ui_last_status[32]{};
    time_t   ui_last_update_wall = 0;
    time_t   ui_last_stopped_wall = 0;     // last time we observed a transition into a stopped/off state
    ...
    wchar_t  ui_cache_last_text[64]{};     // legacy: last update/changed timestamp (kept for internal use)
    wchar_t  ui_cache_autod_text[16]{};     // service auto-disable checkbox glyph (or empty)
...
uint32_t ui_queued_gen = 0;        // last status_gen enqueued into dirty_status (model lock)
    ...
    bool     flap_auto_disable_attempted = false;
    bool     flap_auto_disable_pending = false;
    bool     flap_auto_disabled = false;
    DWORD    flap_auto_disable_err = 0;
...
struct ConfigSnapshot {
    ...
    bool auto_disable_flapping = false;
    int flap_disable_count = 3; // STOPPED->RUNNING restarts threshold
    int flap_disable_window_minutes = 2; // window for restarts
...
typedef struct App {
    HWND hwnd;
    DWORD ui_thread_id = 0; // main UI thread id (for thread-safe logging)
...
bool cols_have[2]{};
int  cols_w[2][5]{};      // pixel widths (svc uses 5, exe uses 4)
int  cols_order[2][5]{};  // display order (svc uses 5, exe uses 4)
...
HWND exe_empty;
    ...
    // Activity (log) listbox: throttle expensive horizontal extent measurement
    size_t activity_max_line_chars = 0;
    int    activity_horiz_extent_px = 0;
    uint32_t activity_append_counter = 0;
    ...
    bool tray_enabled;
    bool close_to_tray;
    bool auto_disable_flapping;
    int flap_disable_count;
    int flap_disable_window_minutes;
    bool tray_added;
    bool force_quit;
    NOTIFYICONDATAW nid;
...
//   - NEVER call action_enqueue/actio
static void item_set_status(Item* it, const wchar_t* s) {
    if (!it) return;
    if (!s) s = L"";
    StringCchCopyW(it->last_status, 32, s);
    StringCchCopyW(it->last_status_lower, 32, it->last_status);
    if (it->last_status_lower[0]) {
        CharLowerBuffW(it->last_status_lower, (DWORD)wcslen(it->last_status_lower));
    }
}

static void item_enqueue_dirty(App& self, int kind, Item* it) {
    if (!it) return;
    // Avoid duplicate pointers in dirty_status when many updates happen before the UI applies them.
    if (it->ui_queued_gen == it->status_gen) return;
    it->ui_queued_gen = it->status_gen;
    self.dirty_status[kind].push_back(it);
}

// n_qdepth/action_clear/action_pop_wait (commented: stray edit artifact)
...
static bool register_item_locked(App& self, ItemKind kind, Item* it) {
    ...
    // Cache lowercase name/status for fast filter matching (keeps UI thread allocation-free).
    if (!it->name.empty() && it->name_lower.empty()) {
        it->name_lower = it->name;
        CharLowerBuffW(it->name_lower.data(), (DWORD)it->name_lower.size());
    }
    if (it->last_status[0]) {
        item_set_status(it, it->last_status); // sync lowercase cache
    } else {
        it->last_status_lower[0] = 0;
    }
...
static bool parse_setting(App& self, const wchar_t* key, const wchar_t* val) {
    ...
    // Auto-disable flapping services (global setting)
    if (_wcsicmp(key, L"auto_disable_flapping") == 0) {
        int v = 0; parse_int(&v);
        self.auto_disable_flapping = (v != 0);
        return true;
    }
    if (_wcsicmp(key, L"flap_disable_count") == 0) {
        int v = 0; parse_int(&v);
        if (v < 1) v = 1;
        if (v > 100) v = 100;
        self.flap_disable_count = v;
        return true;
    }
    if (_wcsicmp(key, L"flap_disable_window_minutes") == 0) {
        int v = 0; parse_int(&v);
        if (v < 1) v = 1;
        if (v > 120) v = 120;
        self.flap_disable_window_minutes = v;
        return true;
    }
...
if (_wcsicmp(key, L"win_rect") == 0) {
    ...
    int w[5] = {};
    // Back-compat: older configs have 4 columns; services now have 5 (AutoStop, Name, Status, AutoDisable, Last)
    ...
    if (kind == KIND_EXE) {
        ...
        if (n == 4) {
            // Insert a reasonable default width for the new Auto Disable column (index 3), and shift old "Last" (old index 3) to index 4.
            self.cols_w[kind][0] = w[0];
            self.cols_w[kind][1] = w[1];
            self.cols_w[kind][2] = w[2];
            self.cols_w[kind][3] = 90; // default, will be DPI-scaled when applied
            self.cols_w[kind][4] = w[3];
            self.cols_have[kind] = true;
            return true;
        }
        if (n == 5) {
            for (int i = 0; i < 5; ++i) self.cols_w[kind][i] = w[i];
            self.cols_have[kind] = true;
            return true;
        }
        return false;
...
if (_wcsicmp(key, L"svc_colorder") == 0 || _wcsicmp(key, L"exe_colorder") == 0) {
    ...
    if (kind == KIND_EXE) {
        if (n >= 4) {
            ...
            // Keep Auto-stop (column index 0) pinned to the left-most display
            {
                int pos0 = -1;
        ...
        if (n == 4) {
            // Back-compat: old order is [0,1,2,3] for (AutoStop, Name, Status, Last)
            // New model columns: 0 AutoStop, 1 Name, 2 Status, 3 AutoDisable, 4 Last
            self.cols_order[kind][0] = o[0];
            self.cols_order[kind][1] = o[1];
            self.cols_order[kind][2] = o[2];
            self.cols_order[kind][3] = 3;      // new column stays near middle by default
            self.cols_order[kind][4] = o[3] + 1; // old last (3) becomes 4
            // Keep Auto-stop (column index 0) pinned to the left-most display
            {
                int pos0 = -1;
                for (int i = 0; i < 5; i++) if (self.cols_order[kind][i] == 0) { pos0 = i; break; }
                if (pos0 > 0) {
                    int tmp = self.cols_order[kind][0];
                    self.cols_order[kind][0] = self.cols_order[kind][pos0];
                    self.cols_order[kind][pos0] = tmp;
        ...
        if (n == 5) {
            for (int i = 0; i < 5; ++i) self.cols_order[kind][i] = o[i];
            // Keep Auto-stop (column index 0) pinned to the left-most display
            {
                int pos0 = -1;
                for (int i = 0; i < 5; i++) if (self.cols_order[kind][i] == 0) { pos0 = i; break; }
...
static void theme_apply_to_window(App& self, HWND hwnd) {
    ...
    // Force a non-client repaint so the title bar/theme updates immediately (no minimize/restore needed).
...
#ifdef IDI_APPICON
        if (self.nid.hIcon) DestroyIcon(self.nid.hIcon);
#endif
...
static void tray_balloon(App& self, const wchar_t* title, const wchar_t* text, DWORD flags) {
    if (!self.tray_added) return;
    NOTIFYICONDATAW nid = self.nid;
    nid.uFlags = NIF_INFO;
    StringCchCopyW(nid.szInfoTitle, ARRAYSIZE(nid.szInfoTitle), title ? title : L"");
    StringCchCopyW(nid.szInfo,      ARRAYSIZE(nid.szInfo),      text  ? text  : L"");
    nid.dwInfoFlags = flags ? flags : NIIF_INFO;
...
if (kind == KIND_SVC) {
            wchar_t st[32]{};
            if (query_service_status_fast(norm, st, 32)) {
                item_set_status(it, st);
            } else {
                item_set_status(it, L"not found");
            }
        } else {
            int c = process_count_by_name_lower(norm);
            item_set_status(it, (c > 0) ? L"running" : L"stopped");
        }
...
int n = snprintf(header, sizeof(header),
        ...
        "SETTING|dark_mode|%d\n"
        "SETTING|auto_disable_flapping|%d\n"
        "SETTING|flap_disable_count|%d\n"
        "SETTING|flap_disable_window_minutes|%d\n"
...
"SETTING|exe_colorder|%d,%d,%d,%d\n"
        ...
        self.dark_mode ? 1 : 0,
        self.auto_disable_flapping ? 1 : 0,
                self.flap_disable_count,
        self.flap_disable_window_minutes,
wx, wy, ww, wh, wmx,
        apw,
        lsy_svc,
        lsy_exe,

        self.cols_w[KIND_SVC][0], self.cols_w[KIND_SVC][1], self.cols_w[KIND_SVC][2], self.cols_w[KIND_SVC][3], self.cols_w[KIND_SVC][4],
        self.cols_w[KIND_EXE][0], self.cols_w[KIND_EXE][1], self.cols_w[KIND_EXE][2], self.cols_w[KIND_EXE][3],
        self.cols_order[KIND_SVC][0], self.cols_order[KIND_SVC][1], self.cols_order[KIND_SVC][2], self.cols_order[KIND_SVC][3], self.cols_order[KIND_SVC][4],
...
static bool batch_query_service_states_direct(SC_HANDLE scm_in, MonScratch& sc,
                                            ...
                                            wchar_t out_status[][32], DWORD* out_pid_opt);
...
struct MonScratch {
    ...
    DWORD* svc_pid = nullptr; // service dwProcessId (0 if none/unknown)
...
static bool batch_query_service_states_direct(SC_HANDLE scm_in, MonScratch& sc,
    ...
    if (out_pid_opt) { for (size_t i = 0; i < ns; i++) out_pid_opt[i] = 0; }
    ...
    for (size_t i = 0; i < ns; i++) {
        ...
        if (ok) {
            ...
            if (out_pid_opt) out_pid_opt[i] = ssp.dwProcessId;
        }
    }
...
static bool mons_ensure_scratch(MonScratch* sc, size_t ns, size_t ne) {
    if (ns > sc->cap_s) {
        ...
        while (nc < ns) nc *= 2;
        wchar_t (*st)[32] = (wchar_t (*)[32])realloc(sc->svc_status, nc * sizeof(wchar_t[32]));
        DWORD* pid = (DWORD*)realloc(sc->svc_pid, nc * sizeof(DWORD));
        if (!st || !pid) {
            if (st) sc->svc_status = st;
            if (pid) sc->svc_pid = pid;
            return false;
        }
        sc->svc_status = st;
        sc->svc_pid = pid;
...
// (profile auto-activation moved below to reuse the single per-tick process snapshot)
        ...
        // Deferred work buffers (to keep model lock hold times low)
        std::vector<std::wstring> tmp_svc_names;
        std::vector<std::wstring> tmp_exe_names;
        bool rebuild_names_svc = false;
        bool rebuild_names_exe = false;
        ...
        // Snapshot pointers + cheap fields under lock.
        {
            ...
            if (ok_alloc) {
                                ...
                                // IMPORTANT: Defer wcsdup/sort/heap work until after we release the model lock.
                                rebuild_names_svc = (gen_s != snap.gen_s || ns != snap.ns);
                                rebuild_names_exe = (gen_e != snap.gen_e || ne != snap.ne);

                                if (rebuild_names_svc) {
                                    tmp_svc_names.clear();
                                    tmp_svc_names.reserve(ns);
                                    for (size_t i = 0; i < ns; i++) {
                                        Item* it = self.items[KIND_SVC].v[i].get();
                                        tmp_svc_names.push_back(it ? it->name : std::wstring());
                                    }
                                } else {
                                    tmp_svc_names.clear();
                                }

                                if (rebuild_names_exe) {
                                    tmp_exe_names.clear();
                                    tmp_exe_names.reserve(ne);
                                    for (size_t i = 0; i < ne; i++) {
                                        Item* it = self.items[KIND_EXE].v[i].get();
                                        tmp_exe_names.push_back(it ? it->name : std::wstring());
                                    }
                                } else {
                                    tmp_exe_names.clear();
                                }
for (size_t i = 0; i < ns; i++) {
        ...
        // Rebuild name copies + sorted indices OUTSIDE the model lock (prevents UI stalls).
        if (ok_alloc) {
            if (rebuild_names_svc) {
                // Free old names in range [0, snap.ns)
                free_name_array(snap.svc_names, snap.ns);
                for (size_t i = 0; i < ns; i++) {
                    const std::wstring& s = tmp_svc_names[i];
                    snap.svc_names[i] = (!s.empty()) ? wcsdup_heap(s.c_str()) : NULL;
                }
                ...
                for (size_t i = 0; i < ns; i++) {
                    if (snap.svc_names[i] && snap.svc_names[i][0]) {
                        snap.svc_idx_sorted.push_back(NameIdx{ snap.svc_names[i], i });
                    }
                }
            ...
            if (rebuild_names_exe) {
                ...
                for (size_t i = 0; i < ne; i++) {
                    const std::wstring& s = tmp_exe_names[i];
                    snap.exe_names[i] = (!s.empty()) ? wcsdup_heap(s.c_str()) : NULL;
                }
                ...
                for (size_t i = 0; i < ne; i++) {
                    if (snap.exe_names[i] && snap.exe_names[i][0]) {
                        snap.exe_idx_sorted.push_back(NameIdx{ snap.exe_names[i], i });
                    }
                }
            ...
            // Sizes always track current monitored lists.
            snap.ns = ns;
            snap.ne = ne;
        ...
        if (ns && snap.svc_names && sc.svc_status) {
            ...
            if (sc.svc_pid) { for (size_t i = 0; i < ns; i++) sc.svc_pid[i] = 0; }
            bool ok = batch_query_service_states_direct(scm_connect.get(), sc, snap.svc_names, ns, gen_s, sc.svc_status, sc.svc_pid);
            ...
            post_status_bulk(self, KIND_SVC, snap.svc_names, sc.svc_status, sc.svc_pid, ns, wall_now);
        }
...
// derive running/stopped ---
        if (ne && snap.exe_names && sc.exe_counts && sc.exe_status) {
            ...
            post_status_bulk(self, KIND_EXE, snap.exe_names, sc.exe_status, nullptr, ne, wall_now);
        }
...
static void post_log(App& self, const wchar_t* text) {
    // Avoid posting after shutdown / hwnd destruction (prevents rare crashes).
    if (!self.hwnd || self.shutting_down.load(std::memory_order_acquire)) return;
...
static void post_notify(App& self, const wchar_t* title, const wchar_t* text, DWORD flags) {
    // Avoid posting after shutdown / hwnd destruction (prevents rare crashes).
    if (!self.hwnd || self.shutting_down.load(std::memory_order_acquire)) return;

    NotifyMsg* m = (NotifyMsg*)malloc(sizeof(NotifyMsg));
    if (!m) return;
    m->title = wcsdup_heap(title ? title : L"");
    m->text  = wcsdup_heap(text  ? text  : L"");
    m->flags = flags;
    ...
    if (!PostMessageW(self.hwnd, WM_APP_NOTIFY, 0, (LPARAM)m)) {
        free(m->title);
        free(m->text);
        free(m);
    }
...
static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], const DWORD* pids_opt, size_t n, time_t wall_ts) {
    ...
    std::vector<std::wstring> to_disable_flap;
    {
        ModelLockGuard lk(self);
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            const DWORD pid_now = (kind == KIND_SVC && pids_opt) ? pids_opt[i] : 0;
            const bool status_changed = (wcscmp(it->last_status, ns) != 0);

            bool pid_restart = false;
            if (kind == KIND_SVC) {
                const bool now_running = (_wcsicmp(ns, L"running") == 0);
                if (now_running && pid_now != 0) {
                    if (it->svc_last_pid != 0 && it->svc_last_pid != pid_now) pid_restart = true;
                    it->svc_last_pid = pid_now;
                } else {
                    it->svc_last_pid = 0;
                }
            }

            if (status_changed || pid_restart) {
                // Service flapping detection (restarts within window)
                if (kind == KIND_SVC) {
                    const bool prev_stopped = (_wcsicmp(it->last_status, L"stopped") == 0);
                    const bool now_running  = (_wcsicmp(ns, L"running") == 0);
                    // Reset window if old
                    if (it->flap_window_start_wall == 0 || (wall_ts - it->flap_window_start_wall) > (time_t)((self.flap_disable_window_minutes > 0 ? self.flap_disable_window_minutes : 2) * 60)) {
                        it->flap_window_start_wall = wall_ts;
                        it->flap_changes = 0;
                        it->flap_restarts = 0;
                        it->flap_flagged = false;
                        it->flap_auto_disable_attempted = false;
                        it->flap_auto_disable_pending = false;
                        it->flap_auto_disabled = false;
                        it->flap_auto_disable_err = 0;
                    }

                    it->flap_changes++;
                    it->flap_last_change_wall = wall_ts;
                    if ((prev_stopped && now_running) || pid_restart) it->flap_restarts++;

                    if (!it->flap_flagged && it->flap_restarts >= (uint32_t)(self.flap_disable_count > 0 ? self.flap_disable_count : 3)) {
                        it->flap_flagged = true;
                        log_linef(self, L"%s: detected frequent restarts (%u in ~2 min)", it->name.c_str(), (unsigned)it->flap_restarts);

                        if (self.auto_disable_flapping && !it->flap_auto_disable_attempted) {
                            it->flap_auto_disable_attempted = true;
                            it->flap_auto_disable_pending = true;
                            it->flap_auto_disabled = false;
                            it->flap_auto_disable_err = 0;
                            to_disable_flap.push_back(it->name);
                            item_enqueue_dirty(self, kind, it);
                            any_changed = true;
                        }
                    }
                }

                if (status_changed) item_set_status(it, ns);
                it->last_update_wall = wall_ts; // "Last" column = last change time (status change OR detected restart)
                it->status_gen++;
                item_enqueue_dirty(self, kind, it);
                any_changed = true;
            }
    // Auto-disable flapping services (outside model lock)
    if (!to_disable_flap.empty()) {
        for (auto& svcname : to_disable_flap) {
            DWORD err = 0;
            const bool ok = set_service_start_type(svcname.c_str(), SERVICE_DISABLED, &err);
            if (ok) {
                // Stop immediately to break restart loop
                Action a;
                a.kind = KIND_SVC;
                a.op = ACTION_STOP;
                a.name = svcname;
                a.reason = L"auto-disable flapping";
                a.wait_ms = self.stop_wait_ms;
                action_enqueue(self, std::move(a));
                log_linef(self, L"%s: auto-disabled due to flapping", svcname.c_str());
                wchar_t note[256];
                StringCchPrintfW(note, ARRAYSIZE(note), L"%s was auto-disabled due to frequent restarts.", svcname.c_str());
                post_notify(self, L"Service auto-disabled", note, NIIF_WARNING);
            } else {
                log_linef(self, L"%s: auto-disable failed (err=%lu) - run as admin?", svcname.c_str(), (unsigned long)err);
                wchar_t note[256];
                StringCchPrintfW(note, ARRAYSIZE(note), L"Failed to auto-disable %s (error %lu).", svcname.c_str(), (unsigned long)err);
                post_notify(self, L"Auto-disable failed", note, NIIF_WARNING);
            }

            // Record result in model so UI can show it
            {
                ModelLockGuard lk(self);
                Item* it = list_find(&self.items[KIND_SVC], svcname.c_str());
                if (it) {
                    it->flap_auto_disable_pending = false;
                    it->flap_auto_disabled = ok;
                    it->flap_auto_disable_err = ok ? 0 : err;

                    // UI-affecting change even if status text didn't change
                    it->status_gen++;
                    item_enqueue_dirty(self, KIND_SVC, it);
                    any_changed = true;

                    // Wake UI to apply this row update even when no status-string change occurred.
                    self.status_gen[KIND_SVC]++;
                    PostMessageW(self.hwnd, WM_APP_STATUS_GEN, (WPARAM)KIND_SVC, (LPARAM)(INT_PTR)self.status_gen[KIND_SVC]);
                }
            }
        }
    }
...
static void lv_set_columns5(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3, const wchar_t* col4) {
    ListView_DeleteAllItems(lv);
    while (ListView_DeleteColumn(lv, 0)) {}
...
static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) {
    ...
    // Figure out the ListView column layout at runtime.
    int col_count = 0;
    HWND hdr = ListView_GetHeader(lv);
    if (hdr) col_count = Header_GetItemCount(hdr);
    const bool has_auto_disable_col = (col_count >= 5);
    const int  col_autod = has_auto_disable_col ? 3 : -1;
    const int  col_last  = has_auto_disable_col ? 4 : 3;
    ...
    if (idx < 0) {
        ...
        // Auto Disable checkbox glyph: only show when service is flagged/pending/disabled/attempted.
        if (it->kind == KIND_SVC) {
            const bool need_disable = it->flap_flagged || it->flap_auto_disable_attempted || it->flap_auto_disable_pending || it->flap_auto_disabled;
            if (need_disable) {
                // Checked box (text glyph). Keep empty otherwise per UX request.
                StringCchCopyW(it->ui_cache_autod_text, 16, L"☑");
            } else {
                it->ui_cache_autod_text[0] = 0;
            }
        } else {
            it->ui_cache_autod_text[0] = 0;
        }
...
ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str());
        ...
        if (it->kind == KIND_SVC && has_auto_disable_col) {
            wchar_t ad[64] = {0};
            if (it->flap_auto_disable_pending) {
                StringCchCopyW(ad, 64, L"Pending");
            } else if (it->flap_auto_disabled) {
                StringCchCopyW(ad, 64, L"Disabled");
            } else if (it->flap_auto_disable_err != 0) {
                StringCchPrintfW(ad, 64, L"Failed (e%lu)", (unsigned long)it->flap_auto_disable_err);
            } else if (it->flap_auto_disable_attempted) {
                StringCchCopyW(ad, 64, L"Flagged");
            } else {
                ad[0] = 0;
            }
            if (col_autod >= 0) ListView_SetItemText(lv, row, col_autod, ad);
            ListView_SetItemText(lv, row, col_last, it->ui_cache_last_text);
        } else {
            ListView_SetItemText(lv, row, col_last, it->ui_cache_last_text);
        }
        ...
        if (it->kind == KIND_SVC && has_auto_disable_col) {
            ...
            if (col_autod >= 0) ListView_SetItemText(lv, idx, col_autod, ad);
            ListView_SetItemText(lv, idx, col_last, it->ui_cache_last_text);
        } else {
            ListView_SetItemText(lv, idx, col_last, it->ui_cache_last_text);
        }
...
// This preserves scroll position when the user is reviewing older log lines.
bool stick_to_bottom = true;
if (count > 0) {
    int top = (int)SendMessageW(lb, LB_GETTOPINDEX, 0, 0);
    int itemH = (int)SendMessageW(lb, LB_GETITEMHEIGHT, 0, 0);
    if (itemH <= 0) itemH = 1;
    RECT rc{};
    GetClientRect(lb, &rc);
    const int visible = (rc.bottom - rc.top) / itemH;
    // If the last visible line is within ~1 line of the end, consider it "at bottom".
    stick_to_bottom = (top + visible >= count - 1);
}
    ...
    // Measuring text width (GetTextExtentPoint32) is relatively expensive; avoid doing it for every line.
    // Heuristic:
    //  - Measure when we see a new "longest line" (by char count), or
    //  - Periodically (every 32 appends) to recover from rare underestimates (e.g., wide glyphs).
    const size_t len = (line ? wcslen(line) : 0);
    self.activity_append_counter++;

    const bool periodic = ((self.activity_append_counter & 31u) == 0);
    const bool new_longest = (len > self.activity_max_line_chars);

    if (periodic || new_longest) {
        self.activity_max_line_chars = std::max(self.activity_max_line_chars, len);
        ...
        if (hdc) {
            ...
            SIZE sz{};
            if (len) GetTextExtentPoint32W(hdc, line, (int)len, &sz);

            SelectObject(hdc, old);
            ReleaseDC(lb, hdc);

            int extra = dpi_scale(self.dpi ? self.dpi : 96, 24);
            int want = sz.cx + extra;
            if (want > self.activity_horiz_extent_px) {
                self.activity_horiz_extent_px = want;
                SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)want, 0);
            }
        }
    }
...
static void ui_apply_status_updates(App& self, ItemKind kind) {
    ...
    // Small bursts: redraw only the exact changed rows (merged into contiguous ranges).
    // Large bursts: redraw a single range to minimize overhead.
    if (!big_burst && rows.size() <= 96) {
        std::sort(rows.begin(), rows.end());
        rows.erase(std::unique(rows.begin(), rows.end()), rows.end());

        int a = rows[0], b = rows[0];
        for (size_t i = 1; i < rows.size(); ++i) {
            int r = rows[i];
            if (r == b + 1) { b = r; continue; }
            ListView_RedrawItems(lv, a, b);
            a = b = r;
        }
        ListView_RedrawItems(lv, a, b);
        // Let the normal paint cycle coalesce work (avoid synchronous UpdateWindow).
...
static void log_linef(App& self, const wchar_t* fmt, ...) {
    ...
    // IMPORTANT:
    // - activity_append() uses SendMessage() to a UI control and MUST run on the UI thread.
    // - Worker threads should never SendMessage() into the UI thread while holding locks (deadlock/crash risk).
    // So: if we're not on the UI thread, post the raw message and let WM_APP_LOG -> log_linef()
    // add timestamps + append on the UI thread.
    if (self.ui_thread_id && GetCurrentThreadId() != self.ui_thread_id) {
        post_log(self, msg);
        return;
    }
...
static bool item_matches_filter(const Item* it, const std::wstring& flt_lower) {
    ...
    // Fast, allocation-free matching (name/status are already stored in lowercase caches).
    if (!it->name_lower.empty()) {
        if (it->name_lower.find(flt_lower) != std::wstring::npos) return true;
    } else if (!it->name.empty()) {
        // Fallback (should be rare): on-the-fly lowercase without heap churn.
        std::wstring tmp = it->name;
        if (!tmp.empty()) CharLowerBuffW(tmp.data(), (DWORD)tmp.size());
        if (tmp.find(flt_lower) != std::wstring::npos) return true;
    }

    if (it->last_status_lower[0]) {
        if (wcsstr(it->last_status_lower, flt_lower.c_str()) != nullptr) return true;
    } else if (it->last_status[0]) {
        wchar_t tmp[32];
        StringCchCopyW(tmp, 32, it->last_status);
        CharLowerBuffW(tmp, (DWORD)wcslen(tmp));
        if (wcsstr(tmp, flt_lower.c_str()) != nullptr) return true;
    }
...
static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out) {
    ...
    out.auto_disable_flapping = self.auto_disable_flapping;
    out.flap_disable_count = self.flap_disable_count;
    out.flap_disable_window_minutes = self.flap_disable_window_minutes;
...
if (kind == KIND_SVC) {
                wchar_t st[32]{};
                if (query_service_status_fast(it->name.c_str(), st, 32)) {
                    item_set_status(it, st);
                } else {
                    item_set_status(it, L"not found");
                }
...
// ----------------------------
static bool prefs_apply(App& self, HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close, HWND c_dark, HWND c_flap, HWND e_flap_count, HWND e_flap_winmin);
...
static bool prefs_apply(App& self, HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close, HWND c_dark, HWND c_flap, HWND e_flap_count, HWND e_flap_winmin) {
    ...
    wchar_t b5[64], b6[64];
    GetWindowTextW(e_flap_count, b5, 64);
    GetWindowTextW(e_flap_winmin, b6, 64);
    ...
    int flap_count = _wtoi(b5);
    int flap_min = _wtoi(b6);
    ...
    bool dark_changed = false;
    {
        ...
        // Tray settings
        const BOOL tray_checked = (SendMessageW(c_tray, BM_GETCHECK, 0, 0) == BST_CHECKED);
        const BOOL close_checked = (SendMessageW(c_close, BM_GETCHECK, 0, 0) == BST_CHECKED);
        ...
        // Theme
        const BOOL dark_checked = (SendMessageW(c_dark, BM_GETCHECK, 0, 0) == BST_CHECKED);
        const bool new_dark = dark_checked ? true : false;
        if (self.dark_mode != new_dark) {
            self.dark_mode = new_dark;
            dark_changed = true;
        }

        // Auto-disable flapping services
        const BOOL flap_checked = (SendMessageW(c_flap, BM_GETCHECK, 0, 0) == BST_CHECKED);
        self.auto_disable_flapping = flap_checked ? true : false;

        // Flap thresholds (even if checkbox is off, keep values persisted)
        if (flap_count < 1) flap_count = 1;
        if (flap_count > 100) flap_count = 100;
        if (flap_min < 1) flap_min = 1;
        if (flap_min > 120) flap_min = 120;

        self.flap_disable_count = flap_count;
        self.flap_disable_window_minutes = flap_min;
    }
...
static LRESULT CALLBACK PrefsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    static HWND c_tray, c_close, c_dark, c_flap;
    static HWND e_flap_count, e_flap_winmin;
    ...
    case WM_CREATE: {
        ...
        c_flap = CreateWindowW(L"BUTTON", L"Auto-disable services when flapping", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
            S(12), S(224), S(320), S(22), dlg, (HMENU)IDC_PREF_AUTO_DISABLE_FLAP, NULL, NULL);

        // Flap thresholds
        CreateWindowW(L"STATIC", L"Disable after restarts:", WS_CHILD | WS_VISIBLE,
            S(28), S(248), S(180), S(20), dlg, NULL, NULL, NULL);
        e_flap_count = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
            S(220), S(246), S(60), S(24), dlg, (HMENU)IDC_PREF_FLAP_COUNT_EDIT, NULL, NULL);

        CreateWindowW(L"STATIC", L"Within minutes:", WS_CHILD | WS_VISIBLE,
            S(28), S(276), S(180), S(20), dlg, NULL, NULL, NULL);
        e_flap_winmin = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
            S(220), S(274), S(60), S(24), dlg, (HMENU)IDC_PREF_FLAP_WINDOW_MIN_EDIT, NULL, NULL);
        wchar_t buf[64];
        ...
        SendMessageW(c_flap, BM_SETCHECK, self.auto_disable_flapping ? BST_CHECKED : BST_UNCHECKED, 0);
        StringCchPrintfW(buf, 64, L"%d", self.flap_disable_count);
        SetWindowTextW(e_flap_count, buf);
        StringCchPrintfW(buf, 64, L"%d", self.flap_disable_window_minutes);
        SetWindowTextW(e_flap_winmin, buf);
        EnableWindow(e_flap_count, self.auto_disable_flapping ? TRUE : FALSE);
        EnableWindow(e_flap_winmin, self.auto_disable_flapping ? TRUE : FALSE);
        ...
        if (e_flap_winmin) {
            RECT r{}; GetWindowRect(e_flap_winmin, &r);
        ...
        int yBtn = cfgY + cfgH + S(14);

CreateWindowW(L"BUTTON", L"Cancel", WS_CHILD | WS_VISIBLE,
            S(230), yBtn, S(80), S(28), dlg, (HMENU)IDCANCEL, NULL, NULL);
        CreateWindowW(L"BUTTON", L"Apply", WS_CHILD | WS_VISIBLE,
            S(318), yBtn, S(80), S(28), dlg, (HMENU)IDC_PREF_APPLY, NULL, NULL);
        CreateWindowW(L"BUTTON", L"OK", WS_CHILD | WS_VISIBLE,
            S(406), yBtn, S(80), S(28), dlg, (HMENU)IDOK, NULL, NULL);
...
case WM_COMMAND: {
        ...
        if (id == IDC_PREF_AUTO_DISABLE_FLAP) {
            BOOL on = (SendMessageW(c_flap, BM_GETCHECK, 0, 0) == BST_CHECKED);
            EnableWindow(e_flap_count, on);
            EnableWindow(e_flap_winmin, on);
            return 0;
        }
...
if (id == IDC_TOP_QUIT && code == BN_CLICKED) {
    ...
    return 0;
        ...
        if (id == IDC_PREF_APPLY) { prefs_apply(self, dlg, e_ui, e_mon, e_cd, e_sw, c_tray, c_close, c_dark, c_flap, e_flap_count, e_flap_winmin); return 0; }
        if (id == IDOK) {
            if (prefs_apply(self, dlg, e_ui, e_mon, e_cd, e_sw, c_tray, c_close, c_dark, c_flap, e_flap_count, e_flap_winmin)) DestroyWindow(dlg);
            return 0;
        }
        return 0;
    }
    ...
    case WM_SIZE: {
        ...
        int yBtn = S(262);
        {
            // Place buttons below the config-path line (and below the flap settings if present)
            int cfgY = S(228);
            int cfgH = S(20);
            if (e_flap_winmin) {
                RECT r{}; GetWindowRect(e_flap_winmin, &r);
...
self.left_tabs = make_tab(hwnd, IDC_LEFT_TABS);
    ...
    lv_set_columns5(self.lv_svc, L"Auto Stop", L"Service Name", L"Status", L"Auto Disable", L"Last Updated");
    lv_set_columns(self.lv_exe, L"Auto Stop", L"Executable", L"Status", L"Last Updated");
...
static void capture_columns_now(App& self, int kind) {
    ...
    int ncol = (kind == KIND_SVC) ? 5 : 4; // svc: AutoStop, Name, Status, AutoDisable, Last; exe: AutoStop, Name, Status, Last
    if (ncol <= 0) return;
...

{
    ...
    if (!it) { msgbox_err(self.hwnd, L"Add", L"Failed to add item (out of memory)."); return; }
...
bool removed = false;
    ...
    if (!removed) return;
    // Owner-data list: rebuild view mapping + item count (do not call ListView_DeleteItem).
    rebuild_listview_filtered(self, kind);
...
// to seed Status/Last Updated for items that never change.
if (itp->ui_cache_status_disp[0] == 0 || itp->ui_cache_last_text[0] == 0) {
    ...
    if (kind == ItemKind::Svc && itp->ui_cache_autod_text[0] == 0) {
        const bool need_disable = itp->flap_flagged || itp->flap_auto_disable_attempted || itp->flap_auto_disable_pending || itp->flap_auto_disabled;
        if (need_disable) StringCchCopyW(itp->ui_cache_autod_text, 16, L"☑");
        else itp->ui_cache_autod_text[0] = 0;
    }
}
...
switch (di->item.iSubItem) {
                ...
                case 3:
                    if (kind == ItemKind::Svc) di->item.pszText = (LPWSTR)itp->ui_cache_autod_text; // Auto Disable checkbox glyph (or blank)
                    else di->item.pszText = (LPWSTR)itp->ui_cache_last_text; // EXE: Last Updated
                    break;
                case 4:
                    if (kind == ItemKind::Svc) di->item.pszText = (LPWSTR)itp->ui_cache_last_text; // Service: Last Updated
                    else di->item.pszText = (LPWSTR)L"";
                    break;
                default:
                    di->item.pszText = (LPWSTR)L"";
                    break;
            }
}
...
// Checkbox toggle (auto-stop)
            if ((lv->uNewState ^ lv->uOldState) & LVIS_STATEIMAGEMASK) {
                ...
                // Owner-data listview: ensure the row is repainted immediately.
                if (lv->iItem >= 0) {
                    ListView_RedrawItems(nh->hwndFrom, lv->iItem, lv->iItem);
                    UpdateWindow(nh->hwndFrom);
                }
...
switch (id) {
    ...
    case WM_APP_LOG: {
        ...
        if (m) {
            // WM_APP_LOG is handled on the UI thread; append directly to avoid cross-thread SendMessage.
            activity_append(self, self.activity, m->text ? m->text : L"");
            free(m->text);
            free(m);
        }
        return 0;
    ...
    case WM_APP_NOTIFY: {
        NotifyMsg* m = (NotifyMsg*)lParam;
        if (m) {
            // Tray balloon if available; otherwise, just beep.
            if (self.tray_added) {
                tray_balloon(self, m->title, m->text, m->flags);
            } else {
    ...
    case WM_APP_STATUS: {
        ...
        if (m && m->name) {
            ...
            {
                ModelLockGuard lk(self);
                Item* it = list_find(&self.items[kind], m->name);
                if (it) {
                    self.last_any_status_wall = m->wall_ts;
                    if (wcscmp(it->last_status, m->status) != 0) {
                        item_set_status(it, m->status);
it->last_update_wall = m->wall_ts;
                        it->status_gen++;
                        self.status_gen[kind]++;
                        item_enqueue_dirty(self, kind, it);
                        changed = true;
                        new_gen = self.status_gen[kind];
                    }
                }
            }

            // Avoid using Item* outside the lock. Instead, trigger the standard event-driven UI path.
            if (changed) {
                PostMessageW(self.hwnd, WM_APP_STATUS_GEN, (WPARAM)kind, (LPARAM)(INT_PTR)new_gen);
                post_model_dirty(self);
            }

...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    auto do_layout = [&]() {
        ...
        if (s_prefH <= 0) s_prefH = dpi_scale(s_prof_dpi, 720);
        const int layoutH = (viewH < s_prefH) ? s_prefH : viewH;
        s_contentH = layoutH;

        const int maxScroll = (layoutH > viewH) ? (layoutH - viewH) : 0;
        if (s_scrollY < 0) s_scrollY = 0;
        if (s_scrollY > maxScroll) s_scrollY = maxScroll;
        const int yOff = -s_scrollY;

        auto MW = [&](HWND h, int x, int y, int w, int hgt, BOOL repaint = TRUE) {
            ::MoveWindow(h, x, y + yOff, w, hgt, TRUE);
        };
        ...
        const int btnH   = dpi_scale(s_prof_dpi, 26);

        const int leftW  = dpi_scale(s_prof_dpi, 260);
        int startW       = dpi_scale(s_prof_dpi, 260);

        const int btnW_sm = dpi_scale(s_prof_dpi, 32);
        const int btnW    = dpi_scale(s_prof_dpi, 112);
        const int closeW  = dpi_scale(s_prof_dpi, 80);

        // Keep a bottom strip for the Close button
        const int bottom_strip = btnH + g;
        const int content_bottom = std::max(m, ch - m - bottom_strip);

        // Left pane: profile list + name + buttons
        int xL = m;
        int y = m;
        int xR0 = xL + leftW + g;

        if (st_profiles_hdr) MW(st_profiles_hdr, xL, y, leftW, labelH, TRUE);
        y += labelH + g2;

        // Bottom controls height under list
        const int name_block_h = labelH + g2 + editH + g2 + (btnH * 2 + g2);
        int listH = std::max(dpi_scale(s_prof_dpi, 140), (content_bottom - y) - name_block_h);
        MW(lb_profiles, xL, y, leftW, listH, TRUE);

        int yName = y + listH + g2;
        if (st_name_hdr) MW(st_name_hdr, xL, yName, leftW, labelH, TRUE);
        yName += labelH + g2;
        MW(e_name, xL, yName, leftW, editH, TRUE);

        int yBtn = yName + editH + g2;
        // row 1
        MW(b_add,    xL,                 yBtn, dpi_scale(s_prof_dpi, 60), btnH, TRUE);
        MW(b_rename, xL + dpi_scale(s_prof_dpi, 66), yBtn, dpi_scale(s_prof_dpi, 70), btnH, TRUE);
        MW(b_del,    xL + dpi_scale(s_prof_dpi, 142), yBtn, dpi_scale(s_prof_dpi, 70), btnH, TRUE);
        MW(b_up,     xL + leftW - btnW_sm, yBtn, btnW_sm, btnH, TRUE);

        // row 2 (down)
        int yBtn2 = yBtn + btnH + g2;
        MW(b_down, xL + leftW - btnW_sm, yBtn2, btnW_sm, btnH, TRUE);

        // Close button bottom-right
        if (b_close) {
            MoveWindow(b_close, cw - m - closeW, viewH - m - btnH, closeW, btnH, TRUE);
        }

        // Right pane split: main editor + "Start EXEs" panel on the right
        int xStart = cw - m - startW;
        int mainX = xR0;
        int mainW = xStart - g - mainX;

        // If window is narrow, shrink start panel a bit (but keep it on the right as requested)
        int mainMin = dpi_scale(s_prof_dpi, 360);
        int startMin = dpi_scale(s_prof_dpi, 200);
        if (mainW < mainMin) {
            startW = std::max(startMin, startW - (mainMin - mainW));
            xStart = cw - m - startW;
            mainW = xStart - g - mainX;
        }

        // ---- WATCH section (top, main area) ----
        int yR = m;
        if (st_watch_hdr) MW(st_watch_hdr, mainX, yR, mainW, labelH, TRUE);
        yR += labelH + g2;

        int watchBtnW = btnW;
        int watchListW = std::max(dpi_scale(s_prof_dpi, 220), mainW - g - watchBtnW);
        int watchListH = dpi_scale(s_prof_dpi, 230);
        MW(lb_watch, mainX, yR, watchListW, watchListH, TRUE);

        int xWB = mainX + watchListW + g;
        MW(b_watch_add, xWB, yR, watchBtnW, btnH, TRUE);
        MW(b_watch_del, xWB, yR + btnH + g2, watchBtnW, btnH, TRUE);

        int yAddWatch = yR + watchListH + g2;
        if (st_watch_add_hdr) MW(st_watch_add_hdr, mainX, yAddWatch, watchListW, labelH, TRUE);
        yAddWatch += labelH + g2;

        int editW = watchListW;
        int addBtnW = watchBtnW;
        MW(e_watch, mainX, yAddWatch, editW, editH, TRUE);

        // Place Add WATCH button aligned with the edit (keep "Remove selected" under it)
        MW(b_watch_add, xWB, yAddWatch, addBtnW, btnH, TRUE);
        MW(b_watch_del, xWB, yAddWatch + btnH + g2, addBtnW, btnH, TRUE);

        int yContents = yAddWatch + std::max(editH, btnH * 2 + g2) + g;

        // ---- Start EXEs panel (right) ----
        int startY = yContents;
        int startLabelH = dpi_scale(s_prof_dpi, 36);
        if (st_start_hdr) MW(st_start_hdr, xStart, startY, startW, startLabelH, TRUE);
        int yStartBody = startY + startLabelH + g2;

        int startBtnW = dpi_scale(s_prof_dpi, 80);
        int startEditW = std::max(dpi_scale(s_prof_dpi, 140), startW - g - startBtnW);

        int startAddAreaH = labelH + g2 + std::max(editH, btnH * 2 + g2);
        int startListH = std::max(dpi_scale(s_prof_dpi, 140), (content_bottom - yStartBody) - g - startAddAreaH);

        MW(lb_start_exe, xStart, yStartBody, startW, startListH, TRUE);

        int yStartAdd = yStartBody + startListH + g2;
        if (st_start_add_hdr) MW(st_start_add_hdr, xStart, yStartAdd, startW, labelH, TRUE);
        yStartAdd += labelH + g2;

        MW(e_start_exe, xStart, yStartAdd, startEditW, editH, TRUE);
        int xSB = xStart + startEditW + g;
        MW(b_start_exe_add, xSB, yStartAdd, startBtnW, btnH, TRUE);
        MW(b_start_exe_del, xSB, yStartAdd + btnH + g2, startBtnW, btnH, TRUE);

        // ---- Profile contents (services / exes) in main area ----
        int yC = yContents;
        if (st_contents_hdr) MW(st_contents_hdr, mainX, yC, mainW, labelH, TRUE);
        yC += labelH + g2;

        int colW = std::max(dpi_scale(s_prof_dpi, 200), (mainW - g) / 2);
        int xSvc = mainX;
        int xExe = mainX + colW + g;
        colW = mainW - g - colW; // exe column uses remaining width

        // Bottom labels
        int noteH = dpi_scale(s_prof_dpi, 40);
        int activeY = content_bottom - (noteH + g2 + labelH);
        int noteY = content_bottom - noteH;
        if (st_active) MW(st_active, mainX, activeY, mainW, labelH, TRUE);
        if (st_note) MW(st_note, mainX, noteY, mainW, noteH, TRUE);

        // Compute list height to fill remaining vertical space
        int colControlH = editH + g2 + chkH + g2 + (btnH * 3 + g2 * 2);
        int listTop = yC + labelH + g2;
        int listH2 = std::max(dpi_scale(s_prof_dpi, 140), (activeY - g) - listTop - colControlH);

        // Services column
        if (st_svc_hdr) MW(st_svc_hdr, xSvc, yC, (mainW - g) / 2, labelH, TRUE);
        MW(lb_svc, xSvc, listTop, (mainW - g) / 2, listH2, TRUE);

        int ySvcCtrl = listTop + listH2 + g2;
        MW(e_svc, xSvc, ySvcCtrl, (mainW - g) / 2, editH, TRUE);
        ySvcCtrl += editH + g2;
        MW(chk_svc, xSvc, ySvcCtrl, (mainW - g) / 2, chkH, TRUE);
        ySvcCtrl += chkH + g2;
        MW(b_svc_add, xSvc, ySvcCtrl, (mainW - g) / 2, btnH, TRUE);
        ySvcCtrl += btnH + g2;
        MW(b_svc_toggle, xSvc, ySvcCtrl, (mainW - g) / 2, btnH, TRUE);
        ySvcCtrl += btnH + g2;
        MW(b_svc_del, xSvc, ySvcCtrl, (mainW - g) / 2, btnH, TRUE);

        // EXEs column
        if (st_exe_hdr) MW(st_exe_hdr, xExe, yC, colW, labelH, TRUE);
        MW(lb_exe, xExe, listTop, colW, listH2, TRUE);

        int yExeCtrl = listTop + listH2 + g2;
        MW(e_exe, xExe, yExeCtrl, colW, editH, TRUE);
        yExeCtrl += editH + g2;
        MW(chk_exe, xExe, yExeCtrl, colW, chkH, TRUE);
        yExeCtrl += chkH + g2;
        MW(b_exe_add, xExe, yExeCtrl, colW, btnH, TRUE);
        yExeCtrl += btnH + g2;
        MW(b_exe_toggle, xExe, yExeCtrl, colW, btnH, TRUE);
        yExeCtrl += btnH + g2;
        MW(b_exe_del, xExe, yExeCtrl, colW, btnH, TRUE);
    
        // Update vertical scrollbar based on virtual content height
        SCROLLINFO si{};
        si.cbSize = sizeof(si);
        si.fMask = SIF_RANGE | SIF_PAGE | SIF_POS;
        si.nMin = 0;
        si.nMax = (layoutH > 0) ? (layoutH - 1) : 0;
        si.nPage = (UINT)viewH;
        si.nPos = s_scrollY;
    ...
    case WM_GETMINMAXINFO: {
        ...
        if (p) {
            ...
            const int minH = dpi_scale(s_prof_dpi, 520);
    ...
    case WM_VSCROLL: {
        ...
        int pos = si.nPos;
        const int line = dpi_scale(s_prof_dpi, 28);
    ...
    case WM_MOUSEWHEEL: {
        ...
        if (lines <= 0) lines = 3;
        const int step = dpi_scale(s_prof_dpi, 24) * lines;

        s_scrollY -= (delta / WHEEL_DELTA) * step;
        do_layout();
        InvalidateRect(dlg, NULL, TRUE);
        return 0;
    }

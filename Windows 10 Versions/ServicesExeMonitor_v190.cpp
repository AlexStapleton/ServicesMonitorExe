...
#define TIMER_THEME_FALLBACK 106
...
static void ensure_hwnd_visible_on_screen(HWND hwnd) {
    if (!hwnd) return;
    RECT rc{};
    ...
    const int margin = 24;
    bool offscreen =
        (rc.right  < wa.left + margin) ||
        (rc.left   > wa.right - margin) ||
        (rc.bottom < wa.top + margin) ||
        (rc.top    > wa.bottom - margin);
...
static [[maybe_unused]] bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts, int* out_row_idx);
...
static bool try_enable_dark_titlebar(HWND hwnd, bool enable) {
    ...
    PFN_DwmFlush pFlush = nullptr;
    LOAD_PROC(dwm, "DwmFlush", PFN_DwmFlush, pFlush);
...
struct Item {
    ...
    bool     ui_cache_autostop = false;  // cached checkbox state for owner-data ListView
...
// Per-monitor DPI (for correct scaling at non-100% Windows display scaling)
    ...
    // View pointers (UI-owned): row i -> Item* for current view (rebuilt alongside view_uids)
    std::vector<Item*> view_items[2];
...
HWND exe_empty;
    ...
    // theme
    bool dark_mode;
    // Deferred "hard" non-client refresh (to avoid blink unless needed)
    bool theme_hard_fallback_pending = false;
    ...
    std::atomic<bool> ui_dirty_posted{false};       // coalesce WM_APP_MODEL_DIRTY
...
static LRESULT CALLBACK ListViewSubclassProc(HWND, UINT, WPARAM, LPARAM, UINT_PTR, DWORD_PTR);
...
static LRESULT CALLBACK ListViewSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR, DWORD_PTR refData) {
    App* self = (App*)refData;
    if (!self) return DefSubclassProc(hwnd, msg, wParam, lParam);

    // Handle Space ourselves so the owner-data checkbox toggles immediately without relying on
    // the ListView's internal checkbox state (which can be inconsistent for LVS_OWNERDATA).
    if (msg == WM_KEYDOWN && wParam == VK_SPACE) {
        const int kind = (hwnd == self->lv_svc) ? KIND_SVC : KIND_EXE;
        toggle_autostop_selected(*self, kind);

        // Force an immediate repaint of the selected row (and its state image).
        const int iSel = ListView_GetNextItem(hwnd, -1, LVNI_SELECTED);
        if (iSel >= 0) {
            ListView_RedrawItems(hwnd, iSel, iSel);
        } else {
    ...
    // Also swallow WM_CHAR space to prevent beep / duplicate handling on some systems.
    if (msg == WM_CHAR && wParam == L' ') {
        return 0;
    }
...
static void darkmode_set_preferred_app_mode(bool dark);
...
static LRESULT theme_customdraw_listview(App& self, NMLVCUSTOMDRAW* lcd) {
    ...
    case CDDS_ITEMPREPAINT | CDDS_SUBITEM: {
        ...
        // Selected row styling (keep it subtle). On some Win10 builds, returning CDRF_NEWFONT
        // here can interfere with the default text paint; only set colors and let the control
        // draw normally.
...
// Build new colors/brushes first, then swap into App in one go.
// This avoids transient mismatches (old brush + new color) during theme toggles.
struct ThemeRes {
    COLORREF col_bg = 0;
    ...
    HBRUSH br_btn = NULL;
    bool owns = false;
    bool ok() const { return br_bg && br_panel && br_edit && br_btn; }
    void cleanup() {
        if (owns) {
            if (br_bg) DeleteObject(br_bg);
            if (br_panel) DeleteObject(br_panel);
            if (br_edit) DeleteObject(br_edit);
            if (br_btn) DeleteObject(br_btn);
        }
        br_bg = br_panel = br_edit = br_btn = NULL;
    }
};

ThemeRes nr{};
if (self.dark_mode) {
    nr.col_bg       = RGB(32,32,32);
    nr.col_panel    = RGB(40,40,40);
    nr.col_edit_bg  = RGB(28,28,28);
    nr.col_text     = RGB(230,230,230);
    nr.col_text_dim = RGB(160,160,160);

    nr.br_bg    = CreateSolidBrush(nr.col_bg);
    nr.br_panel = CreateSolidBrush(nr.col_panel);
    nr.br_edit  = CreateSolidBrush(nr.col_edit_bg);
    nr.br_btn   = CreateSolidBrush(nr.col_panel);
    nr.owns = true;

    if (!nr.ok()) {
        nr.cleanup();
        // If we fail to allocate, fall back to system colors to remain usable.
        nr.owns = false;
    }
}

if (!nr.owns) {
    nr.col_bg       = GetSysColor(COLOR_BTNFACE);
    nr.col_panel    = GetSysColor(COLOR_BTNFACE);
    nr.col_edit_bg  = GetSysColor(COLOR_WINDOW);
    nr.col_text     = GetSysColor(COLOR_WINDOWTEXT);
    nr.col_text_dim = GetSysColor(COLOR_GRAYTEXT);

    nr.br_bg    = GetSysColorBrush(COLOR_BTNFACE);
    nr.br_panel = GetSysColorBrush(COLOR_BTNFACE);
    nr.br_edit  = GetSysColorBrush(COLOR_WINDOW);
    nr.br_btn   = GetSysColorBrush(COLOR_BTNFACE);
    nr.owns = false;
}

// Swap in new resources atomically.
theme_release_brushes(self);
self.col_bg       = nr.col_bg;
self.col_panel    = nr.col_panel;
self.col_edit_bg  = nr.col_edit_bg;
self.col_text     = nr.col_text;
self.col_text_dim = nr.col_text_dim;

self.br_bg    = nr.br_bg;
self.br_panel = nr.br_panel;
self.br_edit  = nr.br_edit;
self.br_btn   = nr.br_btn;

self.theme_owns_brushes = nr.owns;
...
static DWORD win_build_number();
static void theme_broadcast_change(HWND hwnd);
static void theme_broadcast_change(HWND hwnd, const std::vector<HWND>* children);
// --------------------------------------------------
// Theme transaction (reduces flicker + ensures consistent repaint ordering)
// --------------------------------------------------
struct ThemeTxn {
    App& self;
    HWND hwnd = NULL;
    std::vector<HWND> themed_children;
    DWORD build = 0;
    bool is_win10 = false;
    bool ended = false;

    ThemeTxn(App& s, HWND h) : self(s), hwnd(h) {
        build = win_build_number();
        is_win10 = (build != 0 && build < 22000);
        if (hwnd) SendMessageW(hwnd, WM_SETREDRAW, FALSE, 0);
    }

    void force_nc_refresh(bool activate) {
        if (!hwnd) return;
        // Nudge Win10 to repaint the non-client area (caption buttons/titlebar).
        SendMessageW(hwnd, WM_THEMECHANGED, 0, 0);
        SendMessageW(hwnd, WM_SETTINGCHANGE, 0, (LPARAM)L"ImmersiveColorSet");
        SendMessageW(hwnd, WM_NCACTIVATE, (WPARAM)(activate ? TRUE : FALSE), 0);
        SetWindowPos(hwnd, nullptr, 0, 0, 0, 0,
            SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);
    }

    void end() {
        if (ended || !hwnd) return;
        ended = true;

        SendMessageW(hwnd, WM_SETREDRAW, TRUE, 0);

        // Broadcast theme/color changes so controls repaint immediately (avoids needing minimize/restore).
        theme_broadcast_change(hwnd, &themed_children);

        // Final redraw pass (single, synchronous).
        force_nc_refresh(true);
        RedrawWindow(hwnd, NULL, NULL, RDW_INVALIDATE | RDW_ALLCHILDREN | RDW_FRAME | RDW_UPDATENOW);
    }

    ~ThemeTxn() {
        if (!ended && hwnd) {
            // Best-effort restore redraw even if caller exits early.
            SendMessageW(hwnd, WM_SETREDRAW, TRUE, 0);
        }
    }
};
...
static void theme_force_immediate_repaint_soft(HWND hwnd) {
    if (!hwnd) return;

    // Force non-client metrics to be recalculated and painted immediately.
    SetWindowPos(hwnd, NULL, 0, 0, 0, 0,
                 SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);

    // Ask for NC repaint explicitly (some Win10 builds otherwise wait for a future state change).
    SendMessageW(hwnd, WM_NCPAINT, 1, 0);
    SendMessageW(hwnd, WM_NCACTIVATE, TRUE, 0);

    // Strong full repaint: include erase once to clear stale light-mode remnants.
    RedrawWindow(hwnd, NULL, NULL,
                 RDW_INVALIDATE | RDW_ERASE | RDW_FRAME | RDW_ALLCHILDREN | RDW_UPDATENOW);

    // Best-effort flush of DWM to apply caption/titlebar changes immediately.
    HMODULE dwm = GetModuleHandleW(L"dwmapi.dll");
    bool loaded = false;
    if (!dwm) { dwm = LoadLibraryW(L"dwmapi.dll"); loaded = (dwm != NULL); }
    if (dwm) {
        using PFN_DwmFlush = HRESULT (WINAPI*)();
        auto pDwmFlush = (PFN_DwmFlush)GetProcAddress(dwm, "DwmFlush");
        if (pDwmFlush) pDwmFlush();
        if (loaded) FreeLibrary(dwm);
    }
}


static void theme_force_immediate_repaint_hard(HWND hwnd) {
    if (!hwnd) return;

    // As a last resort (when Win10/Win11 refuses to repaint the caption immediately),
    // simulate the "minimize/restore fixes it" behavior without actually minimizing.
    if (IsWindowVisible(hwnd)) {
            ...
            if (GetWindowPlacement(hwnd, &wp)) {
                if (wp.showCmd != SW_SHOWMINIMIZED && wp.showCmd != SW_MINIMIZE) {
                    ...
                    // Restore original placement/rect and force another NC recompute.
                    SetWindowPlacement(hwnd, &wp);
                    SetWindowPos(hwnd, NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top,
                                 SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);
                    RedrawWindow(hwnd, NULL, NULL, RDW_INVALIDATE | RDW_FRAME | RDW_ALLCHILDREN | RDW_UPDATENOW);
                }
            }
        }
}
...
struct ThemeEnumCtx {
    App* self = nullptr;
    ThemeTxn* txn = nullptr;
};

static BOOL CALLBACK enum_theme_children_collect_proc(HWND child, LPARAM lp) {
    ThemeEnumCtx* ctx = (ThemeEnumCtx*)lp;
    if (ctx && ctx->self) {
        enum_theme_children_proc(child, (LPARAM)ctx->self);
    }
    if (ctx && ctx->txn) {
        ctx->txn->themed_children.push_back(child);
    }
    return TRUE;
}
...
void theme_apply_to_window(App& self, HWND hwnd) {
    ...
    ThemeTxn txn(self, hwnd);

    // Apply Explorer-style theming where supported (best-effort).
    // Apply Explorer-style theming where supported (best-effort).
    // Use dynamic lookup to avoid build/SDK differences across toolchains.
    try_set_explorer_theme_mode(hwnd, self.dark_mode);
    // Opt into dark mode (best-effort across Win10/11 builds).
    darkmode_set_preferred_app_mode(self.dark_mode);
    darkmode_allow_window(hwnd, self.dark_mode);

    // Apply dark titlebar (DWM attribute). Win10 can be stubborn unless we
    // toggle + force a frame refresh.
    if (txn.is_win10) {
        // Clear then set to force DWM to recompute the caption buttons.
        try_enable_dark_titlebar(hwnd, false);
        txn.force_nc_refresh(true);
    ...
    // Retheme children.
    ThemeEnumCtx ctx{ &self, &txn };
    EnumChildWindows(hwnd, enum_theme_children_collect_proc, (LPARAM)&ctx);
    txn.end();
    theme_force_immediate_repaint_soft(hwnd);
    // If the OS refuses to repaint the non-client area immediately (title bar stays light),
    // schedule a single "hard" refresh shortly after the toggle. This avoids unnecessary blinking
    // on systems where the soft refresh is sufficient.
    if (txn.is_win10) {
        self.theme_hard_fallback_pending = true;
        SetTimer(hwnd, TIMER_THEME_FALLBACK, 140, NULL);
    }
}
        ...
        if (ok) {
                {
                    ...
                    if (!target.empty() && ends_with_i(target.c_str(), L".exe")) {
                        ...
                        if (!key.empty()) {
                            auto ins = self.profile_started_exes.insert(key);
                            if (ins.second) {
                                Item* ex = list_find(&self.items[KIND_EXE], key.c_str());
                                if (ex) self.dirty_status[KIND_EXE].push_back(ex);
                            }
                        }
                    }
                }
...
struct DarkModeApi {
    ...
    using PFN_RefreshImmersiveColorPolicyState = void (WINAPI*)();
    ...
    PFN_RefreshImmersiveColorPolicyState RefreshImmersiveColorPolicyState = nullptr;
...
static DWORD win_build_number() {
    ...
    void* fp = (void*)GetProcAddress(ntdll, "RtlGetVersion");
    if (!fp) return 0;

    auto p = (PFN_RtlGetVersion)fp;
...
static void darkmode_init_api() {
    ...
    // Win10 varies widely by build; these undocumented UxTheme ordinals can crash or behave
    // unexpectedly on older builds. Gate to a conservative baseline (Win10 1809 / build 17763+).
    const DWORD build = win_build_number();
    if (build && build < 17763) return;

    g_dm.ux = LoadLibraryW(L"uxtheme.dll");
    if (!g_dm.ux) return;

    // Best-effort: if ordinals don't exist we simply no-op.
    ...
    g_dm.RefreshImmersiveColorPolicyState = dm_get_ordinal<DarkModeApi::PFN_RefreshImmersiveColorPolicyState>(g_dm.ux, 104);
}
...
static void darkmode_set_preferred_app_mode(bool dark) {
    // Best-effort: informs UxTheme that the app participates in dark mode.
    darkmode_init_api();
    if (g_dm.SetPreferredAppMode) {
        // Values widely used: 0=Default, 1=AllowDark, 2=ForceDark, 3=ForceLight
        g_dm.SetPreferredAppMode(dark ? 1 : 0);
    }
    if (g_dm.FlushMenuThemes) g_dm.FlushMenuThemes();
    if (g_dm.RefreshImmersiveColorPolicyState) g_dm.RefreshImmersiveColorPolicyState();
}
...
static void theme_broadcast_change(HWND hwnd) {
    theme_broadcast_change(hwnd, nullptr);
}

static void theme_broadcast_change(HWND hwnd, const std::vector<HWND>* children) {
    if (!hwnd) return;

    auto send_sync = [](HWND w, UINT msg, WPARAM wp, LPARAM lp) {
        DWORD_PTR out = 0;
        SendMessageTimeoutW(w, msg, wp, lp, SMTO_ABORTIFHUNG | SMTO_BLOCK, 15, &out);
    };

    // Nudge the top-level window to refresh theme-dependent state.
    send_sync(hwnd, WM_THEMECHANGED, 0, 0);
    send_sync(hwnd, WM_SYSCOLORCHANGE, 0, 0);
    send_sync(hwnd, WM_SETTINGCHANGE, 0, (LPARAM)L"ImmersiveColorSet");

    auto touch_child = [&](HWND child) {
        if (!child || !IsWindow(child)) return;

        // Non-blocking for children: avoids slow cascades while still poking controls.
        PostMessageW(child, WM_THEMECHANGED, 0, 0);
        PostMessageW(child, WM_SYSCOLORCHANGE, 0, 0);
        PostMessageW(child, WM_SETTINGCHANGE, 0, (LPARAM)L"ImmersiveColorSet");

        // Force a repaint (avoid background erase flicker for most controls).
        InvalidateRect(child, NULL, FALSE);

        // Targeted fix: listview headers can lag behind the listview.
        wchar_t cls[64] = {};
        GetClassNameW(child, cls, 63);
        if (lstrcmpiW(cls, L"SysListView32") == 0) {
            ...
            if (hdr && IsWindow(hdr)) {
                ...
                RedrawWindow(hdr, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
                UpdateWindow(hdr);
            }
        }
    ...
    // Final pass: redraw parent and all children once in a consistent order.
    RedrawWindow(hwnd, NULL, NULL, RDW_INVALIDATE | RDW_ALLCHILDREN | RDW_FRAME | RDW_UPDATENOW);
    UpdateWindow(hwnd);
}
...
static void show_main_window(App& self) {
    ...
    if (self.have_last_wp) {
        ...
        SetWindowPlacement(hwnd, &wp);
        ensure_hwnd_visible_on_screen(hwnd);
    }
...
static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts, int* out_row_idx) {
    ...
    const bool autostop_old = it->ui_cache_autostop;
    const bool autostop_new = it->auto_stop;
...
static void ui_apply_status_updates(App& self, ItemKind kind) {
...

    {
        ...
        // Refresh per-item UI caches while holding the model lock, then redraw rows without locking.
        // LVN_GETDISPINFO stays lock-free: it only reads ui_cache_*.
        for (Item* it : dirty) {
            if (!it) continue;

            auto fr = self.lv_row_of_item[k].find(it);
            if (fr == self.lv_row_of_item[k].end()) continue; // not currently visible

            // Snapshot existing cache to avoid unnecessary redraw if nothing changed.
            wchar_t old_status[64]{};
            wchar_t old_last[64]{};
            StringCchCopyW(old_status, 64, it->ui_cache_status_disp);
            StringCchCopyW(old_last,   64, it->ui_cache_last_text);
            const bool old_auto = it->ui_cache_autostop;

            ui_seed_ownerdata_cache_locked(self, kind, it);

            const bool changed =
                (old_auto != it->ui_cache_autostop) ||
                (wcscmp(old_status, it->ui_cache_status_disp) != 0) ||
                (wcscmp(old_last, it->ui_cache_last_text) != 0);

            if (changed) rows.push_back(fr->second);
        }
    ...
    const bool big_burst = (rows.size() > 12);
    if (big_burst) SendMessageW(lv, WM_SETREDRAW, FALSE, 0);

    const int row_first = rows.front();
    const int row_last  = rows.back();
    const bool dense = (size_t)(row_last - row_first + 1) <= rows.size() * 2;

    if (dense) {
        ListView_RedrawItems(lv, row_first, row_last);
    } else {
        for (int r : rows) ListView_RedrawItems(lv, r, r);
    }
...
static void ui_seed_ownerdata_cache_locked(App& self, ItemKind kind, Item* it) {
    if (!it) return;

    // Build display strings and UI-only cached state while holding the model lock.
    // This allows LVN_GETDISPINFO to be lock-free and read-only.
    const wchar_t* status_raw = it->last_status;
    const time_t wall_ts = it->last_update_wall;

    wchar_t status_disp[64]{};
    StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown");

    if (kind == ItemKind::Exe) {
        const std::wstring key = exe_watch_key_lower(it->name);
        if (!key.empty() && self.profile_started_exes.find(key) != self.profile_started_exes.end()) {
            wchar_t tmp[64]{};
            StringCchCopyW(tmp, 64, status_disp);
            StringCchPrintfW(status_disp, 64, L"* %s", tmp);
        }
    ...
    if (wall_ts) {
        if (it->ui_last_update_wall != wall_ts || it->ui_cache_last_text[0] == 0) {
            fmt_time_local(it->ui_cache_last_text, 64, wall_ts);
        }
    } else {
        it->ui_cache_last_text[0] = 0;
    }

    it->ui_cache_autostop = it->auto_stop;

    // Keep the "last applied" markers consistent so future incremental updates can be cheap.
    StringCchCopyW(it->ui_last_status, 32, it->ui_cache_status_disp);
    it->ui_last_update_wall = wall_ts;
    it->ui_applied_gen = it->status_gen;
}
...
self.view_items[k].clear();
self.view_items[k].reserve(self.items[k].v.size());
self.lv_row_of_item[k].clear();
...
for (auto& up : self.items[k].v) {
    ...
    self.view_items[k].push_back(it);
    self.lv_row_of_item[k][it] = row;

    // Seed/refresh UI caches under lock so LVN_GETDISPINFO can be lock-free.
    ui_seed_ownerdata_cache_locked(self, kind, it);
}
...
static void toggle_autostop_selected(App& self, int kind) {
    ...
    bool now = false;
    {
        ...
        now = real->auto_stop;
        ui_seed_ownerdata_cache_locked(self, kind_from_int(kind), real);
    }

    int row = lv_find_item_by_uid(lv, sel->uid);
    // Owner-data ListView: force repaint so the state image updates immediately.
    if (row >= 0) {
...
static LRESULT CALLBACK HotkeysWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        if (b_copy) SendMessageW(b_copy, WM_SETFONT, (WPARAM)f, TRUE);
        if (b_close) SendMessageW(b_close, WM_SETFONT, (WPARAM)f, TRUE);

        // Apply dark mode/theme to this window + its children (Win10 needs explicit application).
        theme_apply_to_window(self, dlg);
    ...
    case WM_NOTIFY: {
        ...
        if (self.dark_mode && nh->code == NM_CUSTOMDRAW) {
            wchar_t ccls[64]{0};
            ...
            std::vector<StartedProc> to_close;
            {
                ...
                if (removed && self.active_profile == psel) {
                    ...
                    if (ends_with_i(target.c_str(), L".exe")) {
                        std::wstring key = exe_watch_key_lower(target);
                        if (!key.empty()) {
                            if (self.profile_started_exes.erase(key) > 0) {
                                Item* ex = list_find(&self.items[KIND_EXE], key.c_str());
                                if (ex) self.dirty_status[KIND_EXE].push_back(ex);
                            }
                        }
...
self.left_tabs = make_tab(hwnd, IDC_LEFT_TABS);
    ...
    // Subclass owner-data ListViews so Space toggles Auto-Stop immediately and reliably.
    SetWindowSubclass(self.lv_svc, ListViewSubclassProc, 2, (DWORD_PTR)&self);
    SetWindowSubclass(self.lv_exe, ListViewSubclassProc, 3, (DWORD_PTR)&self);
...
static bool cmd_set_autostop_uid(App& self, uint32_t uid, bool enabled, std::wstring* out_name_opt = nullptr) {
    ...
    bool changed = false;
    {
        ...
        if (it->auto_stop != enabled) {
            ...
            changed = true;
            ui_seed_ownerdata_cache_locked(self, kind_from_int(it->kind), it);
        }
...
// Fast path: view_items is rebuilt on the UI thread alongside view_uids and is stable for this view.
if (row >= 0 && row < (int)self.view_items[kind_i].size()) {
    itp = self.view_items[kind_i][row];
    if (itp) uid = itp->uid;
}

// Fallback (rare): resolve via uid_map if view_items is unavailable/out-of-sync.
if (!itp || !uid) {
    ModelLockGuard lk(self);
    if (row >= 0 && row < (int)self.view_uids[kind_i].size()) uid = self.view_uids[kind_i][row];
    if (uid) {
        auto f = self.uid_map.find(uid);
...
if (!uid || !itp) { out = 0; return true; }
        ...
        if (di->item.mask & LVIF_STATE) {
            ...
            di->item.state = INDEXTOSTATEIMAGEMASK(itp->ui_cache_autostop ? 2 : 1);
        }
...
// Checkbox toggle (auto-stop)
            if ((lv->uNewState ^ lv->uOldState) & LVIS_STATEIMAGEMASK) {
                ...
                int img = (int)((lv->uNewState & LVIS_STATEIMAGEMASK) >> 12);
                bool enabled = false;
                if (img == 2) enabled = true;
                else if (img == 1) enabled = false;
                else enabled = (ListView_GetCheckState(nh->hwndFrom, lv->iItem) != 0);

                const bool changed = cmd_set_autostop_uid(self, uid, enabled);
                if (changed) {
    ...
    // Click on checkbox/state icon toggles Auto-Stop (especially needed for owner-data ListView on Win10).
    if (nh->code == NM_CLICK) {
        NMITEMACTIVATE* ia = (NMITEMACTIVATE*)lParam;
        if (ia) {
            LVHITTESTINFO ht;
            memset(&ht, 0, sizeof(ht));
            ht.pt = ia->ptAction; // client coords
            int hit = ListView_SubItemHitTest(nh->hwndFrom, &ht);
            if (hit >= 0) {
                // Ensure selection/focus follows the click even when the state icon is clicked.
                ListView_SetItemState(nh->hwndFrom, hit, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
                ListView_SetSelectionMark(nh->hwndFrom, hit);

                if (ht.flags & LVHT_ONITEMSTATEICON) {
                    uint32_t uid = 0;
                    {
                        ModelLockGuard lk(self);
                        if (hit < (int)self.view_uids[kind_i].size()) uid = self.view_uids[kind_i][hit];
                    }
                    if (uid) {
                        bool cur = false;
                        {
                            ModelLockGuard lk(self);
                            auto f = self.uid_map.find(uid);
                            if (f != self.uid_map.end() && f->second) cur = f->second->auto_stop;
                        }
                        (void)cmd_set_autostop_uid(self, uid, !cur);
                        ListView_RedrawItems(nh->hwndFrom, hit, hit);
                    }
                    out = 0;
                    return true;
                }
    ...
    if (nh->code == LVN_KEYDOWN) {
        ...
        // Space toggles Auto-Stop for the selected row (Windows 10 owner-data ListView
        // does not always toggle the checkbox state image automatically).
        if (kd->wVKey == VK_SPACE) {
            toggle_autostop_selected(self, kind_i);
            out = 0;
            return true;
        }
...
static void post_model_dirty(App& self) {
    // Coalesce: many model updates -> one UI message.
    // Can be called from any thread.
    if (self.ui_dirty_posted.exchange(true, std::memory_order_relaxed)) return;
    if (!self.hwnd) return;
...
switch (msg) {
    case WM_CREATE: {
        ...
        // Apply dark mode to the top-level window (non-client area + caption buttons) early.
        theme_apply_to_window(self, hwnd);
...
switch (id) {
    ...
    case WM_APP_MODEL_DIRTY: {
        ...
        self.ui_dirty_posted.store(false, std::memory_order_relaxed);
        ...
        // Apply any queued per-row cache changes (autostop toggles, profile-start markers, etc.).
        ui_apply_status_updates(self, KIND_SVC);
        ui_apply_status_updates(self, KIND_EXE);
...
if (wParam == TIMER_SEARCH_DEBOUNCE) {
    ...
    return 0;
        ...
        if (wParam == TIMER_THEME_FALLBACK) {
            KillTimer(hwnd, TIMER_THEME_FALLBACK);
            if (self.theme_hard_fallback_pending) {
                self.theme_hard_fallback_pending = false;
                theme_force_immediate_repaint_hard(hwnd);
            }
            return 0;
        }
        return 0;
...
self.taskbar_restart_msg = 0;
    ...
    // Win10 fix: persisted window placement can end up off-screen after monitor/DPI changes.
    ensure_hwnd_visible_on_screen(hwnd);
    ...
    // Some Win10 setups launch us minimized (window only appears in the taskbar).
    // We don't have an explicit "start minimized" feature here, so prefer showing normally.

...
static void open_hotkeys_dialogstatic void open_hotkeys_dialog(App& self, HWND parent) {
    WNDCLASSW wc;
    memset(&wc, 0, sizeof(wc));
...
if ((nh->idFrom == IDC_LV_SVC || nh->idFrom == IDC_LV_EXE) && nh->code == LVN_KEYDOWN) {
    ...
    if (kd) {
        ...
        if (kd->wVKey == VK_SPACE) {
            HWND lvw = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe;
            int row = ListView_GetNextItem(lvw, -1, LVNI_SELECTED);
            if (row >= 0) toggle_autostop_by_row(self, kind, row);
            return 0;
        }
    }
}

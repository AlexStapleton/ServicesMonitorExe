...
static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], size_t n, time_t wall_ts) {
...

    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (!it) continue;
            StringCchCopyW(it->last_status, 32, statuses[i][0] ? statuses[i] : L"unknown");
            it->last_update_wall = wall_ts;
        }
        self.last_any_status_wall = wall_ts;
    }

    // Wake UI to apply deltas without copying names/status blobs.
    PostMessageW(self.hwnd, WM_APP_STATUS_GEN, (WPARAM)kind, (LPARAM)(INT_PTR)wall_ts);
}
...
static inline bool is_ws_char(wchar_t c) {
...
static std::wstring exe_watch_key_lower(const std::wstring& raw) {
    ...
    while (!s.empty() && is_ws_char(s.front())) s.erase(s.begin());
    while (!s.empty() && is_ws_char(s.back()))  s.pop_back();
    if (s.empty()) return s;
...
static bool build_running_exe_set_lower(std::unordered_set<std::wstring>& out_set) {
    out_set.clear();
    ...
    do {
        std::wstring exe = pe.szExeFile; // typically already basename
        if (!exe.empty()) {
            CharLowerBuffW(exe.data(), (DWORD)exe.size());
            out_set.insert(std::move(exe));
        }
...
static void ui_apply_status_updates(App& self, int kind) {
    ...
    std::vector<Item*> to_update;
    to_update.reserve(64);

    {
        std::lock_guard<std::mutex> lk(self.mtx);
        ItemList& L = self.items[kind];
        for (auto& up : L.v) {
            Item* it = up.get();
            if (!it) continue;

            if (it->last_update_wall != it->ui_last_update_wall ||
                wcscmp(it->last_status, it->ui_last_status) != 0) {

                // Update UI cache now so we only repaint once per change.
                StringCchCopyW(it->ui_last_status, 32, it->last_status);
                it->ui_last_update_wall = it->last_update_wall;
...
static void monitor_thread_main(std::stop_token st, App* self_ptr) {
    ...
    // Reused per-tick: running process exe names (lowercased)
    std::unordered_set<std::wstring> running_exes;
    running_exes.reserve(256);
...
{
    // Take ONE Toolhelp snapshot per tick, then do O(1) membership checks.
    build_running_exe_set_lower(running_exes);

    int cur = -1;
    std::shared_ptr<const std::vector<std::vector<std::wstring>>> watches_sp;
    {
        std::lock_guard<std::mutex> lk(self.mtx);
        cur = self.active_profile;
        watches_sp = self.profile_watch_keys_sp;
    }
    ...
    if (watches_sp) {
        ...
        for (size_t i = 0; i < watches.size(); i++) {
            ...
            for (const auto& key : ws) {
                if (key.empty()) continue; // keys are already normalized
                if (running_exes.find(key) != running_exes.end()) { desired = (int)i; break; }
            }
            if (desired >= 0) break;
        }
    }

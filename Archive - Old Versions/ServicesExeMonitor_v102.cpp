...
// --------------------------------------------------
static bool parse_setting_to_cfg(ConfigSnapshot& cfg, const wchar_t* key, const wchar_t* val) { if (!key || !val) return false; auto parse_int = [&](int* out) -> bool { if (!out) return false; *out = _wtoi(val); return true; }; auto parse_seconds_to_ms = [&](int* out_ms) -> bool { if (!out_ms) return false; double s = _wtof(val); if (s < 0) return false; long long ms = (long long)(s * 1000.0 + 0.5); if (ms > 0x7fffffffLL) ms = 0x7fffffffLL; *out_ms = (int)ms; return true; }; if (_wcsicmp(key, L"ui_refresh_ms") == 0) { int v = 0; parse_int(&v); if (v >= 200) { cfg.ui_refresh_ms = v; return true; } return false; } if (_wcsicmp(key, L"monitor_interval_ms") == 0) { int v = 0; parse_int(&v); if (v >= 200) { cfg.monitor_interval_ms = v; return true; } return false; } if (_wcsicmp(key, L"monitor_interval_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms) && ms >= 200) { cfg.monitor_interval_ms = ms; return true; } return false; } if (_wcsicmp(key, L"autostop_cooldown_ms") == 0) { int v = 0; parse_int(&v); if (v >= 0) { cfg.autostop_cooldown_ms = v; return true; } return false; } if (_wcsicmp(key, L"autostop_cooldown_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms)) { cfg.autostop_cooldown_ms = ms; return true; } return false; } if (_wcsicmp(key, L"stop_wait_ms") == 0) { int v = 0; parse_int(&v); if (v >= 0) { cfg.stop_wait_ms = v; return true; } return false; } if (_wcsicmp(key, L"stop_wait_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms)) { cfg.stop_wait_ms = ms; return true; } return false; } if (_wcsicmp(key, L"tray_enabled") == 0) { int v = 0; parse_int(&v); cfg.tray_enabled = (v != 0); if (!cfg.tray_enabled) cfg.close_to_tray = false; return true; } if (_wcsicmp(key, L"close_to_tray") == 0) { int v = 0; parse_int(&v); cfg.close_to_tray = (v != 0); if (!cfg.tray_enabled) cfg.close_to_tray = false; return true; } return false; } static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out); static void snapshot_active_from_runtime_locked(App& self); [[maybe_unused]] static void snapshot_active_from_runtime(App& self); static void apply_profile_index(App& self, int idx); static void action_enqueue(App& self, int kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms); static void update_statusbar(App& self); // ---------------------------- // Profile helpers // ---------------------------- static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out) { out.ui_refresh_ms = self.ui_refresh_ms; out.monitor_interval_ms = self.monitor_interval_ms; out.autostop_cooldown_ms = self.autostop_cooldown_ms; out.stop_wait_ms = self.stop_wait_ms; out.tray_enabled = self.tray_enabled; out.close_to_tray = self.close_to_tray; cfg_clear_items(out); for (int kind = 0; kind < 2; kind++) { out.items[kind].reserve(self.items[kind].v.size()); for (auto& up : self.items[kind].v) { Item* it = up.get(); if (!it || it->name.empty()) continue; ItemRow r; r.name = it->name; r.auto_stop = it->auto_stop; if (kind == KIND_EXE && !it->exe_path.empty()) r.exe_path = it->exe_path; out.items[kind].push_back(std::move(r)); } } } static void snapshot_active_from_runtime_locked(App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { snapshot_from_runtime_locked(self, self.profiles[(size_t)self.active_profile].cfg); } else { snapshot_from_runtime_locked(self, self.default_cfg); self.have_default_cfg = true; } } static void snapshot_active_from_runtime(App& self) { std::lock_guard<std::mutex> lk(self.mtx); snapshot_active_from_runtime_locked(self); } static void rebuild_runtime_items_locked(App& self, const ConfigSnapshot& cfg) { list_free(&self.items[KIND_SVC]); list_free(&self.items[KIND_EXE]); for (int kind = 0; kind < 2; kind++) { for (const auto& row : cfg.items[kind]) { if (row.name.empty()) continue; Item* it = new (std::nothrow) Item(); if (!it) continue; it->kind = kind; it->name = row.name; if (kind == KIND_EXE && !row.exe_path.empty()) it->exe_path = row.exe_path; it->auto_stop = row.auto_stop; it->img = (kind == KIND_SVC) ? 0 : 1; it->autostop_count = 0; it->last_autostop_mono_ms = 0; it->last_update_wall = time(NULL); if (kind == KIND_SVC) { wchar_t st[32]; if (query_service_status_fast(it->name.c_str(), st, 32)) StringCchCopyW(it->last_status, 32, st); else StringCchCopyW(it->last_status, 32, L"not found"); } else { int c = process_count_by_name_lower(it->name.c_str()); StringCchCopyW(it->last_status, 32, (c > 0) ? L"running" : L"stopped"); } list_push(&self.items[kind], it); } } self.items_gen[KIND_SVC]++; self.items_gen[KIND_EXE]++; } static const wchar_t* active_profile_name_locked(const App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { const auto& p = self.profiles[(size_t)self.active_profile].name; return p.empty() ? L"(unnamed)" : p.c_str(); } return L"Default"; } // UI thread: apply config snapshot to runtime + listviews static void apply_profile_index(App& self, int idx) { // idx: -1 = default, 0..N-1 = profiles std::vector<Item*> svc_ptrs; std::vector<Item*> exe_ptrs; std::wstring pname; // Processes started by the *previous* active profile (close them when leaving) std::vector<StartedProc> to_stop_started; // Shell-open targets to launch for the *new* profile activation std::vector<std::wstring> to_launch;;
static bool parse_setting(App& self, const wchar_t* key, const wchar_t* val) { if (!key || !val) return false; // NOTE: Settings are stored in milliseconds (ms). For backward compatibility, // we still accept legacy *_s keys (seconds) and convert to ms. auto parse_int = [&](int* out) -> bool { if (!out) return false; // _wtoi tolerates leading/trailing whitespace and stops on non-digit. *out = _wtoi(val); return true; }; auto parse_seconds_to_ms = [&](int* out_ms) -> bool { if (!out_ms) return false; double s = _wtof(val); if (s < 0) return false; // round to nearest millisecond long long ms = (long long)(s * 1000.0 + 0.5); if (ms > 0x7fffffffLL) ms = 0x7fffffffLL; *out_ms = (int)ms; return true; }; if (_wcsicmp(key, L"ui_refresh_ms") == 0) { int v = 0; parse_int(&v); if (v >= 200) { self.ui_refresh_ms = v; return true; } return false; } if (_wcsicmp(key, L"monitor_interval_ms") == 0) { int v = 0; parse_int(&v); if (v >= 200) { self.monitor_interval_ms = v; return true; } return false; } if (_wcsicmp(key, L"monitor_interval_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms) && ms >= 200) { self.monitor_interval_ms = ms; return true; } return false; } if (_wcsicmp(key, L"autostop_cooldown_ms") == 0) { int v = 0; parse_int(&v); if (v >= 0) { self.autostop_cooldown_ms = v; return true; } return false; } if (_wcsicmp(key, L"autostop_cooldown_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms)) { self.autostop_cooldown_ms = ms; return true; } return false; } if (_wcsicmp(key, L"stop_wait_ms") == 0) { int v = 0; parse_int(&v); if (v >= 0) { self.stop_wait_ms = v; return true; } return false; } if (_wcsicmp(key, L"stop_wait_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms)) { self.stop_wait_ms = ms; return true; } return false; } // Tray settings (were written previously but not parsed; now supported) if (_wcsicmp(key, L"tray_enabled") == 0) { int v = 0; parse_int(&v); self.tray_enabled = (v != 0); if (!self.tray_enabled) self.close_to_tray = false; return true; } if (_wcsicmp(key, L"close_to_tray") == 0) { int v = 0; parse_int(&v); self.close_to_tray = (v != 0); if (!self.tray_enabled) self.close_to_tray = false; return true; } if (_wcsicmp(key, L"dark_mode") == 0) { int v = 0; parse_int(&v); self.dark_mode = (v != 0); return true; } return false; } static bool parse_setting_to_cfg(ConfigSnapshot& cfg, const wchar_t* key, const wchar_t* val) { if (!key || !val) return false; auto parse_int = [&](int* out) -> bool { if (!out) return false; *out = _wtoi(val); return true; }; auto parse_seconds_to_ms = [&](int* out_ms) -> bool { if (!out_ms) return false; double s = _wtof(val); if (s < 0) return false; long long ms = (long long)(s * 1000.0 + 0.5); if (ms > 0x7fffffffLL) ms = 0x7fffffffLL; *out_ms = (int)ms; return true; }; if (_wcsicmp(key, L"ui_refresh_ms") == 0) { int v = 0; parse_int(&v); if (v >= 200) { cfg.ui_refresh_ms = v; return true; } return false; } if (_wcsicmp(key, L"monitor_interval_ms") == 0) { int v = 0; parse_int(&v); if (v >= 200) { cfg.monitor_interval_ms = v; return true; } return false; } if (_wcsicmp(key, L"monitor_interval_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms) && ms >= 200) { cfg.monitor_interval_ms = ms; return true; } return false; } if (_wcsicmp(key, L"autostop_cooldown_ms") == 0) { int v = 0; parse_int(&v); if (v >= 0) { cfg.autostop_cooldown_ms = v; return true; } return false; } if (_wcsicmp(key, L"autostop_cooldown_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms)) { cfg.autostop_cooldown_ms = ms; return true; } return false; } if (_wcsicmp(key, L"stop_wait_ms") == 0) { int v = 0; parse_int(&v); if (v >= 0) { cfg.stop_wait_ms = v; return true; } return false; } if (_wcsicmp(key, L"stop_wait_s") == 0) { int ms = 0; if (parse_seconds_to_ms(&ms)) { cfg.stop_wait_ms = ms; return true; } return false; } if (_wcsicmp(key, L"tray_enabled") == 0) { int v = 0; parse_int(&v); cfg.tray_enabled = (v != 0); if (!cfg.tray_enabled) cfg.close_to_tray = false; return true; } if (_wcsicmp(key, L"close_to_tray") == 0) { int v = 0; parse_int(&v); cfg.close_to_tray = (v != 0); if (!cfg.tray_enabled) cfg.close_to_tray = false; return true; } return false; } static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out); static void snapshot_active_from_runtime_locked(App& self); [[maybe_unused]] static void snapshot_active_from_runtime(App& self); static void apply_profile_index(App& self, int idx); static void action_enqueue(App& self, int kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms); static void update_statusbar(App& self); // ---------------------------- // Profile helpers // ---------------------------- static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out) { out.ui_refresh_ms = self.ui_refresh_ms; out.monitor_interval_ms = self.monitor_interval_ms; out.autostop_cooldown_ms = self.autostop_cooldown_ms; out.stop_wait_ms = self.stop_wait_ms; out.tray_enabled = self.tray_enabled; out.close_to_tray = self.close_to_tray; cfg_clear_items(out); for (int kind = 0; kind < 2; kind++) { out.items[kind].reserve(self.items[kind].v.size()); for (auto& up : self.items[kind].v) { Item* it = up.get(); if (!it || it->name.empty()) continue; ItemRow r; r.name = it->name; r.auto_stop = it->auto_stop; if (kind == KIND_EXE && !it->exe_path.empty()) r.exe_path = it->exe_path; out.items[kind].push_back(std::move(r)); } } } static void snapshot_active_from_runtime_locked(App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { snapshot_from_runtime_locked(self, self.profiles[(size_t)self.active_profile].cfg); } else { snapshot_from_runtime_locked(self, self.default_cfg); self.have_default_cfg = true; } } static void snapshot_active_from_runtime(App& self) { std::lock_guard<std::mutex> lk(self.mtx); snapshot_active_from_runtime_locked(self); } static void rebuild_runtime_items_locked(App& self, const ConfigSnapshot& cfg) { list_free(&self.items[KIND_SVC]); list_free(&self.items[KIND_EXE]); for (int kind = 0; kind < 2; kind++) { for (const auto& row : cfg.items[kind]) { if (row.name.empty()) continue; Item* it = new (std::nothrow) Item(); if (!it) continue; it->kind = kind; it->name = row.name; if (kind == KIND_EXE && !row.exe_path.empty()) it->exe_path = row.exe_path; it->auto_stop = row.auto_stop; it->img = (kind == KIND_SVC) ? 0 : 1; it->autostop_count = 0; it->last_autostop_mono_ms = 0; it->last_update_wall = time(NULL); if (kind == KIND_SVC) { wchar_t st[32]; if (query_service_status_fast(it->name.c_str(), st, 32)) StringCchCopyW(it->last_status, 32, st); else StringCchCopyW(it->last_status, 32, L"not found"); } else { int c = process_count_by_name_lower(it->name.c_str()); StringCchCopyW(it->last_status, 32, (c > 0) ? L"running" : L"stopped"); } list_push(&self.items[kind], it); } } self.items_gen[KIND_SVC]++; self.items_gen[KIND_EXE]++; } static const wchar_t* active_profile_name_locked(const App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { const auto& p = self.profiles[(size_t)self.active_profile].name; return p.empty() ? L"(unnamed)" : p.c_str(); } return L"Default"; } // UI thread: apply config snapshot to runtime + listviews static void apply_profile_index(App& self, int idx) { // idx: -1 = default, 0..N-1 = profiles std::vector<Item*> svc_ptrs; std::vector<Item*> exe_ptrs; std::wstring pname; // Processes started by the *previous* active profile (close them when leaving) std::vector<StartedProc> to_stop_started; // Shell-open targets to launch for the *new* profile activation std::vector<std::wstring> to_launch;;
static void normalize_exe_input(const wchar_t* raw, wchar_t* out, size_t cch_out) { out[0] = 0; if (!raw) return; wchar_t buf[512]; StringCchCopyW(buf, 512, raw); trim_ws_inplace(buf); if (buf[0] == L'"') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'"') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } if (buf[0] == L'\'') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'\'') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } trim_ws_inplace(buf); if (!buf[0]) return; wchar_t exp[1024]; DWORD got = ExpandEnvironmentStringsW(buf, exp, 1024); const wchar_t* s = (got > 0 && got < 1024) ? exp : buf; const wchar_t* base = s; for (const wchar_t* p = s; *p; p++) { if (*p == L'\\' || *p == L'/' || *p == L':') base = p + 1; } wchar_t name[512]; StringCchCopyW(name, 512, base); trim_ws_inplace(name); if (!name[0]) return; if (!ends_with_i(name, L".exe")) { StringCchCatW(name, 512, L".exe"); } // binsearch_nameidx is case-insensitive; no need to lowercase per-process. StringCchCopyW(out, cch_out, name); } static void resolve_exe_launch_path(const wchar_t* raw, wchar_t* out, size_t cch_out) { out[0] = 0; if (!raw) return; wchar_t buf[1024]; StringCchCopyW(buf, 1024, raw); trim_ws_inplace(buf); // Strip simple quotes if (buf[0] == L'"') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'"') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } else if (buf[0] == L'\'') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'\'') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } trim_ws_inplace(buf); if (!buf[0]) return; // Expand environment vars (e.g. %SystemRoot%) wchar_t exp[1024]; DWORD nExp = ExpandEnvironmentStringsW(buf, exp, 1024); if (nExp > 0 && nExp < 1024) { StringCchCopyW(buf, 1024, exp); trim_ws_inplace(buf); } // If input looks like a path, canonicalize + validate. bool looks_path = (wcschr(buf, L'\\') || wcschr(buf, L'/') || wcschr(buf, L':')); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(buf, MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { // Ensure .exe suffix if missing if (!ends_with_i(full, L".exe")) { StringCchCatW(full, MAX_PATH, L".exe"); } if (PathFileExistsW(full)) { StringCchCopyW(out, cch_out, full); return; } } // If it was a path and doesn't exist, fall through to SearchPath. } // Try SearchPath on PATH (also handles names without .exe) wchar_t found[MAX_PATH]; DWORD r = SearchPathW(NULL, buf, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(out, cch_out, found); return; } // Try with ".exe" explicitly if missing. if (!ends_with_i(buf, L".exe")) { wchar_t buf2[1024]; StringCchCopyW(buf2, 1024, buf); StringCchCatW(buf2, 1024, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(out, cch_out, found); return; } } } // ---------------------------- // Profile "Start Items" helpers (shell-open + best-effort close) // ---------------------------- static std::wstring normalize_start_target_best_effort(const std::wstring& raw) { std::wstring s = raw; // Trim whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple surrounding quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // Expand env vars (%TEMP%, etc.) if present if (s.find(L'%') != std::wstring::npos) { wchar_t exp[2048]; DWORD got = ExpandEnvironmentStringsW(s.c_str(), exp, 2048); if (got > 0 && got < 2048) { s.assign(exp); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); } } // If it looks like a filesystem path, canonicalize to full path best-effort. // For URLs / shell targets, leave as-is. // PathIsURLW covers http/https/ftp/mailto, etc. if (!s.empty() && !PathIsURLW(s.c_str())) { bool looks_path = (s.find(L'\\') != std::wstring::npos) || (s.find(L'/') != std::wstring::npos) || (s.find(L':') != std::wstring::npos) || (s.rfind(L".\\", 0) == 0) || (s.rfind(L"..\\", 0) == 0) || (s.rfind(L"./", 0) == 0) || (s.rfind(L"../", 0) == 0); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(s.c_str(), MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { s.assign(full); } } } return s; } static std::wstring normalize_start_target_best_effort(const wchar_t* raw) { if (!raw) return std::wstring(); return normalize_start_target_best_effort(std::wstring(raw)); } static std::wstring reg_read_sz_hkcr(const std::wstring& subkey, const wchar_t* value_name) { wchar_t buf[2048]; DWORD cb = sizeof(buf); DWORD type = 0; LONG r = RegGetValueW(HKEY_CLASSES_ROOT, subkey.c_str(), value_name, RRF_RT_REG_SZ, &type, buf, &cb); if (r != ERROR_SUCCESS) return std::wstring(); // Ensure null-terminated buf[(sizeof(buf)/sizeof(buf[0])) - 1] = 0; return std::wstring(buf); } static void wstr_replace_all(std::wstring& s, const wchar_t* from, const std::wstring& to) { if (!from || !from[0]) return; size_t pos = 0; const size_t nfrom = wcslen(from); while ((pos = s.find(from, pos)) != std::wstring::npos) { s.replace(pos, nfrom, to); pos += to.size(); } } static bool create_process_track(const std::wstring& cmdline, const wchar_t* started_target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (cmdline.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty command line."); return false; } std::vector<wchar_t> cl(cmdline.begin(), cmdline.end()); cl.push_back(0); STARTUPINFOW si{}; si.cb = sizeof(si); si.dwFlags = STARTF_USESHOWWINDOW; si.wShowWindow = SW_SHOWNORMAL; PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW(NULL, cl.data(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi); if (!ok) { DWORD e = GetLastError(); if (out_result && cch) StringCchPrintfW(out_result, cch, L"CreateProcess failed (%lu).", (unsigned long)e); return false; } CloseHandle(pi.hThread); out_sp.started_target = started_target ? started_target : L""; out_sp.pid = pi.dwProcessId; out_sp.hproc.reset(pi.hProcess); if (out_result && cch) StringCchPrintfW(out_result, cch, L"Started (pid %lu).", (unsigned long)out_sp.pid); return true; } static bool start_ahk_via_association_command(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } const wchar_t* ext = PathFindExtensionW(target); if (!ext || _wcsicmp(ext, L".ahk") != 0) return false; // Get ProgID from HKCR\.ahk (default value) std::wstring progid = reg_read_sz_hkcr(L".ahk", NULL); if (progid.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"No .ahk association found (install AutoHotkey or associate .ahk)."); return false; } // Get open command from HKCR\<ProgID>\shell\open\command std::wstring cmdkey = progid + L"\\shell\\open\\command"; std::wstring cmd = reg_read_sz_hkcr(cmdkey, NULL); if (cmd.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Missing association open command for .ahk."); return false; } // Expand env vars in command if (cmd.find(L'%') != std::wstring::npos) { wchar_t exp[4096]; DWORD got = ExpandEnvironmentStringsW(cmd.c_str(), exp, 4096); if (got > 0 && got < 4096) cmd.assign(exp); } std::wstring qtarget = L"\""; qtarget += target; qtarget += L"\""; // Replace common placeholders. Do the quoted variants first to avoid double-quotes. const std::wstring qt = qtarget; wstr_replace_all(cmd, L"\"%1\"", qt); wstr_replace_all(cmd, L"\"%L\"", qt); wstr_replace_all(cmd, L"\"%l\"", qt); wstr_replace_all(cmd, L"%1", qt); wstr_replace_all(cmd, L"%L", qt); wstr_replace_all(cmd, L"%l", qt); // Drop %* (extra args placeholder) wstr_replace_all(cmd, L"%*", L""); // If no placeholder was present, append the script path if (cmd.find(target) == std::wstring::npos && cmd.find(qt) == std::wstring::npos) { cmd.push_back(L' '); cmd += qt; } return create_process_track(cmd, target, out_sp, out_result, cch); } // If the target is an .exe path, derive the lowercased basename (for fallback stop-by-name). static bool shell_open_and_track_process(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } // Special-case .ahk: start via file-association command so we reliably get a process handle (for later profile teardown).;
static std::wstring normalize_start_target_best_effort(const std::wstring& raw) { std::wstring s = raw; // Trim whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple surrounding quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // Expand env vars (%TEMP%, etc.) if present if (s.find(L'%') != std::wstring::npos) { wchar_t exp[2048]; DWORD got = ExpandEnvironmentStringsW(s.c_str(), exp, 2048); if (got > 0 && got < 2048) { s.assign(exp); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); } } // If it looks like a filesystem path, canonicalize to full path best-effort. // For URLs / shell targets, leave as-is. // PathIsURLW covers http/https/ftp/mailto, etc. if (!s.empty() && !PathIsURLW(s.c_str())) { bool looks_path = (s.find(L'\\') != std::wstring::npos) || (s.find(L'/') != std::wstring::npos) || (s.find(L':') != std::wstring::npos) || (s.rfind(L".\\", 0) == 0) || (s.rfind(L"..\\", 0) == 0) || (s.rfind(L"./", 0) == 0) || (s.rfind(L"../", 0) == 0); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(s.c_str(), MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { s.assign(full); } } } return s; } static std::wstring normalize_start_target_best_effort(const wchar_t* raw) { if (!raw) return std::wstring(); return normalize_start_target_best_effort(std::wstring(raw)); } static std::wstring reg_read_sz_hkcr(const std::wstring& subkey, const wchar_t* value_name) { wchar_t buf[2048]; DWORD cb = sizeof(buf); DWORD type = 0; LONG r = RegGetValueW(HKEY_CLASSES_ROOT, subkey.c_str(), value_name, RRF_RT_REG_SZ, &type, buf, &cb); if (r != ERROR_SUCCESS) return std::wstring(); // Ensure null-terminated buf[(sizeof(buf)/sizeof(buf[0])) - 1] = 0; return std::wstring(buf); } static void wstr_replace_all(std::wstring& s, const wchar_t* from, const std::wstring& to) { if (!from || !from[0]) return; size_t pos = 0; const size_t nfrom = wcslen(from); while ((pos = s.find(from, pos)) != std::wstring::npos) { s.replace(pos, nfrom, to); pos += to.size(); } } static bool create_process_track(const std::wstring& cmdline, const wchar_t* started_target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (cmdline.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty command line."); return false; } std::vector<wchar_t> cl(cmdline.begin(), cmdline.end()); cl.push_back(0); STARTUPINFOW si{}; si.cb = sizeof(si); si.dwFlags = STARTF_USESHOWWINDOW; si.wShowWindow = SW_SHOWNORMAL; PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW(NULL, cl.data(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi); if (!ok) { DWORD e = GetLastError(); if (out_result && cch) StringCchPrintfW(out_result, cch, L"CreateProcess failed (%lu).", (unsigned long)e); return false; } CloseHandle(pi.hThread); out_sp.started_target = started_target ? started_target : L""; out_sp.pid = pi.dwProcessId; out_sp.hproc.reset(pi.hProcess); if (out_result && cch) StringCchPrintfW(out_result, cch, L"Started (pid %lu).", (unsigned long)out_sp.pid); return true; } static bool start_ahk_via_association_command(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } const wchar_t* ext = PathFindExtensionW(target); if (!ext || _wcsicmp(ext, L".ahk") != 0) return false; // Get ProgID from HKCR\.ahk (default value) std::wstring progid = reg_read_sz_hkcr(L".ahk", NULL); if (progid.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"No .ahk association found (install AutoHotkey or associate .ahk)."); return false; } // Get open command from HKCR\<ProgID>\shell\open\command std::wstring cmdkey = progid + L"\\shell\\open\\command"; std::wstring cmd = reg_read_sz_hkcr(cmdkey, NULL); if (cmd.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Missing association open command for .ahk."); return false; } // Expand env vars in command if (cmd.find(L'%') != std::wstring::npos) { wchar_t exp[4096]; DWORD got = ExpandEnvironmentStringsW(cmd.c_str(), exp, 4096); if (got > 0 && got < 4096) cmd.assign(exp); } std::wstring qtarget = L"\""; qtarget += target; qtarget += L"\""; // Replace common placeholders. Do the quoted variants first to avoid double-quotes. const std::wstring qt = qtarget; wstr_replace_all(cmd, L"\"%1\"", qt); wstr_replace_all(cmd, L"\"%L\"", qt); wstr_replace_all(cmd, L"\"%l\"", qt); wstr_replace_all(cmd, L"%1", qt); wstr_replace_all(cmd, L"%L", qt); wstr_replace_all(cmd, L"%l", qt); // Drop %* (extra args placeholder) wstr_replace_all(cmd, L"%*", L""); // If no placeholder was present, append the script path if (cmd.find(target) == std::wstring::npos && cmd.find(qt) == std::wstring::npos) { cmd.push_back(L' '); cmd += qt; } return create_process_track(cmd, target, out_sp, out_result, cch); } // If the target is an .exe path, derive the lowercased basename (for fallback stop-by-name). static bool shell_open_and_track_process(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } // Special-case .ahk: start via file-association command so we reliably get a process handle (for later profile teardown).;
static void resolve_exe_launch_path(const wchar_t* raw, wchar_t* out, size_t cch_out) { out[0] = 0; if (!raw) return; wchar_t buf[1024]; StringCchCopyW(buf, 1024, raw); trim_ws_inplace(buf); // Strip simple quotes if (buf[0] == L'"') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'"') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } else if (buf[0] == L'\'') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'\'') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } trim_ws_inplace(buf); if (!buf[0]) return; // Expand environment vars (e.g. %SystemRoot%) wchar_t exp[1024]; DWORD nExp = ExpandEnvironmentStringsW(buf, exp, 1024); if (nExp > 0 && nExp < 1024) { StringCchCopyW(buf, 1024, exp); trim_ws_inplace(buf); } // If input looks like a path, canonicalize + validate. bool looks_path = (wcschr(buf, L'\\') || wcschr(buf, L'/') || wcschr(buf, L':')); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(buf, MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { // Ensure .exe suffix if missing if (!ends_with_i(full, L".exe")) { StringCchCatW(full, MAX_PATH, L".exe"); } if (PathFileExistsW(full)) { StringCchCopyW(out, cch_out, full); return; } } // If it was a path and doesn't exist, fall through to SearchPath. } // Try SearchPath on PATH (also handles names without .exe) wchar_t found[MAX_PATH]; DWORD r = SearchPathW(NULL, buf, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(out, cch_out, found); return; } // Try with ".exe" explicitly if missing. if (!ends_with_i(buf, L".exe")) { wchar_t buf2[1024]; StringCchCopyW(buf2, 1024, buf); StringCchCatW(buf2, 1024, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(out, cch_out, found); return; } } } // ---------------------------- // Profile "Start Items" helpers (shell-open + best-effort close) // ---------------------------- static std::wstring normalize_start_target_best_effort(const std::wstring& raw) { std::wstring s = raw; // Trim whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple surrounding quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // Expand env vars (%TEMP%, etc.) if present if (s.find(L'%') != std::wstring::npos) { wchar_t exp[2048]; DWORD got = ExpandEnvironmentStringsW(s.c_str(), exp, 2048); if (got > 0 && got < 2048) { s.assign(exp); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); } } // If it looks like a filesystem path, canonicalize to full path best-effort. // For URLs / shell targets, leave as-is. // PathIsURLW covers http/https/ftp/mailto, etc. if (!s.empty() && !PathIsURLW(s.c_str())) { bool looks_path = (s.find(L'\\') != std::wstring::npos) || (s.find(L'/') != std::wstring::npos) || (s.find(L':') != std::wstring::npos) || (s.rfind(L".\\", 0) == 0) || (s.rfind(L"..\\", 0) == 0) || (s.rfind(L"./", 0) == 0) || (s.rfind(L"../", 0) == 0); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(s.c_str(), MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { s.assign(full); } } } return s; } static std::wstring normalize_start_target_best_effort(const wchar_t* raw) { if (!raw) return std::wstring(); return normalize_start_target_best_effort(std::wstring(raw)); } static std::wstring reg_read_sz_hkcr(const std::wstring& subkey, const wchar_t* value_name) { wchar_t buf[2048]; DWORD cb = sizeof(buf); DWORD type = 0; LONG r = RegGetValueW(HKEY_CLASSES_ROOT, subkey.c_str(), value_name, RRF_RT_REG_SZ, &type, buf, &cb); if (r != ERROR_SUCCESS) return std::wstring(); // Ensure null-terminated buf[(sizeof(buf)/sizeof(buf[0])) - 1] = 0; return std::wstring(buf); } static void wstr_replace_all(std::wstring& s, const wchar_t* from, const std::wstring& to) { if (!from || !from[0]) return; size_t pos = 0; const size_t nfrom = wcslen(from); while ((pos = s.find(from, pos)) != std::wstring::npos) { s.replace(pos, nfrom, to); pos += to.size(); } } static bool create_process_track(const std::wstring& cmdline, const wchar_t* started_target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (cmdline.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty command line."); return false; } std::vector<wchar_t> cl(cmdline.begin(), cmdline.end()); cl.push_back(0); STARTUPINFOW si{}; si.cb = sizeof(si); si.dwFlags = STARTF_USESHOWWINDOW; si.wShowWindow = SW_SHOWNORMAL; PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW(NULL, cl.data(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi); if (!ok) { DWORD e = GetLastError(); if (out_result && cch) StringCchPrintfW(out_result, cch, L"CreateProcess failed (%lu).", (unsigned long)e); return false; } CloseHandle(pi.hThread); out_sp.started_target = started_target ? started_target : L""; out_sp.pid = pi.dwProcessId; out_sp.hproc.reset(pi.hProcess); if (out_result && cch) StringCchPrintfW(out_result, cch, L"Started (pid %lu).", (unsigned long)out_sp.pid); return true; } static bool start_ahk_via_association_command(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } const wchar_t* ext = PathFindExtensionW(target); if (!ext || _wcsicmp(ext, L".ahk") != 0) return false; // Get ProgID from HKCR\.ahk (default value) std::wstring progid = reg_read_sz_hkcr(L".ahk", NULL); if (progid.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"No .ahk association found (install AutoHotkey or associate .ahk)."); return false; } // Get open command from HKCR\<ProgID>\shell\open\command std::wstring cmdkey = progid + L"\\shell\\open\\command"; std::wstring cmd = reg_read_sz_hkcr(cmdkey, NULL); if (cmd.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Missing association open command for .ahk."); return false; } // Expand env vars in command if (cmd.find(L'%') != std::wstring::npos) { wchar_t exp[4096]; DWORD got = ExpandEnvironmentStringsW(cmd.c_str(), exp, 4096); if (got > 0 && got < 4096) cmd.assign(exp); } std::wstring qtarget = L"\""; qtarget += target; qtarget += L"\""; // Replace common placeholders. Do the quoted variants first to avoid double-quotes. const std::wstring qt = qtarget; wstr_replace_all(cmd, L"\"%1\"", qt); wstr_replace_all(cmd, L"\"%L\"", qt); wstr_replace_all(cmd, L"\"%l\"", qt); wstr_replace_all(cmd, L"%1", qt); wstr_replace_all(cmd, L"%L", qt); wstr_replace_all(cmd, L"%l", qt); // Drop %* (extra args placeholder) wstr_replace_all(cmd, L"%*", L""); // If no placeholder was present, append the script path if (cmd.find(target) == std::wstring::npos && cmd.find(qt) == std::wstring::npos) { cmd.push_back(L' '); cmd += qt; } return create_process_track(cmd, target, out_sp, out_result, cch); } // If the target is an .exe path, derive the lowercased basename (for fallback stop-by-name). static bool shell_open_and_track_process(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } // Special-case .ahk: start via file-association command so we reliably get a process handle (for later profile teardown).;
static std::wstring exe_watch_key_lower(const std::wstring& raw) { std::wstring s = raw; // Trim simple whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // If a path was provided, use the basename to match Toolhelp's szExeFile. size_t slash = s.find_last_of(L"\\/"); if (slash != std::wstring::npos) s = s.substr(slash + 1); // Lower-case (in place) for stable matching if (!s.empty()) CharLowerBuffW(s.data(), (DWORD)s.size()); return s; } // Fast, allocation-free hash of a case-insensitive exe basename (typically already basename). // 64-bit FNV-1a over wchar_t code units (Windows: UTF-16). static inline uint64_t fnv1a64_exe_lower_hash(const wchar_t* s) { constexpr uint64_t FNV_OFFSET = 14695981039346656037ull; constexpr uint64_t FNV_PRIME = 1099511628211ull; uint64_t h = FNV_OFFSET; if (!s) return h; for (const wchar_t* p = s; *p; ++p) { // Fold to lower without allocating; towlower is fine for ASCII exe names. wchar_t c = (wchar_t)towlower(*p); h ^= (uint16_t)c; h *= FNV_PRIME; } return h; } // Build sorted+unique running exe hashes once per tick (no heap per-process string allocations). static bool build_running_exe_hashes_lower(std::vector<uint64_t>& out_hashes) { out_hashes.clear(); unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe{}; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { if (pe.szExeFile[0]) { out_hashes.push_back(fnv1a64_exe_lower_hash(pe.szExeFile)); } } while (Process32NextW(snap.get(), &pe)); std::sort(out_hashes.begin(), out_hashes.end()); out_hashes.erase(std::unique(out_hashes.begin(), out_hashes.end()), out_hashes.end()); return true; } static bool terminate_all_by_name_lower(const wchar_t* exe_lower, int* terminated, int* failed) { if (terminated) *terminated = 0; if (failed) *failed = 0; unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { wchar_t tmp[MAX_PATH]; StringCchCopyW(tmp, ARRAYSIZE(tmp), pe.szExeFile); CharLowerBuffW(tmp, (DWORD)wcslen(tmp)); if (wcscmp(tmp, exe_lower) == 0) { unique_handle h(OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, pe.th32ProcessID)); if (!h) { if (failed) (*failed)++; continue; } BOOL ok = TerminateProcess(h.get(), 1); if (ok) { if (terminated) (*terminated)++; } else { if (failed) (*failed)++; } } } while (Process32NextW(snap.get(), &pe)); return true; } // ---------------------------- // Win32 error formatting // ---------------------------- static void format_winerr(DWORD e, wchar_t* out, size_t cch) { if (!out || cch == 0) return; out[0] = 0; wchar_t sys[256]; sys[0] = 0; FormatMessageW( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, e, 0, sys, 256, NULL ); trim_ws_inplace(sys); if (sys[0]) { StringCchPrintfW(out, cch, L"winerror %lu: %s", e, sys); } else { StringCchPrintfW(out, cch, L"winerror %lu", e); } } // ---------------------------- // Service helpers // ---------------------------- static bool query_service_status_fast(const wchar_t* svc_name, wchar_t* out_status, size_t cch) { StringCchCopyW(out_status, cch, L"unknown"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) return false; unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_QUERY_STATUS)); if (!h) return false; SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; BOOL ok = QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes); if (!ok) return false; StringCchCopyW(out_status, cch, svc_state_to_str(ssp.dwCurrentState)); return true; } static bool stop_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp0{}; DWORD bytes0 = 0; if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp0, sizeof(ssp0), &bytes0)) { if (ssp0.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } } SERVICE_STATUS st{}; BOOL ok = ControlService(h.get(), SERVICE_CONTROL_STOP, &st); if (!ok) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_NOT_ACTIVE) { StringCchCopyW(out_result, cch, L"stopped"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: stop refused (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"stop requested"); return true; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"stop requested (status query failed: %s)", ebuf); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, L"stop requested (still stopping or refused)"); return true; } static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static bool query_service_status_fast(const wchar_t* svc_name, wchar_t* out_status, size_t cch) { StringCchCopyW(out_status, cch, L"unknown"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) return false; unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_QUERY_STATUS)); if (!h) return false; SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; BOOL ok = QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes); if (!ok) return false; StringCchCopyW(out_status, cch, svc_state_to_str(ssp.dwCurrentState)); return true; } static bool stop_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp0{}; DWORD bytes0 = 0; if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp0, sizeof(ssp0), &bytes0)) { if (ssp0.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } } SERVICE_STATUS st{}; BOOL ok = ControlService(h.get(), SERVICE_CONTROL_STOP, &st); if (!ok) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_NOT_ACTIVE) { StringCchCopyW(out_result, cch, L"stopped"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: stop refused (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"stop requested"); return true; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"stop requested (status query failed: %s)", ebuf); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, L"stop requested (still stopping or refused)"); return true; } static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static int process_count_by_name_lower(const wchar_t* exe_lower) { if (!exe_lower || !exe_lower[0]) return 0; HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0); if (snap == INVALID_HANDLE_VALUE) return 0; PROCESSENTRY32W pe; pe.dwSize = sizeof(pe); int count = 0; if (Process32FirstW(snap, &pe)) { do { wchar_t name[260]; StringCchCopyW(name, 260, pe.szExeFile); // binsearch_nameidx is case-insensitive; no need to lowercase per-process. if (_wcsicmp(name, exe_lower) == 0) count++; } while (Process32NextW(snap, &pe)); } CloseHandle(snap); return count; } // Build a set of running process exe names (lowercased) from ONE Toolhelp snapshot. static std::wstring exe_watch_key_lower(const std::wstring& raw) { std::wstring s = raw; // Trim simple whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // If a path was provided, use the basename to match Toolhelp's szExeFile. size_t slash = s.find_last_of(L"\\/"); if (slash != std::wstring::npos) s = s.substr(slash + 1); // Lower-case (in place) for stable matching if (!s.empty()) CharLowerBuffW(s.data(), (DWORD)s.size()); return s; } // Fast, allocation-free hash of a case-insensitive exe basename (typically already basename). // 64-bit FNV-1a over wchar_t code units (Windows: UTF-16). static inline uint64_t fnv1a64_exe_lower_hash(const wchar_t* s) { constexpr uint64_t FNV_OFFSET = 14695981039346656037ull; constexpr uint64_t FNV_PRIME = 1099511628211ull; uint64_t h = FNV_OFFSET; if (!s) return h; for (const wchar_t* p = s; *p; ++p) { // Fold to lower without allocating; towlower is fine for ASCII exe names. wchar_t c = (wchar_t)towlower(*p); h ^= (uint16_t)c; h *= FNV_PRIME; } return h; } // Build sorted+unique running exe hashes once per tick (no heap per-process string allocations). static bool build_running_exe_hashes_lower(std::vector<uint64_t>& out_hashes) { out_hashes.clear(); unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe{}; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { if (pe.szExeFile[0]) { out_hashes.push_back(fnv1a64_exe_lower_hash(pe.szExeFile)); } } while (Process32NextW(snap.get(), &pe)); std::sort(out_hashes.begin(), out_hashes.end()); out_hashes.erase(std::unique(out_hashes.begin(), out_hashes.end()), out_hashes.end()); return true; } static bool terminate_all_by_name_lower(const wchar_t* exe_lower, int* terminated, int* failed) { if (terminated) *terminated = 0; if (failed) *failed = 0; unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { wchar_t tmp[MAX_PATH]; StringCchCopyW(tmp, ARRAYSIZE(tmp), pe.szExeFile); CharLowerBuffW(tmp, (DWORD)wcslen(tmp)); if (wcscmp(tmp, exe_lower) == 0) { unique_handle h(OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, pe.th32ProcessID)); if (!h) { if (failed) (*failed)++; continue; } BOOL ok = TerminateProcess(h.get(), 1); if (ok) { if (terminated) (*terminated)++; } else { if (failed) (*failed)++; } } } while (Process32NextW(snap.get(), &pe)); return true; } // ---------------------------- // Win32 error formatting // ---------------------------- static void format_winerr(DWORD e, wchar_t* out, size_t cch) { if (!out || cch == 0) return; out[0] = 0; wchar_t sys[256]; sys[0] = 0; FormatMessageW( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, e, 0, sys, 256, NULL ); trim_ws_inplace(sys); if (sys[0]) { StringCchPrintfW(out, cch, L"winerror %lu: %s", e, sys); } else { StringCchPrintfW(out, cch, L"winerror %lu", e); } } // ---------------------------- // Service helpers // ---------------------------- static bool query_service_status_fast(const wchar_t* svc_name, wchar_t* out_status, size_t cch) { StringCchCopyW(out_status, cch, L"unknown"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) return false; unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_QUERY_STATUS)); if (!h) return false; SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; BOOL ok = QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes); if (!ok) return false; StringCchCopyW(out_status, cch, svc_state_to_str(ssp.dwCurrentState)); return true; } static bool stop_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp0{}; DWORD bytes0 = 0; if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp0, sizeof(ssp0), &bytes0)) { if (ssp0.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } } SERVICE_STATUS st{}; BOOL ok = ControlService(h.get(), SERVICE_CONTROL_STOP, &st); if (!ok) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_NOT_ACTIVE) { StringCchCopyW(out_result, cch, L"stopped"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: stop refused (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"stop requested"); return true; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"stop requested (status query failed: %s)", ebuf); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, L"stop requested (still stopping or refused)"); return true; } static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static bool build_running_exe_hashes_lower(std::vector<uint64_t>& out_hashes); static void rebuild_profile_watch_keys_locked(App& self) { auto nv = std::make_shared<std::vector<std::vector<std::wstring>>>(); auto hv = std::make_shared<std::vector<std::vector<uint64_t>>>(); nv->reserve(self.profiles.size()); hv->reserve(self.profiles.size()); for (auto& p : self.profiles) { // Keep the cached keys in sync. watch_exes are already normalized lower-case basenames. if (p.watch_keys_lower.empty() && !p.watch_exes.empty()) { p.watch_keys_lower = p.watch_exes; } else if (p.watch_keys_lower.size() != p.watch_exes.size()) { // Best-effort resync; avoids stale cache if older configs/UI paths changed. p.watch_keys_lower = p.watch_exes; } nv->push_back(p.watch_keys_lower); // Also cache hashes for allocation-free matching in the monitor thread. std::vector<uint64_t> hs; hs.reserve(p.watch_keys_lower.size()); for (const auto& k : p.watch_keys_lower) { if (k.empty()) continue; hs.push_back(fnv1a64_exe_lower_hash(k.c_str())); // already lower-case } std::sort(hs.begin(), hs.end()); hs.erase(std::unique(hs.begin(), hs.end()), hs.end()); hv->push_back(std::move(hs)); } self.profile_watch_keys_sp = std::move(nv); self.profile_watch_hashes_sp = std::move(hv); } // -------------------------------------------------- // Final-form structure note // - We removed the global App* and the global app() accessor. // - Legacy code still uses self.field; we map app() -> `self` via a macro. // Any function that uses app() must have an `App& self` in scope. // - The main window proc is now instance-bound via GWLP_USERDATA. // -------------------------------------------------- #define app() (self) // ---------------------------- // Dark mode / theme helpers // ---------------------------- // ---------------------------- // Dark mode opt-in (UxTheme ordinals) // ---------------------------- struct DarkModeApi { HMODULE ux = NULL; // Ordinals commonly used (undocumented; best-effort) using PFN_SetPreferredAppMode = int (WINAPI*)(int); using PFN_FlushMenuThemes = void (WINAPI*)(); using PFN_AllowDarkModeForWindow = BOOL (WINAPI*)(HWND, BOOL); PFN_SetPreferredAppMode SetPreferredAppMode = nullptr; PFN_FlushMenuThemes FlushMenuThemes = nullptr; PFN_AllowDarkModeForWindow AllowDarkModeForWindow = nullptr; bool inited = false; }; static DarkModeApi g_dm; template<typename T> static T dm_get_ordinal(HMODULE m, int ordinal) { if (!m) return nullptr; FARPROC fp = GetProcAddress(m, MAKEINTRESOURCEA(ordinal)); if (!fp) return nullptr; // Avoid -Wcast-function-type on GCC/Clang by casting via integer. return (T)(uintptr_t)fp; } static void darkmode_init_api() { if (g_dm.inited) return; g_dm.inited = true; g_dm.ux = LoadLibraryW(L"uxtheme.dll"); if (!g_dm.ux) return; // These are ordinals used by many Win32 apps. If they don't exist, we simply no-op. g_dm.SetPreferredAppMode = dm_get_ordinal<DarkModeApi::PFN_SetPreferredAppMode>(g_dm.ux, 135); g_dm.FlushMenuThemes = dm_get_ordinal<DarkModeApi::PFN_FlushMenuThemes>(g_dm.ux, 136); g_dm.AllowDarkModeForWindow = dm_get_ordinal<DarkModeApi::PFN_AllowDarkModeForWindow>(g_dm.ux, 133); } static void darkmode_set_app(bool want_dark) { darkmode_init_api(); if (!g_dm.SetPreferredAppMode) return; // PreferredAppMode: Default=0, AllowDark=1, ForceDark=2, ForceLight=3 g_dm.SetPreferredAppMode(want_dark ? 1 : 0); if (g_dm.FlushMenuThemes) g_dm.FlushMenuThemes(); } static void darkmode_allow_window(HWND hwnd, bool allow_dark) { if (!hwnd) return; darkmode_init_api(); if (g_dm.AllowDarkModeForWindow) { g_dm.AllowDarkModeForWindow(hwnd, allow_dark ? TRUE : FALSE); } } static bool try_set_explorer_theme_mode(HWND hwnd, bool dark) { if (!hwnd) return false; HMODULE ux = LoadLibraryW(L"uxtheme.dll"); if (!ux) return false; typedef HRESULT (WINAPI *PFN_SetWindowTheme)(HWND, LPCWSTR, LPCWSTR); PFN_SetWindowTheme p = nullptr; LOAD_PROC(ux, "SetWindowTheme", PFN_SetWindowTheme, p); bool ok = false; if (p) { // "DarkMode_Explorer" is honored by common controls on Win10/11; harmless if ignored. p(hwnd, dark ? L"DarkMode_Explorer" : L"Explorer", nullptr); ok = true; } FreeLibrary(ux); return ok; } // Custom-draw helpers for controls that don't fully honor DarkMode_Explorer on all builds. // Custom-draw for listviews (items) so text/bk colors are correct in dark mode even when the control ignores LVM_* colors. // Dark-mode push buttons: subclass paint so we don't rely on BS_OWNERDRAW + parent WM_DRAWITEM. // This is more reliable because many buttons live inside container child windows (tab pages/panels), // and WM_DRAWITEM would go to those parents, not the main window. static LRESULT CALLBACK DarkButtonSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR /*idSubclass*/, DWORD_PTR refData) { App* self = reinterpret_cast<App*>(refData); switch (msg) { case WM_ERASEBKGND: if (self && self->dark_mode) return 1; break; case WM_ENABLE: case WM_THEMECHANGED: case WM_STYLECHANGED: InvalidateRect(hwnd, NULL, TRUE); break; case WM_PAINT: { if (!self || !self->dark_mode) break; PAINTSTRUCT ps{}; HDC hdc = BeginPaint(hwnd, &ps); RECT rc{}; GetClientRect(hwnd, &rc); const bool disabled = !IsWindowEnabled(hwnd); const LRESULT st = SendMessageW(hwnd, BM_GETSTATE, 0, 0); const bool pressed = (st & BST_PUSHED) != 0; COLORREF bg = pressed ? RGB(55,55,55) : self->col_panel; HBRUSH br = CreateSolidBrush(bg); FillRect(hdc, &rc, br); DeleteObject(br); // Border HPEN pen = CreatePen(PS_SOLID, 1, RGB(70,70,70)); HGDIOBJ oldp = SelectObject(hdc, pen); HGDIOBJ oldb = SelectObject(hdc, GetStockObject(NULL_BRUSH)); Rectangle(hdc, rc.left, rc.top, rc.right, rc.bottom); SelectObject(hdc, oldb); SelectObject(hdc, oldp); DeleteObject(pen); // Text wchar_t txt[256]{0}; GetWindowTextW(hwnd, txt, 255); SetBkMode(hdc, TRANSPARENT); SetTextColor(hdc, disabled ? self->col_text_dim : self->col_text); // Ensure we draw with the button's assigned font (WM_SETFONT), otherwise GDI may use a default font // during subclass painting, causing mismatched text size between light/dark mode. HFONT hfont = (HFONT)SendMessageW(hwnd, WM_GETFONT, 0, 0); HGDIOBJ oldf = NULL; if (hfont) oldf = SelectObject(hdc, hfont); RECT tr = rc; if (pressed) { tr.left += 1; tr.top += 1; } DrawTextW(hdc, txt, -1, &tr, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS); if (oldf) SelectObject(hdc, oldf); // Focus rect if (GetFocus() == hwnd && !disabled) { RECT fr = rc; InflateRect(&fr, -3, -3); DrawFocusRect(hdc, &fr); } EndPaint(hwnd, &ps); return 0; } case WM_NCDESTROY: RemoveWindowSubclass(hwnd, DarkButtonSubclassProc, 1); break; } return DefSubclassProc(hwnd, msg, wParam, lParam); } // Apply theming (including dark mode opt-in and owner-draw toggles) to a single control. static BOOL CALLBACK enum_theme_children_proc(HWND child, LPARAM lp) { App* self = reinterpret_cast<App*>(lp); if (!self) return TRUE; theme_apply_to_control(*self, child); EnumChildWindows(child, enum_theme_children_proc, lp); return TRUE; } static bool try_enable_dark_titlebar(HWND hwnd, bool enable); static BOOL CALLBACK enum_theme_thread_windows_proc(HWND hwnd, LPARAM lp) { App* self = reinterpret_cast<App*>(lp); if (!self) return TRUE; theme_apply_to_window(*self, hwnd); return TRUE; } // ---------------------------- // Theme + per-window DPI font cache helpers // ---------------------------- static void theme_compute(App& self); // Back-compat wrapper (older call sites). Prefer theme_compute()+theme_apply_all_controls(). // ---------------------------- // Tray icon helpers // ---------------------------- static HICON load_app_icon(HINSTANCE hInst, int cx, int cy) { #ifdef IDI_APPICON return (HICON)LoadImageW(hInst, MAKEINTRESOURCEW(IDI_APPICON), IMAGE_ICON, cx, cy, 0); #else (void)hInst; (void)cx; (void)cy; return LoadIconW(NULL, IDI_APPLICATION); #endif } // Forward decls for event-driven UI maintenance (definitions later) static void ui_timer_suspend(App& self, HWND hwnd); static void ui_timer_resume(App& self, HWND hwnd); static void restart_ui_timer(App& self, HWND hwnd); static void post_model_dirty(App& self); static void show_main_window(App& self) { HWND hwnd = self.hwnd; if (!hwnd) return; // Ensure visible (close-to-tray uses SW_HIDE) ShowWindow(hwnd, SW_SHOW); // When restoring from tray/hidden, resume UI maintenance. ui_timer_resume(self, hwnd); // If minimized, restore. If not, bring to front. if (IsIconic(hwnd)) { ShowWindow(hwnd, SW_RESTORE); } else { ShowWindow(hwnd, SW_SHOWNORMAL); } // Bring to front (foreground restrictions can otherwise make it look like "nothing happened") SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE); SetWindowPos(hwnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE); SetForegroundWindow(hwnd); SetActiveWindow(hwnd); SetFocus(hwnd); restart_ui_timer(self, hwnd); post_model_dirty(self); } static void hide_main_window(App& self) { if (!self.hwnd) return; ui_timer_suspend(self, self.hwnd); ShowWindow(self.hwnd, SW_HIDE); } static void app_request_exit(App& self, HWND hwnd) { self.force_quit = true; DestroyWindow(hwnd); } // ---------------------------- // FIX: Page container window class that forwards WM_COMMAND / WM_NOTIFY // ---------------------------- static const wchar_t* PAGE_CLASS = L"PageContainer_C"; static LRESULT CALLBACK PageProc(HWND page, UINT msg, WPARAM wParam, LPARAM lParam) { HWND parent = GetParent(page); if (msg == WM_COMMAND || msg == WM_NOTIFY || msg == WM_CTLCOLORSTATIC || msg == WM_CTLCOLOREDIT || msg == WM_CTLCOLORBTN || msg == WM_CTLCOLORLISTBOX) { if (parent) return SendMessageW(parent, msg, wParam, lParam); return 0; } if (msg == WM_ERASEBKGND) { HDC hdc = (HDC)wParam; if (hdc) { RECT rc; GetClientRect(page, &rc); App* self = parent ? reinterpret_cast<App*>(GetWindowLongPtrW(parent, GWLP_USERDATA)) : nullptr; HBRUSH br = self ? self->br_bg : GetSysColorBrush(COLOR_BTNFACE); FillRect(hdc, &rc, br); } return 1; } return DefWindowProcW(page, msg, wParam, lParam); } static void register_page_class(void) { static bool registered = false; if (registered) return; registered = true; WNDCLASSW wc; memset(&wc, 0, sizeof(wc)); wc.lpfnWndProc = PageProc; wc.hInstance = GetModuleHandleW(NULL); wc.lpszClassName = PAGE_CLASS; wc.hbrBackground = (HBRUSH)(COLOR_BTNFACE + 1); wc.hCursor = LoadCursor(NULL, IDC_ARROW); RegisterClassW(&wc); } // ---------------------------- // UI logging (via message to main thread) // ---------------------------- typedef struct LogMsg { wchar_t* text; } LogMsg; static void post_log(App& self, const wchar_t* text) { LogMsg* m = (LogMsg*)malloc(sizeof(LogMsg)); if (!m) return; m->text = wcsdup_heap(text ? text : L""); if (!m->text) { free(m); return; } if (!PostMessageW(self.hwnd, WM_APP_LOG, 0, (LPARAM)m)) { free(m->text); free(m); } } typedef struct StatusMsg { int kind; wchar_t* name; wchar_t status[32]; time_t wall_ts; } StatusMsg; // Bulk status update (generation-only, no heap churn): // - Monitor thread updates Item::last_status/last_update_wall under lock. // - UI thread repaints only rows that changed (via WM_APP_STATUS_GEN / UI timer). static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], size_t n, time_t wall_ts) { if (!names || !statuses || n == 0) return; bool any_changed = false;;
static bool terminate_all_by_name_lower(const wchar_t* exe_lower, int* terminated, int* failed) { if (terminated) *terminated = 0; if (failed) *failed = 0; unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { wchar_t tmp[MAX_PATH]; StringCchCopyW(tmp, ARRAYSIZE(tmp), pe.szExeFile); CharLowerBuffW(tmp, (DWORD)wcslen(tmp)); if (wcscmp(tmp, exe_lower) == 0) { unique_handle h(OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, pe.th32ProcessID)); if (!h) { if (failed) (*failed)++; continue; } BOOL ok = TerminateProcess(h.get(), 1); if (ok) { if (terminated) (*terminated)++; } else { if (failed) (*failed)++; } } } while (Process32NextW(snap.get(), &pe)); return true; } // ---------------------------- // Win32 error formatting // ---------------------------- static void format_winerr(DWORD e, wchar_t* out, size_t cch) { if (!out || cch == 0) return; out[0] = 0; wchar_t sys[256]; sys[0] = 0; FormatMessageW( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, e, 0, sys, 256, NULL ); trim_ws_inplace(sys); if (sys[0]) { StringCchPrintfW(out, cch, L"winerror %lu: %s", e, sys); } else { StringCchPrintfW(out, cch, L"winerror %lu", e); } } // ---------------------------- // Service helpers // ---------------------------- static bool query_service_status_fast(const wchar_t* svc_name, wchar_t* out_status, size_t cch) { StringCchCopyW(out_status, cch, L"unknown"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) return false; unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_QUERY_STATUS)); if (!h) return false; SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; BOOL ok = QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes); if (!ok) return false; StringCchCopyW(out_status, cch, svc_state_to_str(ssp.dwCurrentState)); return true; } static bool stop_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp0{}; DWORD bytes0 = 0; if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp0, sizeof(ssp0), &bytes0)) { if (ssp0.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } } SERVICE_STATUS st{}; BOOL ok = ControlService(h.get(), SERVICE_CONTROL_STOP, &st); if (!ok) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_NOT_ACTIVE) { StringCchCopyW(out_result, cch, L"stopped"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: stop refused (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"stop requested"); return true; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"stop requested (status query failed: %s)", ebuf); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, L"stop requested (still stopping or refused)"); return true; } static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static bool shell_open_and_track_process(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } // Special-case .ahk: start via file-association command so we reliably get a process handle (for later profile teardown).;
static void close_process_best_effort(DWORD pid, HANDLE hproc_in, int wait_ms, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; unique_handle hproc; if (hproc_in && hproc_in != INVALID_HANDLE_VALUE) { // Borrow handle: do NOT close (caller owns); duplicate so we can safely manage lifetime. HANDLE dup = NULL; DuplicateHandle(GetCurrentProcess(), hproc_in, GetCurrentProcess(), &dup, SYNCHRONIZE | PROCESS_TERMINATE | PROCESS_QUERY_LIMITED_INFORMATION, FALSE, 0); if (dup) hproc.reset(dup); } if (!hproc && pid) { HANDLE hp = OpenProcess(SYNCHRONIZE | PROCESS_TERMINATE | PROCESS_QUERY_LIMITED_INFORMATION, FALSE, pid); if (hp) hproc.reset(hp); } if (!hproc) { if (out_result && cch) { if (pid) StringCchPrintfW(out_result, cch, L"Cannot open process (pid %lu).", (unsigned long)pid); else StringCchCopyW(out_result, cch, L"No process to close."); } return; } // If already exited, we're done. DWORD code = STILL_ACTIVE; if (GetExitCodeProcess(hproc.get(), &code) && code != STILL_ACTIVE) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Already exited."); return; } // Ask nicely: WM_CLOSE to top-level windows for that PID. if (!pid) pid = GetProcessId(hproc.get()); if (pid) { EnumCloseWindowsData d{ pid }; EnumWindows(enum_close_windows_cb, (LPARAM)&d); } // Wait for graceful exit. int ms = wait_ms; if (ms < 0) ms = 0; DWORD wr = WaitForSingleObject(hproc.get(), (DWORD)ms); if (wr == WAIT_OBJECT_0) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Closed."); return; } // Still running: force terminate. TerminateProcess(hproc.get(), 1); WaitForSingleObject(hproc.get(), 2000); if (out_result && cch) StringCchCopyW(out_result, cch, L"Terminated."); } static int process_count_by_name_lower(const wchar_t* exe_lower) { if (!exe_lower || !exe_lower[0]) return 0; HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0); if (snap == INVALID_HANDLE_VALUE) return 0; PROCESSENTRY32W pe; pe.dwSize = sizeof(pe); int count = 0; if (Process32FirstW(snap, &pe)) { do { wchar_t name[260]; StringCchCopyW(name, 260, pe.szExeFile); // binsearch_nameidx is case-insensitive; no need to lowercase per-process. if (_wcsicmp(name, exe_lower) == 0) count++; } while (Process32NextW(snap, &pe)); } CloseHandle(snap); return count; } // Build a set of running process exe names (lowercased) from ONE Toolhelp snapshot. static std::wstring exe_watch_key_lower(const std::wstring& raw) { std::wstring s = raw; // Trim simple whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // If a path was provided, use the basename to match Toolhelp's szExeFile. size_t slash = s.find_last_of(L"\\/"); if (slash != std::wstring::npos) s = s.substr(slash + 1); // Lower-case (in place) for stable matching if (!s.empty()) CharLowerBuffW(s.data(), (DWORD)s.size()); return s; } // Fast, allocation-free hash of a case-insensitive exe basename (typically already basename). // 64-bit FNV-1a over wchar_t code units (Windows: UTF-16). static inline uint64_t fnv1a64_exe_lower_hash(const wchar_t* s) { constexpr uint64_t FNV_OFFSET = 14695981039346656037ull; constexpr uint64_t FNV_PRIME = 1099511628211ull; uint64_t h = FNV_OFFSET; if (!s) return h; for (const wchar_t* p = s; *p; ++p) { // Fold to lower without allocating; towlower is fine for ASCII exe names. wchar_t c = (wchar_t)towlower(*p); h ^= (uint16_t)c; h *= FNV_PRIME; } return h; } // Build sorted+unique running exe hashes once per tick (no heap per-process string allocations). static bool build_running_exe_hashes_lower(std::vector<uint64_t>& out_hashes) { out_hashes.clear(); unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe{}; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { if (pe.szExeFile[0]) { out_hashes.push_back(fnv1a64_exe_lower_hash(pe.szExeFile)); } } while (Process32NextW(snap.get(), &pe)); std::sort(out_hashes.begin(), out_hashes.end()); out_hashes.erase(std::unique(out_hashes.begin(), out_hashes.end()), out_hashes.end()); return true; } static bool terminate_all_by_name_lower(const wchar_t* exe_lower, int* terminated, int* failed) { if (terminated) *terminated = 0; if (failed) *failed = 0; unique_handle snap(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)); if (!snap) return false; PROCESSENTRY32W pe; pe.dwSize = sizeof(pe); if (!Process32FirstW(snap.get(), &pe)) return false; do { wchar_t tmp[MAX_PATH]; StringCchCopyW(tmp, ARRAYSIZE(tmp), pe.szExeFile); CharLowerBuffW(tmp, (DWORD)wcslen(tmp)); if (wcscmp(tmp, exe_lower) == 0) { unique_handle h(OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, pe.th32ProcessID)); if (!h) { if (failed) (*failed)++; continue; } BOOL ok = TerminateProcess(h.get(), 1); if (ok) { if (terminated) (*terminated)++; } else { if (failed) (*failed)++; } } } while (Process32NextW(snap.get(), &pe)); return true; } // ---------------------------- // Win32 error formatting // ---------------------------- static void format_winerr(DWORD e, wchar_t* out, size_t cch) { if (!out || cch == 0) return; out[0] = 0; wchar_t sys[256]; sys[0] = 0; FormatMessageW( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, e, 0, sys, 256, NULL ); trim_ws_inplace(sys); if (sys[0]) { StringCchPrintfW(out, cch, L"winerror %lu: %s", e, sys); } else { StringCchPrintfW(out, cch, L"winerror %lu", e); } } // ---------------------------- // Service helpers // ---------------------------- static bool query_service_status_fast(const wchar_t* svc_name, wchar_t* out_status, size_t cch) { StringCchCopyW(out_status, cch, L"unknown"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) return false; unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_QUERY_STATUS)); if (!h) return false; SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; BOOL ok = QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes); if (!ok) return false; StringCchCopyW(out_status, cch, svc_state_to_str(ssp.dwCurrentState)); return true; } static bool stop_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp0{}; DWORD bytes0 = 0; if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp0, sizeof(ssp0), &bytes0)) { if (ssp0.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } } SERVICE_STATUS st{}; BOOL ok = ControlService(h.get(), SERVICE_CONTROL_STOP, &st); if (!ok) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_NOT_ACTIVE) { StringCchCopyW(out_result, cch, L"stopped"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: stop refused (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"stop requested"); return true; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"stop requested (status query failed: %s)", ebuf); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, L"stop requested (still stopping or refused)"); return true; } static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static bool stop_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp0{}; DWORD bytes0 = 0; if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp0, sizeof(ssp0), &bytes0)) { if (ssp0.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } } SERVICE_STATUS st{}; BOOL ok = ControlService(h.get(), SERVICE_CONTROL_STOP, &st); if (!ok) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_NOT_ACTIVE) { StringCchCopyW(out_result, cch, L"stopped"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: stop refused (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"stop requested"); return true; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes = 0; uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"stop requested (status query failed: %s)", ebuf); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) { StringCchCopyW(out_result, cch, L"stopped"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, L"stop requested (still stopping or refused)"); return true; } static bool start_service_and_wait(const wchar_t* svc_name, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_CONNECT)); if (!scm) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenSCManager failed (%s)", ebuf); return false; } unique_sc_handle h(OpenServiceW(scm.get(), svc_name, SERVICE_START | SERVICE_QUERY_STATUS)); if (!h) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: OpenService failed (%s)", ebuf); return false; } SERVICE_STATUS_PROCESS ssp{}; DWORD bytes_needed = 0; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: status query failed (%s)", ebuf); return false; } if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } if (!StartServiceW(h.get(), 0, NULL)) { DWORD e = GetLastError(); if (e == ERROR_SERVICE_ALREADY_RUNNING) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t ebuf[300]; format_winerr(e, ebuf, 300); StringCchPrintfW(out_result, cch, L"error: start failed (%s)", ebuf); return false; } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, L"start requested"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; for (;;) { if (now_mono_ms() >= deadline) break; if (!QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) break; if (ssp.dwCurrentState == SERVICE_RUNNING) { StringCchCopyW(out_result, cch, L"running"); return true; } if (ssp.dwCurrentState == SERVICE_STOPPED) break; Sleep(150); } // Final status snapshot if (QueryServiceStatusEx(h.get(), SC_STATUS_PROCESS_INFO, (LPBYTE)&ssp, sizeof(ssp), &bytes_needed)) { if (ssp.dwCurrentState == SERVICE_RUNNING) StringCchCopyW(out_result, cch, L"running"); else if (ssp.dwCurrentState == SERVICE_START_PENDING) StringCchCopyW(out_result, cch, L"start pending"); else StringCchCopyW(out_result, cch, L"start requested"); } else { StringCchCopyW(out_result, cch, L"start requested"); } return true; } static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static bool start_exe_and_wait(const wchar_t* exe_lower_name, const wchar_t* launch_path_opt, int wait_ms, wchar_t* out_result, size_t cch) { StringCchCopyW(out_result, cch, L"error"); if (!exe_lower_name || !exe_lower_name[0]) { StringCchCopyW(out_result, cch, L"error: invalid name"); return false; } if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"already running"); return true; } wchar_t path[MAX_PATH] = {0}; if (launch_path_opt && launch_path_opt[0] && PathFileExistsW(launch_path_opt)) { StringCchCopyW(path, MAX_PATH, launch_path_opt); } else { // Try SearchPath on PATH using the *original* exe name; best effort. wchar_t found[MAX_PATH] = {0}; DWORD r = SearchPathW(NULL, exe_lower_name, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } else { // Try with ".exe" suffix if not present if (!ends_with_i(exe_lower_name, L".exe")) { wchar_t buf2[260]; StringCchCopyW(buf2, 260, exe_lower_name); StringCchCatW(buf2, 260, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(path, MAX_PATH, found); } } } } // If we still don't have a path, fall back to ShellExecute using the name. bool used_shell_execute = false; if (!path[0]) { HINSTANCE h = ShellExecuteW(NULL, L"open", exe_lower_name, NULL, NULL, SW_SHOWNORMAL); if ((INT_PTR)h <= 32) { StringCchCopyW(out_result, cch, L"error: not found on PATH (add with full path or use Browse\u2026)"); return false; } used_shell_execute = true; } else { // CreateProcess wchar_t cmd[2 * MAX_PATH + 8]; StringCchPrintfW(cmd, _countof(cmd), L"\"%s\"", path); // Working directory = folder of exe wchar_t workdir[MAX_PATH]; StringCchCopyW(workdir, MAX_PATH, path); PathRemoveFileSpecW(workdir); STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW( NULL, cmd, NULL, NULL, FALSE, 0, NULL, workdir[0] ? workdir : NULL, &si, &pi ); if (!ok) { DWORD e = GetLastError(); wchar_t ebuf[300]; format_winerr(e, ebuf, 300); wchar_t msg[300]; StringCchPrintfW(msg, 300, L"error: start failed (%s)", ebuf); StringCchCopyW(out_result, cch, msg); return false; } CloseHandle(pi.hThread); CloseHandle(pi.hProcess); } if (wait_ms <= 0) { StringCchCopyW(out_result, cch, used_shell_execute ? L"started (ShellExecute)" : L"started"); return true; } uint64_t deadline = now_mono_ms() + (uint64_t)wait_ms; while (now_mono_ms() < deadline) { if (process_count_by_name_lower(exe_lower_name) > 0) { StringCchCopyW(out_result, cch, L"running"); return true; } Sleep(150); } StringCchCopyW(out_result, cch, used_shell_execute ? L"start requested" : L"started"); return true; } static bool enum_services_build_disp_cache(DispCache* out_cache) { dispcache_free(out_cache); unique_sc_handle scm(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE)); if (!scm) return false; DWORD bytes_needed = 0; DWORD count = 0; DWORD resume = 0; std::vector<BYTE> buf(128 * 1024); for (;;) { BOOL ok = EnumServicesStatusExW( scm.get(), SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, buf.data(), (DWORD)buf.size(), &bytes_needed, &count, &resume, NULL ); if (ok) break; DWORD e = GetLastError(); if (e != ERROR_MORE_DATA || bytes_needed == 0) return false; buf.resize(bytes_needed + 16 * 1024); } auto* arr = (ENUM_SERVICE_STATUS_PROCESSW*)buf.data(); for (DWORD i = 0; i < count; i++) { const wchar_t* disp = arr[i].lpDisplayName ? arr[i].lpDisplayName : L""; const wchar_t* key = arr[i].lpServiceName ? arr[i].lpServiceName : L""; if (!disp[0] || !key[0]) continue; std::wstring d(disp); CharLowerBuffW(d.data(), (DWORD)d.size()); dispcache_push(out_cache, d.c_str(), key); } return true; } static void resolve_service_name(App& self, const wchar_t* raw, wchar_t* out, size_t cch) { StringCchCopyW(out, cch, raw ? raw : L""); trim_ws_inplace(out); if (!out[0]) return; wchar_t st[32]; if (query_service_status_fast(out, st, 32)) return; const wchar_t* key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } enum_services_build_disp_cache(&self.disp_cache); key = dispcache_lookup(&self.disp_cache, out); if (key) { StringCchCopyW(out, cch, key); return; } } // ---------------------------- // ListView helpers // ---------------------------- static void lv_set_columns(HWND lv, const wchar_t* col0, const wchar_t* col1, const wchar_t* col2, const wchar_t* col3) { ListView_DeleteAllItems(lv); while (ListView_DeleteColumn(lv, 0)) {} // Scale default column widths to the current DPI so the layout feels consistent UINT dpi = get_dpi_for_hwnd(lv); auto S = [&](int v96) -> int { return dpi_scale(dpi, v96); }; LVCOLUMNW c; memset(&c, 0, sizeof(c)); c.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_FMT; c.fmt = LVCFMT_CENTER; c.pszText = (LPWSTR)col0; c.cx = S(90); ListView_InsertColumn(lv, 0, &c); c.fmt = LVCFMT_LEFT; c.pszText = (LPWSTR)col1; c.cx = S(380); ListView_InsertColumn(lv, 1, &c); c.pszText = (LPWSTR)col2; c.cx = S(140); ListView_InsertColumn(lv, 2, &c); c.pszText = (LPWSTR)col3; c.cx = S(220); ListView_InsertColumn(lv, 3, &c); ListView_SetExtendedListViewStyle(lv, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP); } static void lv_scale_columns(HWND lv, UINT old_dpi, UINT new_dpi) { if (!lv || !old_dpi || !new_dpi || old_dpi == new_dpi) return; int cols = 0; HWND hdr = ListView_GetHeader(lv); if (hdr) cols = Header_GetItemCount(hdr); for (int i = 0; i < cols; i++) { int w = ListView_GetColumnWidth(lv, i); if (w <= 0) continue; int nw = MulDiv(w, (int)new_dpi, (int)old_dpi); if (nw < 16) nw = 16; ListView_SetColumnWidth(lv, i, nw); } } static int lv_find_item_by_name(HWND lv, const wchar_t* name) { int n = ListView_GetItemCount(lv); for (int i = 0; i < n; i++) { wchar_t buf[512]; ListView_GetItemText(lv, i, 1, buf, 512); if (_wcsicmp(buf, name) == 0) return i; } return -1; } static int lv_find_item_by_ptr(HWND lv, const Item* it) { if (!lv || !it) return -1; LVFINDINFOW fi; memset(&fi, 0, sizeof(fi)); fi.flags = LVFI_PARAM; fi.lParam = (LPARAM)it; return ListView_FindItem(lv, -1, &fi); } static void lv_autosize_status_last(HWND lv) { ListView_SetColumnWidth(lv, 2, LVSCW_AUTOSIZE_USEHEADER); ListView_SetColumnWidth(lv, 3, LVSCW_AUTOSIZE_USEHEADER); } static void lv_apply_name_fill(HWND lv) { RECT rc; GetClientRect(lv, &rc); int total = rc.right - rc.left; int w0 = ListView_GetColumnWidth(lv, 0); int w2 = ListView_GetColumnWidth(lv, 2); int w3 = ListView_GetColumnWidth(lv, 3); int slack = 30; int avail = total - (w0 + w2 + w3 + slack); if (avail < 160) avail = 160; ListView_SetColumnWidth(lv, 1, avail); } static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = lv_find_item_by_ptr(lv, it); if (idx < 0) idx = lv_find_item_by_name(lv, it->name.c_str()); wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { // Indicator: EXEs that were started automatically due to an active profile. if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { StringCchCatW(status_disp, 64, L" (Profile)"); } } bool inserted = false; if (idx < 0) { LVITEMW item; memset(&item, 0, sizeof(item)); item.mask = LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE; item.iItem = ListView_GetItemCount(lv); item.iSubItem = 0; item.pszText = const_cast<LPWSTR>(L""); item.lParam = (LPARAM)it; item.iImage = it->img; int row = ListView_InsertItem(lv, &item); inserted = (row >= 0); ListView_SetItemText(lv, row, 1, (LPWSTR)it->name.c_str()); ListView_SetItemText(lv, row, 2, status_disp); ListView_SetItemText(lv, row, 3, last); bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, row, it->auto_stop ? TRUE : FALSE); self.suppress_lv_notify = prev; } else { // Ensure the row is bound to this Item* for O(1) future lookup LVITEMW pi; memset(&pi, 0, sizeof(pi)); pi.mask = LVIF_PARAM | LVIF_IMAGE; pi.iItem = idx; pi.lParam = (LPARAM)it; pi.iImage = it->img; ListView_SetItem(lv, &pi); ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } } return inserted; } // Update an existing ListView row for Item* (NO insertion). Returns true if updated. static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) { if (!lv || !it || it->name.empty()) return false; int idx = -1; auto& mp = self.lv_row_of_item[kind]; auto itf = mp.find(it); if (itf != mp.end()) idx = itf->second; // Validate cached row still points at this Item* if (idx >= 0) { LVITEMW qi{}; qi.mask = LVIF_PARAM; qi.iItem = idx; if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1; } if (idx < 0) { // Fallback search by lParam (still no insertion) idx = lv_find_item_by_ptr(lv, it); if (idx < 0) return false; mp[it] = idx; } wchar_t last[64]; fmt_time_local(last, 64, wall_ts); // Build status display without heap allocations wchar_t status_disp[64]; status_disp[0] = 0; StringCchCopyW(status_disp, 64, (status_raw && status_raw[0]) ? status_raw : L"unknown"); if (lv == self.lv_exe) { if (self.profile_started_exes.find(it->name) != self.profile_started_exes.end()) { // Prefix indicator wchar_t tmp[64]; StringCchCopyW(tmp, 64, status_disp); StringCchPrintfW(status_disp, 64, L"* %s", tmp); } } // Avoid churn if nothing changed if (it->ui_applied_gen == it->status_gen && wcscmp(it->ui_last_status, status_disp) == 0 && it->ui_last_update_wall == wall_ts) { return false; } ListView_SetItemText(lv, idx, 2, status_disp); ListView_SetItemText(lv, idx, 3, last); // Column 1 (Name) generally stable; don't touch. BOOL cur = ListView_GetCheckState(lv, idx); BOOL want = it->auto_stop ? TRUE : FALSE; if ((cur != 0) != (want != 0)) { bool prev = self.suppress_lv_notify; self.suppress_lv_notify = true; ListView_SetCheckState(lv, idx, want); self.suppress_lv_notify = prev; } StringCchCopyW(it->ui_last_status, 32, status_disp); it->ui_last_update_wall = wall_ts; it->ui_applied_gen = it->status_gen; return true; } static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static void post_log(App& self, const wchar_t* text) { LogMsg* m = (LogMsg*)malloc(sizeof(LogMsg)); if (!m) return; m->text = wcsdup_heap(text ? text : L""); if (!m->text) { free(m); return; } if (!PostMessageW(self.hwnd, WM_APP_LOG, 0, (LPARAM)m)) { free(m->text); free(m); } } typedef struct StatusMsg { int kind; wchar_t* name; wchar_t status[32]; time_t wall_ts; } StatusMsg; // Bulk status update (generation-only, no heap churn): // - Monitor thread updates Item::last_status/last_update_wall under lock. // - UI thread repaints only rows that changed (via WM_APP_STATUS_GEN / UI timer). static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], size_t n, time_t wall_ts) { if (!names || !statuses || n == 0) return; bool any_changed = false;;
static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], size_t n, time_t wall_ts) { if (!names || !statuses || n == 0) return; bool any_changed = false;;
static void action_enqueue(App& self, int kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms); static void update_statusbar(App& self); // ---------------------------- // Profile helpers // ---------------------------- static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out) { out.ui_refresh_ms = self.ui_refresh_ms; out.monitor_interval_ms = self.monitor_interval_ms; out.autostop_cooldown_ms = self.autostop_cooldown_ms; out.stop_wait_ms = self.stop_wait_ms; out.tray_enabled = self.tray_enabled; out.close_to_tray = self.close_to_tray; cfg_clear_items(out); for (int kind = 0; kind < 2; kind++) { out.items[kind].reserve(self.items[kind].v.size()); for (auto& up : self.items[kind].v) { Item* it = up.get(); if (!it || it->name.empty()) continue; ItemRow r; r.name = it->name; r.auto_stop = it->auto_stop; if (kind == KIND_EXE && !it->exe_path.empty()) r.exe_path = it->exe_path; out.items[kind].push_back(std::move(r)); } } } static void snapshot_active_from_runtime_locked(App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { snapshot_from_runtime_locked(self, self.profiles[(size_t)self.active_profile].cfg); } else { snapshot_from_runtime_locked(self, self.default_cfg); self.have_default_cfg = true; } } static void snapshot_active_from_runtime(App& self) { std::lock_guard<std::mutex> lk(self.mtx); snapshot_active_from_runtime_locked(self); } static void rebuild_runtime_items_locked(App& self, const ConfigSnapshot& cfg) { list_free(&self.items[KIND_SVC]); list_free(&self.items[KIND_EXE]); for (int kind = 0; kind < 2; kind++) { for (const auto& row : cfg.items[kind]) { if (row.name.empty()) continue; Item* it = new (std::nothrow) Item(); if (!it) continue; it->kind = kind; it->name = row.name; if (kind == KIND_EXE && !row.exe_path.empty()) it->exe_path = row.exe_path; it->auto_stop = row.auto_stop; it->img = (kind == KIND_SVC) ? 0 : 1; it->autostop_count = 0; it->last_autostop_mono_ms = 0; it->last_update_wall = time(NULL); if (kind == KIND_SVC) { wchar_t st[32]; if (query_service_status_fast(it->name.c_str(), st, 32)) StringCchCopyW(it->last_status, 32, st); else StringCchCopyW(it->last_status, 32, L"not found"); } else { int c = process_count_by_name_lower(it->name.c_str()); StringCchCopyW(it->last_status, 32, (c > 0) ? L"running" : L"stopped"); } list_push(&self.items[kind], it); } } self.items_gen[KIND_SVC]++; self.items_gen[KIND_EXE]++; } static const wchar_t* active_profile_name_locked(const App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { const auto& p = self.profiles[(size_t)self.active_profile].name; return p.empty() ? L"(unnamed)" : p.c_str(); } return L"Default"; } // UI thread: apply config snapshot to runtime + listviews static void apply_profile_index(App& self, int idx) { // idx: -1 = default, 0..N-1 = profiles std::vector<Item*> svc_ptrs; std::vector<Item*> exe_ptrs; std::wstring pname; // Processes started by the *previous* active profile (close them when leaving) std::vector<StartedProc> to_stop_started; // Shell-open targets to launch for the *new* profile activation std::vector<std::wstring> to_launch;;
static bool lv_set_row(App& self, HWND lv, Item* it) { // Callers that hold self.mtx can use this wrapper safely. return lv_set_row_with(self, lv, it, it->last_status, it->last_update_wall); } static bool lv_get_selected_name(HWND lv, wchar_t* out, size_t cch) { if (!lv || !out || cch == 0) return false; out[0] = 0; // Prefer explicit selection, but fall back to focused/selection-mark // because clicking the checkbox column doesn't always set LVIS_SELECTED. int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return false; ListView_GetItemText(lv, sel, 1, out, (int)cch); return out[0] != 0; } // ---------------------------- // Activity log listbox // ---------------------------- static void activity_append(App& self, HWND lb, const wchar_t* line) { if (!lb) return; int count = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); if (count >= MAX_LOG_LINES) { int drop = count - MAX_LOG_LINES + 1; for (int i = 0; i < drop; i++) SendMessageW(lb, LB_DELETESTRING, 0, 0); } SendMessageW(lb, LB_ADDSTRING, 0, (LPARAM)line); HDC hdc = GetDC(lb); HFONT hf = (HFONT)SendMessageW(lb, WM_GETFONT, 0, 0); HFONT old = (HFONT)SelectObject(hdc, hf); SIZE sz{}; GetTextExtentPoint32W(hdc, line, (int)wcslen(line), &sz); SelectObject(hdc, old); ReleaseDC(lb, hdc); int extra = dpi_scale(self.dpi ? self.dpi : 96, 24); int cur = (int)SendMessageW(lb, LB_GETHORIZONTALEXTENT, 0, 0); if (sz.cx + extra > cur) SendMessageW(lb, LB_SETHORIZONTALEXTENT, (WPARAM)(sz.cx + extra), 0); int newCount = (int)SendMessageW(lb, LB_GETCOUNT, 0, 0); SendMessageW(lb, LB_SETTOPINDEX, (WPARAM)std::max(0, newCount - 1), 0); } static void ui_apply_status_updates(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; struct UiDelta { Item* it; wchar_t status[32]; time_t wall; }; std::vector<UiDelta> deltas; deltas.reserve(64); std::wstring flt;;
static void log_linef(App& self, const wchar_t* fmt, ...) { wchar_t msg[1024]; va_list ap; va_start(ap, fmt); StringCchVPrintfW(msg, 1024, fmt, ap); va_end(ap); wchar_t ts[16]; SYSTEMTIME st; GetLocalTime(&st); StringCchPrintfW(ts, 16, L"%02u:%02u:%02u", st.wHour, st.wMinute, st.wSecond); wchar_t line[1200]; StringCchPrintfW(line, 1200, L"[%s] %s", ts, msg); activity_append(self, self.activity, line); } // ---------------------------- // Facelift helpers that depend on Item/App // ---------------------------- static bool item_matches_filter(const Item* it, const std::wstring& flt_lower) { if (!it) return false; if (flt_lower.empty()) return true; // Match against name std::wstring name_lower = it->name; if (!name_lower.empty()) CharLowerBuffW(name_lower.data(), (DWORD)name_lower.size()); if (name_lower.find(flt_lower) != std::wstring::npos) return true; // Match against status (stored as wchar_t[32]) std::wstring st_lower = it->last_status; if (!st_lower.empty()) CharLowerBuffW(st_lower.data(), (DWORD)st_lower.size()); if (st_lower.find(flt_lower) != std::wstring::npos) return true; return false; } static void rebuild_listview_filtered(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; // Rebuild from authoritative model under lock to avoid dangling Item*. std::lock_guard<std::mutex> lk(self.mtx); const std::wstring flt = self.filter[kind]; bool prev_suppress = self.suppress_lv_notify; self.suppress_lv_notify = true; // Bulk rebuild: suppress redraw for performance and consistency. SendMessageW(lv, WM_SETREDRAW, FALSE, 0); ListView_DeleteAllItems(lv); self.lv_row_of_item[kind].clear(); for (auto& up : self.items[kind].v) { Item* it = up.get(); if (!it) continue; if (item_matches_filter(it, flt)) { lv_set_row(self, lv, it); int row = lv_find_item_by_ptr(lv, it); if (row >= 0) self.lv_row_of_item[kind][it] = row; } } SendMessageW(lv, WM_SETREDRAW, TRUE, 0); InvalidateRect(lv, NULL, FALSE); self.suppress_lv_notify = prev_suppress; self.lv_needs_layout[kind] = true; post_model_dirty(self); } static Item* lv_get_selected_item_ptr(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return nullptr; int sel = ListView_GetNextItem(lv, -1, LVNI_SELECTED); if (sel < 0) sel = ListView_GetNextItem(lv, -1, LVNI_FOCUSED); if (sel < 0) sel = ListView_GetSelectionMark(lv); if (sel < 0) return nullptr; LVITEMW it{}; it.mask = LVIF_PARAM; it.iItem = sel; if (!ListView_GetItem(lv, &it)) return nullptr; return reinterpret_cast<Item*>(it.lParam); } static void toggle_autostop_selected(App& self, int kind) { HWND lv = (kind == KIND_SVC) ? self.lv_svc : self.lv_exe; if (!lv) return; Item* sel = lv_get_selected_item_ptr(self, kind); if (!sel) { msgbox_warn(self.hwnd, L"Auto-stop", L"Select a row first."); return; } bool now = false;;
static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out); static void snapshot_active_from_runtime_locked(App& self); [[maybe_unused]] static void snapshot_active_from_runtime(App& self); static void apply_profile_index(App& self, int idx); static void action_enqueue(App& self, int kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms); static void update_statusbar(App& self); // ---------------------------- // Profile helpers // ---------------------------- static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out) { out.ui_refresh_ms = self.ui_refresh_ms; out.monitor_interval_ms = self.monitor_interval_ms; out.autostop_cooldown_ms = self.autostop_cooldown_ms; out.stop_wait_ms = self.stop_wait_ms; out.tray_enabled = self.tray_enabled; out.close_to_tray = self.close_to_tray; cfg_clear_items(out); for (int kind = 0; kind < 2; kind++) { out.items[kind].reserve(self.items[kind].v.size()); for (auto& up : self.items[kind].v) { Item* it = up.get(); if (!it || it->name.empty()) continue; ItemRow r; r.name = it->name; r.auto_stop = it->auto_stop; if (kind == KIND_EXE && !it->exe_path.empty()) r.exe_path = it->exe_path; out.items[kind].push_back(std::move(r)); } } } static void snapshot_active_from_runtime_locked(App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { snapshot_from_runtime_locked(self, self.profiles[(size_t)self.active_profile].cfg); } else { snapshot_from_runtime_locked(self, self.default_cfg); self.have_default_cfg = true; } } static void snapshot_active_from_runtime(App& self) { std::lock_guard<std::mutex> lk(self.mtx); snapshot_active_from_runtime_locked(self); } static void rebuild_runtime_items_locked(App& self, const ConfigSnapshot& cfg) { list_free(&self.items[KIND_SVC]); list_free(&self.items[KIND_EXE]); for (int kind = 0; kind < 2; kind++) { for (const auto& row : cfg.items[kind]) { if (row.name.empty()) continue; Item* it = new (std::nothrow) Item(); if (!it) continue; it->kind = kind; it->name = row.name; if (kind == KIND_EXE && !row.exe_path.empty()) it->exe_path = row.exe_path; it->auto_stop = row.auto_stop; it->img = (kind == KIND_SVC) ? 0 : 1; it->autostop_count = 0; it->last_autostop_mono_ms = 0; it->last_update_wall = time(NULL); if (kind == KIND_SVC) { wchar_t st[32]; if (query_service_status_fast(it->name.c_str(), st, 32)) StringCchCopyW(it->last_status, 32, st); else StringCchCopyW(it->last_status, 32, L"not found"); } else { int c = process_count_by_name_lower(it->name.c_str()); StringCchCopyW(it->last_status, 32, (c > 0) ? L"running" : L"stopped"); } list_push(&self.items[kind], it); } } self.items_gen[KIND_SVC]++; self.items_gen[KIND_EXE]++; } static const wchar_t* active_profile_name_locked(const App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { const auto& p = self.profiles[(size_t)self.active_profile].name; return p.empty() ? L"(unnamed)" : p.c_str(); } return L"Default"; } // UI thread: apply config snapshot to runtime + listviews static void apply_profile_index(App& self, int idx) { // idx: -1 = default, 0..N-1 = profiles std::vector<Item*> svc_ptrs; std::vector<Item*> exe_ptrs; std::wstring pname; // Processes started by the *previous* active profile (close them when leaving) std::vector<StartedProc> to_stop_started; // Shell-open targets to launch for the *new* profile activation std::vector<std::wstring> to_launch;;
static void snapshot_active_from_runtime_locked(App& self); [[maybe_unused]] static void snapshot_active_from_runtime(App& self); static void apply_profile_index(App& self, int idx); static void action_enqueue(App& self, int kind, int op, const wchar_t* name, const wchar_t* reason, int wait_ms); static void update_statusbar(App& self); // ---------------------------- // Profile helpers // ---------------------------- static void snapshot_from_runtime_locked(App& self, ConfigSnapshot& out) { out.ui_refresh_ms = self.ui_refresh_ms; out.monitor_interval_ms = self.monitor_interval_ms; out.autostop_cooldown_ms = self.autostop_cooldown_ms; out.stop_wait_ms = self.stop_wait_ms; out.tray_enabled = self.tray_enabled; out.close_to_tray = self.close_to_tray; cfg_clear_items(out); for (int kind = 0; kind < 2; kind++) { out.items[kind].reserve(self.items[kind].v.size()); for (auto& up : self.items[kind].v) { Item* it = up.get(); if (!it || it->name.empty()) continue; ItemRow r; r.name = it->name; r.auto_stop = it->auto_stop; if (kind == KIND_EXE && !it->exe_path.empty()) r.exe_path = it->exe_path; out.items[kind].push_back(std::move(r)); } } } static void snapshot_active_from_runtime_locked(App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { snapshot_from_runtime_locked(self, self.profiles[(size_t)self.active_profile].cfg); } else { snapshot_from_runtime_locked(self, self.default_cfg); self.have_default_cfg = true; } } static void snapshot_active_from_runtime(App& self) { std::lock_guard<std::mutex> lk(self.mtx); snapshot_active_from_runtime_locked(self); } static void rebuild_runtime_items_locked(App& self, const ConfigSnapshot& cfg) { list_free(&self.items[KIND_SVC]); list_free(&self.items[KIND_EXE]); for (int kind = 0; kind < 2; kind++) { for (const auto& row : cfg.items[kind]) { if (row.name.empty()) continue; Item* it = new (std::nothrow) Item(); if (!it) continue; it->kind = kind; it->name = row.name; if (kind == KIND_EXE && !row.exe_path.empty()) it->exe_path = row.exe_path; it->auto_stop = row.auto_stop; it->img = (kind == KIND_SVC) ? 0 : 1; it->autostop_count = 0; it->last_autostop_mono_ms = 0; it->last_update_wall = time(NULL); if (kind == KIND_SVC) { wchar_t st[32]; if (query_service_status_fast(it->name.c_str(), st, 32)) StringCchCopyW(it->last_status, 32, st); else StringCchCopyW(it->last_status, 32, L"not found"); } else { int c = process_count_by_name_lower(it->name.c_str()); StringCchCopyW(it->last_status, 32, (c > 0) ? L"running" : L"stopped"); } list_push(&self.items[kind], it); } } self.items_gen[KIND_SVC]++; self.items_gen[KIND_EXE]++; } static const wchar_t* active_profile_name_locked(const App& self) { if (self.active_profile >= 0 && self.active_profile < (int)self.profiles.size()) { const auto& p = self.profiles[(size_t)self.active_profile].name; return p.empty() ? L"(unnamed)" : p.c_str(); } return L"Default"; } // UI thread: apply config snapshot to runtime + listviews static void apply_profile_index(App& self, int idx) { // idx: -1 = default, 0..N-1 = profiles std::vector<Item*> svc_ptrs; std::vector<Item*> exe_ptrs; std::wstring pname; // Processes started by the *previous* active profile (close them when leaving) std::vector<StartedProc> to_stop_started; // Shell-open targets to launch for the *new* profile activation std::vector<std::wstring> to_launch;;
static void rebuild_profile_watch_keys_locked(App& self) { auto nv = std::make_shared<std::vector<std::vector<std::wstring>>>(); auto hv = std::make_shared<std::vector<std::vector<uint64_t>>>(); nv->reserve(self.profiles.size()); hv->reserve(self.profiles.size()); for (auto& p : self.profiles) { // Keep the cached keys in sync. watch_exes are already normalized lower-case basenames. if (p.watch_keys_lower.empty() && !p.watch_exes.empty()) { p.watch_keys_lower = p.watch_exes; } else if (p.watch_keys_lower.size() != p.watch_exes.size()) { // Best-effort resync; avoids stale cache if older configs/UI paths changed. p.watch_keys_lower = p.watch_exes; } nv->push_back(p.watch_keys_lower); // Also cache hashes for allocation-free matching in the monitor thread. std::vector<uint64_t> hs; hs.reserve(p.watch_keys_lower.size()); for (const auto& k : p.watch_keys_lower) { if (k.empty()) continue; hs.push_back(fnv1a64_exe_lower_hash(k.c_str())); // already lower-case } std::sort(hs.begin(), hs.end()); hs.erase(std::unique(hs.begin(), hs.end()), hs.end()); hv->push_back(std::move(hs)); } self.profile_watch_keys_sp = std::move(nv); self.profile_watch_hashes_sp = std::move(hv); } // -------------------------------------------------- // Final-form structure note // - We removed the global App* and the global app() accessor. // - Legacy code still uses self.field; we map app() -> `self` via a macro. // Any function that uses app() must have an `App& self` in scope. // - The main window proc is now instance-bound via GWLP_USERDATA. // -------------------------------------------------- #define app() (self) // ---------------------------- // Dark mode / theme helpers // ---------------------------- // ---------------------------- // Dark mode opt-in (UxTheme ordinals) // ---------------------------- struct DarkModeApi { HMODULE ux = NULL; // Ordinals commonly used (undocumented; best-effort) using PFN_SetPreferredAppMode = int (WINAPI*)(int); using PFN_FlushMenuThemes = void (WINAPI*)(); using PFN_AllowDarkModeForWindow = BOOL (WINAPI*)(HWND, BOOL); PFN_SetPreferredAppMode SetPreferredAppMode = nullptr; PFN_FlushMenuThemes FlushMenuThemes = nullptr; PFN_AllowDarkModeForWindow AllowDarkModeForWindow = nullptr; bool inited = false; }; static DarkModeApi g_dm; template<typename T> static T dm_get_ordinal(HMODULE m, int ordinal) { if (!m) return nullptr; FARPROC fp = GetProcAddress(m, MAKEINTRESOURCEA(ordinal)); if (!fp) return nullptr; // Avoid -Wcast-function-type on GCC/Clang by casting via integer. return (T)(uintptr_t)fp; } static void darkmode_init_api() { if (g_dm.inited) return; g_dm.inited = true; g_dm.ux = LoadLibraryW(L"uxtheme.dll"); if (!g_dm.ux) return; // These are ordinals used by many Win32 apps. If they don't exist, we simply no-op. g_dm.SetPreferredAppMode = dm_get_ordinal<DarkModeApi::PFN_SetPreferredAppMode>(g_dm.ux, 135); g_dm.FlushMenuThemes = dm_get_ordinal<DarkModeApi::PFN_FlushMenuThemes>(g_dm.ux, 136); g_dm.AllowDarkModeForWindow = dm_get_ordinal<DarkModeApi::PFN_AllowDarkModeForWindow>(g_dm.ux, 133); } static void darkmode_set_app(bool want_dark) { darkmode_init_api(); if (!g_dm.SetPreferredAppMode) return; // PreferredAppMode: Default=0, AllowDark=1, ForceDark=2, ForceLight=3 g_dm.SetPreferredAppMode(want_dark ? 1 : 0); if (g_dm.FlushMenuThemes) g_dm.FlushMenuThemes(); } static void darkmode_allow_window(HWND hwnd, bool allow_dark) { if (!hwnd) return; darkmode_init_api(); if (g_dm.AllowDarkModeForWindow) { g_dm.AllowDarkModeForWindow(hwnd, allow_dark ? TRUE : FALSE); } } static bool try_set_explorer_theme_mode(HWND hwnd, bool dark) { if (!hwnd) return false; HMODULE ux = LoadLibraryW(L"uxtheme.dll"); if (!ux) return false; typedef HRESULT (WINAPI *PFN_SetWindowTheme)(HWND, LPCWSTR, LPCWSTR); PFN_SetWindowTheme p = nullptr; LOAD_PROC(ux, "SetWindowTheme", PFN_SetWindowTheme, p); bool ok = false; if (p) { // "DarkMode_Explorer" is honored by common controls on Win10/11; harmless if ignored. p(hwnd, dark ? L"DarkMode_Explorer" : L"Explorer", nullptr); ok = true; } FreeLibrary(ux); return ok; } // Custom-draw helpers for controls that don't fully honor DarkMode_Explorer on all builds. // Custom-draw for listviews (items) so text/bk colors are correct in dark mode even when the control ignores LVM_* colors. // Dark-mode push buttons: subclass paint so we don't rely on BS_OWNERDRAW + parent WM_DRAWITEM. // This is more reliable because many buttons live inside container child windows (tab pages/panels), // and WM_DRAWITEM would go to those parents, not the main window. static LRESULT CALLBACK DarkButtonSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR /*idSubclass*/, DWORD_PTR refData) { App* self = reinterpret_cast<App*>(refData); switch (msg) { case WM_ERASEBKGND: if (self && self->dark_mode) return 1; break; case WM_ENABLE: case WM_THEMECHANGED: case WM_STYLECHANGED: InvalidateRect(hwnd, NULL, TRUE); break; case WM_PAINT: { if (!self || !self->dark_mode) break; PAINTSTRUCT ps{}; HDC hdc = BeginPaint(hwnd, &ps); RECT rc{}; GetClientRect(hwnd, &rc); const bool disabled = !IsWindowEnabled(hwnd); const LRESULT st = SendMessageW(hwnd, BM_GETSTATE, 0, 0); const bool pressed = (st & BST_PUSHED) != 0; COLORREF bg = pressed ? RGB(55,55,55) : self->col_panel; HBRUSH br = CreateSolidBrush(bg); FillRect(hdc, &rc, br); DeleteObject(br); // Border HPEN pen = CreatePen(PS_SOLID, 1, RGB(70,70,70)); HGDIOBJ oldp = SelectObject(hdc, pen); HGDIOBJ oldb = SelectObject(hdc, GetStockObject(NULL_BRUSH)); Rectangle(hdc, rc.left, rc.top, rc.right, rc.bottom); SelectObject(hdc, oldb); SelectObject(hdc, oldp); DeleteObject(pen); // Text wchar_t txt[256]{0}; GetWindowTextW(hwnd, txt, 255); SetBkMode(hdc, TRANSPARENT); SetTextColor(hdc, disabled ? self->col_text_dim : self->col_text); // Ensure we draw with the button's assigned font (WM_SETFONT), otherwise GDI may use a default font // during subclass painting, causing mismatched text size between light/dark mode. HFONT hfont = (HFONT)SendMessageW(hwnd, WM_GETFONT, 0, 0); HGDIOBJ oldf = NULL; if (hfont) oldf = SelectObject(hdc, hfont); RECT tr = rc; if (pressed) { tr.left += 1; tr.top += 1; } DrawTextW(hdc, txt, -1, &tr, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS); if (oldf) SelectObject(hdc, oldf); // Focus rect if (GetFocus() == hwnd && !disabled) { RECT fr = rc; InflateRect(&fr, -3, -3); DrawFocusRect(hdc, &fr); } EndPaint(hwnd, &ps); return 0; } case WM_NCDESTROY: RemoveWindowSubclass(hwnd, DarkButtonSubclassProc, 1); break; } return DefSubclassProc(hwnd, msg, wParam, lParam); } // Apply theming (including dark mode opt-in and owner-draw toggles) to a single control. static BOOL CALLBACK enum_theme_children_proc(HWND child, LPARAM lp) { App* self = reinterpret_cast<App*>(lp); if (!self) return TRUE; theme_apply_to_control(*self, child); EnumChildWindows(child, enum_theme_children_proc, lp); return TRUE; } static bool try_enable_dark_titlebar(HWND hwnd, bool enable); static BOOL CALLBACK enum_theme_thread_windows_proc(HWND hwnd, LPARAM lp) { App* self = reinterpret_cast<App*>(lp); if (!self) return TRUE; theme_apply_to_window(*self, hwnd); return TRUE; } // ---------------------------- // Theme + per-window DPI font cache helpers // ---------------------------- static void theme_compute(App& self); // Back-compat wrapper (older call sites). Prefer theme_compute()+theme_apply_all_controls(). // ---------------------------- // Tray icon helpers // ---------------------------- static HICON load_app_icon(HINSTANCE hInst, int cx, int cy) { #ifdef IDI_APPICON return (HICON)LoadImageW(hInst, MAKEINTRESOURCEW(IDI_APPICON), IMAGE_ICON, cx, cy, 0); #else (void)hInst; (void)cx; (void)cy; return LoadIconW(NULL, IDI_APPLICATION); #endif } // Forward decls for event-driven UI maintenance (definitions later) static void ui_timer_suspend(App& self, HWND hwnd); static void ui_timer_resume(App& self, HWND hwnd); static void restart_ui_timer(App& self, HWND hwnd); static void post_model_dirty(App& self); static void show_main_window(App& self) { HWND hwnd = self.hwnd; if (!hwnd) return; // Ensure visible (close-to-tray uses SW_HIDE) ShowWindow(hwnd, SW_SHOW); // When restoring from tray/hidden, resume UI maintenance. ui_timer_resume(self, hwnd); // If minimized, restore. If not, bring to front. if (IsIconic(hwnd)) { ShowWindow(hwnd, SW_RESTORE); } else { ShowWindow(hwnd, SW_SHOWNORMAL); } // Bring to front (foreground restrictions can otherwise make it look like "nothing happened") SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE); SetWindowPos(hwnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE); SetForegroundWindow(hwnd); SetActiveWindow(hwnd); SetFocus(hwnd); restart_ui_timer(self, hwnd); post_model_dirty(self); } static void hide_main_window(App& self) { if (!self.hwnd) return; ui_timer_suspend(self, self.hwnd); ShowWindow(self.hwnd, SW_HIDE); } static void app_request_exit(App& self, HWND hwnd) { self.force_quit = true; DestroyWindow(hwnd); } // ---------------------------- // FIX: Page container window class that forwards WM_COMMAND / WM_NOTIFY // ---------------------------- static const wchar_t* PAGE_CLASS = L"PageContainer_C"; static LRESULT CALLBACK PageProc(HWND page, UINT msg, WPARAM wParam, LPARAM lParam) { HWND parent = GetParent(page); if (msg == WM_COMMAND || msg == WM_NOTIFY || msg == WM_CTLCOLORSTATIC || msg == WM_CTLCOLOREDIT || msg == WM_CTLCOLORBTN || msg == WM_CTLCOLORLISTBOX) { if (parent) return SendMessageW(parent, msg, wParam, lParam); return 0; } if (msg == WM_ERASEBKGND) { HDC hdc = (HDC)wParam; if (hdc) { RECT rc; GetClientRect(page, &rc); App* self = parent ? reinterpret_cast<App*>(GetWindowLongPtrW(parent, GWLP_USERDATA)) : nullptr; HBRUSH br = self ? self->br_bg : GetSysColorBrush(COLOR_BTNFACE); FillRect(hdc, &rc, br); } return 1; } return DefWindowProcW(page, msg, wParam, lParam); } static void register_page_class(void) { static bool registered = false; if (registered) return; registered = true; WNDCLASSW wc; memset(&wc, 0, sizeof(wc)); wc.lpfnWndProc = PageProc; wc.hInstance = GetModuleHandleW(NULL); wc.lpszClassName = PAGE_CLASS; wc.hbrBackground = (HBRUSH)(COLOR_BTNFACE + 1); wc.hCursor = LoadCursor(NULL, IDC_ARROW); RegisterClassW(&wc); } // ---------------------------- // UI logging (via message to main thread) // ---------------------------- typedef struct LogMsg { wchar_t* text; } LogMsg; static void post_log(App& self, const wchar_t* text) { LogMsg* m = (LogMsg*)malloc(sizeof(LogMsg)); if (!m) return; m->text = wcsdup_heap(text ? text : L""); if (!m->text) { free(m); return; } if (!PostMessageW(self.hwnd, WM_APP_LOG, 0, (LPARAM)m)) { free(m->text); free(m); } } typedef struct StatusMsg { int kind; wchar_t* name; wchar_t status[32]; time_t wall_ts; } StatusMsg; // Bulk status update (generation-only, no heap churn): // - Monitor thread updates Item::last_status/last_update_wall under lock. // - UI thread repaints only rows that changed (via WM_APP_STATUS_GEN / UI timer). static void post_status_bulk(App& self, int kind, wchar_t** names, wchar_t (*statuses)[32], size_t n, time_t wall_ts) { if (!names || !statuses || n == 0) return; bool any_changed = false;;
static void request_save_debounced(App& self) { PostMessageW(self.hwnd, WM_APP_REQUEST_SAVE, 0, 0); } // ---------------------------- // Process helpers // ---------------------------- static void normalize_exe_input(const wchar_t* raw, wchar_t* out, size_t cch_out) { out[0] = 0; if (!raw) return; wchar_t buf[512]; StringCchCopyW(buf, 512, raw); trim_ws_inplace(buf); if (buf[0] == L'"') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'"') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } if (buf[0] == L'\'') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'\'') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } trim_ws_inplace(buf); if (!buf[0]) return; wchar_t exp[1024]; DWORD got = ExpandEnvironmentStringsW(buf, exp, 1024); const wchar_t* s = (got > 0 && got < 1024) ? exp : buf; const wchar_t* base = s; for (const wchar_t* p = s; *p; p++) { if (*p == L'\\' || *p == L'/' || *p == L':') base = p + 1; } wchar_t name[512]; StringCchCopyW(name, 512, base); trim_ws_inplace(name); if (!name[0]) return; if (!ends_with_i(name, L".exe")) { StringCchCatW(name, 512, L".exe"); } // binsearch_nameidx is case-insensitive; no need to lowercase per-process. StringCchCopyW(out, cch_out, name); } static void resolve_exe_launch_path(const wchar_t* raw, wchar_t* out, size_t cch_out) { out[0] = 0; if (!raw) return; wchar_t buf[1024]; StringCchCopyW(buf, 1024, raw); trim_ws_inplace(buf); // Strip simple quotes if (buf[0] == L'"') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'"') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } else if (buf[0] == L'\'') { size_t n = wcslen(buf); if (n > 1 && buf[n - 1] == L'\'') { buf[n - 1] = 0; memmove(buf, buf + 1, n * sizeof(wchar_t)); } } trim_ws_inplace(buf); if (!buf[0]) return; // Expand environment vars (e.g. %SystemRoot%) wchar_t exp[1024]; DWORD nExp = ExpandEnvironmentStringsW(buf, exp, 1024); if (nExp > 0 && nExp < 1024) { StringCchCopyW(buf, 1024, exp); trim_ws_inplace(buf); } // If input looks like a path, canonicalize + validate. bool looks_path = (wcschr(buf, L'\\') || wcschr(buf, L'/') || wcschr(buf, L':')); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(buf, MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { // Ensure .exe suffix if missing if (!ends_with_i(full, L".exe")) { StringCchCatW(full, MAX_PATH, L".exe"); } if (PathFileExistsW(full)) { StringCchCopyW(out, cch_out, full); return; } } // If it was a path and doesn't exist, fall through to SearchPath. } // Try SearchPath on PATH (also handles names without .exe) wchar_t found[MAX_PATH]; DWORD r = SearchPathW(NULL, buf, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(out, cch_out, found); return; } // Try with ".exe" explicitly if missing. if (!ends_with_i(buf, L".exe")) { wchar_t buf2[1024]; StringCchCopyW(buf2, 1024, buf); StringCchCatW(buf2, 1024, L".exe"); r = SearchPathW(NULL, buf2, NULL, MAX_PATH, found, NULL); if (r > 0 && r < MAX_PATH && PathFileExistsW(found)) { StringCchCopyW(out, cch_out, found); return; } } } // ---------------------------- // Profile "Start Items" helpers (shell-open + best-effort close) // ---------------------------- static std::wstring normalize_start_target_best_effort(const std::wstring& raw) { std::wstring s = raw; // Trim whitespace while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; // Strip simple surrounding quotes if (s.size() >= 2 && ((s.front() == L'"' && s.back() == L'"') || (s.front() == L'\'' && s.back() == L'\''))) { s = s.substr(1, s.size() - 2); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); if (s.empty()) return s; } // Expand env vars (%TEMP%, etc.) if present if (s.find(L'%') != std::wstring::npos) { wchar_t exp[2048]; DWORD got = ExpandEnvironmentStringsW(s.c_str(), exp, 2048); if (got > 0 && got < 2048) { s.assign(exp); while (!s.empty() && (s.front()==L' '||s.front()==L'\t'||s.front()==L'\r'||s.front()==L'\n')) s.erase(s.begin()); while (!s.empty() && (s.back()==L' '||s.back()==L'\t'||s.back()==L'\r'||s.back()==L'\n')) s.pop_back(); } } // If it looks like a filesystem path, canonicalize to full path best-effort. // For URLs / shell targets, leave as-is. // PathIsURLW covers http/https/ftp/mailto, etc. if (!s.empty() && !PathIsURLW(s.c_str())) { bool looks_path = (s.find(L'\\') != std::wstring::npos) || (s.find(L'/') != std::wstring::npos) || (s.find(L':') != std::wstring::npos) || (s.rfind(L".\\", 0) == 0) || (s.rfind(L"..\\", 0) == 0) || (s.rfind(L"./", 0) == 0) || (s.rfind(L"../", 0) == 0); if (looks_path) { wchar_t full[MAX_PATH]; DWORD nFull = GetFullPathNameW(s.c_str(), MAX_PATH, full, NULL); if (nFull > 0 && nFull < MAX_PATH) { s.assign(full); } } } return s; } static std::wstring normalize_start_target_best_effort(const wchar_t* raw) { if (!raw) return std::wstring(); return normalize_start_target_best_effort(std::wstring(raw)); } static std::wstring reg_read_sz_hkcr(const std::wstring& subkey, const wchar_t* value_name) { wchar_t buf[2048]; DWORD cb = sizeof(buf); DWORD type = 0; LONG r = RegGetValueW(HKEY_CLASSES_ROOT, subkey.c_str(), value_name, RRF_RT_REG_SZ, &type, buf, &cb); if (r != ERROR_SUCCESS) return std::wstring(); // Ensure null-terminated buf[(sizeof(buf)/sizeof(buf[0])) - 1] = 0; return std::wstring(buf); } static void wstr_replace_all(std::wstring& s, const wchar_t* from, const std::wstring& to) { if (!from || !from[0]) return; size_t pos = 0; const size_t nfrom = wcslen(from); while ((pos = s.find(from, pos)) != std::wstring::npos) { s.replace(pos, nfrom, to); pos += to.size(); } } static bool create_process_track(const std::wstring& cmdline, const wchar_t* started_target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (cmdline.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty command line."); return false; } std::vector<wchar_t> cl(cmdline.begin(), cmdline.end()); cl.push_back(0); STARTUPINFOW si{}; si.cb = sizeof(si); si.dwFlags = STARTF_USESHOWWINDOW; si.wShowWindow = SW_SHOWNORMAL; PROCESS_INFORMATION pi{}; BOOL ok = CreateProcessW(NULL, cl.data(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi); if (!ok) { DWORD e = GetLastError(); if (out_result && cch) StringCchPrintfW(out_result, cch, L"CreateProcess failed (%lu).", (unsigned long)e); return false; } CloseHandle(pi.hThread); out_sp.started_target = started_target ? started_target : L""; out_sp.pid = pi.dwProcessId; out_sp.hproc.reset(pi.hProcess); if (out_result && cch) StringCchPrintfW(out_result, cch, L"Started (pid %lu).", (unsigned long)out_sp.pid); return true; } static bool start_ahk_via_association_command(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } const wchar_t* ext = PathFindExtensionW(target); if (!ext || _wcsicmp(ext, L".ahk") != 0) return false; // Get ProgID from HKCR\.ahk (default value) std::wstring progid = reg_read_sz_hkcr(L".ahk", NULL); if (progid.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"No .ahk association found (install AutoHotkey or associate .ahk)."); return false; } // Get open command from HKCR\<ProgID>\shell\open\command std::wstring cmdkey = progid + L"\\shell\\open\\command"; std::wstring cmd = reg_read_sz_hkcr(cmdkey, NULL); if (cmd.empty()) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Missing association open command for .ahk."); return false; } // Expand env vars in command if (cmd.find(L'%') != std::wstring::npos) { wchar_t exp[4096]; DWORD got = ExpandEnvironmentStringsW(cmd.c_str(), exp, 4096); if (got > 0 && got < 4096) cmd.assign(exp); } std::wstring qtarget = L"\""; qtarget += target; qtarget += L"\""; // Replace common placeholders. Do the quoted variants first to avoid double-quotes. const std::wstring qt = qtarget; wstr_replace_all(cmd, L"\"%1\"", qt); wstr_replace_all(cmd, L"\"%L\"", qt); wstr_replace_all(cmd, L"\"%l\"", qt); wstr_replace_all(cmd, L"%1", qt); wstr_replace_all(cmd, L"%L", qt); wstr_replace_all(cmd, L"%l", qt); // Drop %* (extra args placeholder) wstr_replace_all(cmd, L"%*", L""); // If no placeholder was present, append the script path if (cmd.find(target) == std::wstring::npos && cmd.find(qt) == std::wstring::npos) { cmd.push_back(L' '); cmd += qt; } return create_process_track(cmd, target, out_sp, out_result, cch); } // If the target is an .exe path, derive the lowercased basename (for fallback stop-by-name). static bool shell_open_and_track_process(const wchar_t* target, StartedProc& out_sp, wchar_t* out_result, size_t cch) { if (out_result && cch) out_result[0] = 0; out_sp = StartedProc{}; if (!target || !target[0]) { if (out_result && cch) StringCchCopyW(out_result, cch, L"Empty target."); return false; } // Special-case .ahk: start via file-association command so we reliably get a process handle (for later profile teardown).;
// ==================================================
// Theme module
// ==================================================
...
static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts) {
    if (!lv || !it || it->name.empty()) return false;
    ...
    // Validate cached row still points at this Item*
    if (idx >= 0) {
        LVITEMW qi{};
        qi.mask = LVIF_PARAM;
        qi.iItem = idx;
        if (!ListView_GetItem(lv, &qi) || (Item*)qi.lParam != it) idx = -1;
    }

    if (idx < 0) {
        // Fallback search by lParam (still no insertion)
        idx = lv_find_item_by_ptr(lv, it);
    ...
    if (it->ui_applied_gen == it->status_gen &&
        wcscmp(it->ui_last_status, status_disp) == 0 &&
    ...
    // Column 1 (Name) generally stable; don't touch.
    BOOL cur = ListView_GetCheckState(lv, idx);
    BOOL want = it->auto_stop ? TRUE : FALSE;
    if ((cur != 0) != (want != 0)) {
        bool prev = self.suppress_lv_notify;
    ...
    StringCchCopyW(it->ui_last_status, 32, status_disp);
    it->ui_last_update_wall = wall_ts;
...
static void ui_apply_status_updates(App& self, int kind) {
    ...
    for (const auto& d : deltas) {
        ...
        lv_update_row_existing_with(self, lv, kind, d.it, d.status, d.wall);
    }
...
static void rebuild_listview_filtered(App& self, int kind) {
    ...
    // Bulk rebuild: suppress redraw for performance and consistency.
    SendMessageW(lv, WM_SETREDRAW, FALSE, 0);

    ListView_DeleteAllItems(lv);
    self.lv_row_of_item[kind].clear();
    for (auto& up : self.items[kind].v) {
        Item* it = up.get();
        if (!it) continue;
        if (item_matches_filter(it, flt)) {
            lv_set_row(self, lv, it);
            int row = lv_find_item_by_ptr(lv, it);
            if (row >= 0) self.lv_row_of_item[kind][it] = row;
}
    }
...
if (id == IDC_TOP_EXIT && code == BN_CLICKED) {
...
self.btn_exit = CreateWindowExW(0, L"BUTTON", L"Exit",
    WS_CHILD | WS_VISIBLE | WS_TABSTOP,
    0, 0, 0, 0, hwnd, (HMENU)IDC_TOP_EXIT, GetModuleHandleW(NULL), NULL);
...
self.activity_panel_w = rightW;
    ...
    int pad = S(8);
    int topbarH = S(26); // room for Settings/Exit buttons
    int tabH = S(28);
    ...
    int btnW = S(72);
    int yTop = 1;
    SetWindowPos(self.btn_settings, NULL, pad, yTop, btnW, topBtnH, SWP_NOZORDER);
    SetWindowPos(self.btn_exit, NULL, pad + btnW + S(8), yTop, btnW, topBtnH, SWP_NOZORDER);
...
AppendMenuW(self.menu_settings, MF_STRING, IDM_EXIT, L"Exit");

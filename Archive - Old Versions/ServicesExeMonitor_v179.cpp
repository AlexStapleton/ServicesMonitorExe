...
struct Item {
    ...
    time_t   last_stopped_wall = 0;     // last time we observed a transition into a stopped state

    // UI-side cache to avoid repainting unchanged rows
    wchar_t  ui_last_status[32]{};
    time_t   ui_last_update_wall = 0;
    time_t   ui_last_stopped_wall = 0;
    ...
    std::vector<std::wstring> to_disable_flap;
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (wcscmp(it->last_status, ns) != 0) {
                ...
                const bool was_stopped = (_wcsicmp(it->last_status, L"stopped") == 0);
                const bool now_stopped = (_wcsicmp(ns, L"stopped") == 0);
                if (!was_stopped && now_stopped) {
                    it->last_stopped_wall = wall_ts;
                }
...
static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) {
    ...
    if (idx < 0) {
        ...
        if (it->ui_last_stopped_wall != wall_ts) {
            if (wall_ts) fmt_time_local(it->ui_cache_last_text, 64, wall_ts);
            else it->ui_cache_last_text[0] = 0;
        ...
        if (lv == self.lv_svc) {
            ...
            } else if (it->flap_flagged) {
                StringCchCopyW(ad, 64, L"Flagged");
            } else {
                ad[0] = 0;
            }
            ListView_SetItemText(lv, row, 3, ad);
            ListView_SetItemText(lv, row, 4, it->ui_cache_last_text);
        } else {
        ...
        if (lv == self.lv_svc) {
            ...
            ListView_SetItemText(lv, idx, 3, ad);
            ListView_SetItemText(lv, idx, 4, it->ui_cache_last_text);
        } else {
...
static bool lv_set_row(App& self, HWND lv, Item* it) {
    ...
    return lv_set_row_with(self, lv, it, it->last_status, it->last_stopped_wall);
}

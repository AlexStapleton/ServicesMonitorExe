...
static void theme_broadcast_change(HWND hwnd) {
    ...
    auto send = [](HWND w, UINT msg, WPARAM wp, LPARAM lp) {
        DWORD_PTR out = 0;
        SendMessageTimeoutW(w, msg, wp, lp, SMTO_ABORTIFHUNG | SMTO_NORMAL, 50, &out);
    };

    // Tell the window + children that theme/colors changed.
    send(hwnd, WM_THEMECHANGED, 0, 0);
    send(hwnd, WM_SYSCOLORCHANGE, 0, 0);
    send(hwnd, WM_SETTINGCHANGE, 0, (LPARAM)L"ImmersiveColorSet");

    EnumChildWindows(hwnd, [](HWND child, LPARAM) -> BOOL {
        DWORD_PTR out = 0;
        SendMessageTimeoutW(child, WM_THEMECHANGED, 0, 0, SMTO_ABORTIFHUNG | SMTO_NORMAL, 50, &out);
        SendMessageTimeoutW(child, WM_SYSCOLORCHANGE, 0, 0, SMTO_ABORTIFHUNG | SMTO_NORMAL, 50, &out);
        SendMessageTimeoutW(child, WM_SETTINGCHANGE, 0, (LPARAM)L"ImmersiveColorSet", SMTO_ABORTIFHUNG | SMTO_NORMAL, 50, &out);
        RedrawWindow(child, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_FRAME | RDW_UPDATENOW);
        return TRUE;
    }, 0);

    // Force immediate repaint now.

...
// Control IDs
#define IDC_HOTKEYS_LIST          7001
#define IDC_HOTKEYS_COPY          7002
...
if (in_profile) {
    ...
    continue;
        ...
        bool ok = list_push(&self.items[kind], it);
        if (ok) {
...
static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts, int* out_row_idx) {
    ...
    if (idx < 0) {
        ...
        idx = lv_find_item_by_uid(lv, it->uid);
...
static void rebuild_listview_filtered(App& self, int kind) {
    ...
    for (auto& up : self.items[kind].v) {
        ...
        if (item_matches_filter(it, flt)) {
            ...
            int row = lv_find_item_by_uid(lv, it->uid);
...
static Item* lv_get_selected_item_ptr(App& self, int kind) {
    ...
    uint32_t uid = (uint32_t)it.lParam;
...
static void rebuild_runtime_items_locked(App& self, const ConfigSnapshot& cfg) {
    for (auto& up : self.items[KIND_SVC].v) { if (up) self.uid_map.erase(up->uid); }
        list_free(&self.items[KIND_SVC]);
    for (auto& up : self.items[KIND_EXE].v) { if (up) self.uid_map.erase(up->uid); }
    ...
    for (int kind = 0; kind < 2; kind++) {
        for (const auto& row : cfg.items[kind]) {
            ...
            it->uid = self.next_uid++;
            if (list_push(&self.items[kind], it)) {
                self.uid_map[it->uid] = it;
            } else {
                delete it;
                it = nullptr;
...
static void toggle_autostop_by_row(App& self, int kind, int row) {
    ...
    itv.iItem = row;
    itv.iSubItem = 0;
    if (!ListView_GetItem(lv, &itv)) return;

    const uint32_t uid = (uint32_t)(uintptr_t)itv.lParam;
    const bool enabled = (ListView_GetCheckState(lv, row) != 0);
...
static bool handle_main_listview_notify(App& self, HWND hwnd, NMHDR* nh, LPARAM lParam, LRESULT& out) {
    ...
    const int kind = (nh->idFrom == IDC_LV_SVC) ? KIND_SVC : KIND_EXE;

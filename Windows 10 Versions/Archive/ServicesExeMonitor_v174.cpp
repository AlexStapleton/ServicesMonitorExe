...
static void theme_apply_to_window(App& self, HWND hwnd) {
    ...
    // Opt into dark mode + dark titlebar (best-effort across Win10/11 builds).
    darkmode_allow_window(hwnd, self.dark_mode);
    try_enable_dark_titlebar(hwnd, self.dark_mode);
    // Apply Explorer-style dark theming where supported.
    SetWindowTheme(hwnd, self.dark_mode ? L"DarkMode_Explorer" : nullptr, nullptr);
    EnumChildWindows(hwnd, enum_theme_children_proc, (LPARAM)&self);

    // Force a non-client refresh; Win10 often won't recolor caption buttons without this.
...
static void theme_force_nonclient_refresh(HWND hwnd) {
    if (!hwnd) return;
    // Nudge Win10 to repaint caption buttons/titlebar when dark mode toggles.
...
static BOOL CALLBACK theme_enum_thread_windows_apply(HWND hwnd, LPARAM lParam) {
    App* self = (App*)lParam;
    if (!self) return TRUE;
    // Apply to all top-level windows in this UI thread (main + dialogs).
    DWORD tid = 0;
    GetWindowThreadProcessId(hwnd, &tid);
    if (tid != GetCurrentThreadId()) return TRUE;

    theme_apply_to_window(*self, hwnd);
    theme_force_nonclient_refresh(hwnd);
    return TRUE;
}

static void theme_apply_everywhere(App& self) {
    // Main window + any modeless dialogs (Hotkeys, Prefs, Profiles, etc.)
    EnumThreadWindows(GetCurrentThreadId(), theme_enum_thread_windows_apply, (LPARAM)&self);
}
...
static DWORD win_build_number() {
    ...
    if (!fp) return 0;
    PFN_RtlGetVersion p = (PFN_RtlGetVersion)fp;

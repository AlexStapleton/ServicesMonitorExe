...
static void theme_apply_to_window(App& self, HWND hwnd) {
    ...
    // Apply dark-mode opt-in and titlebar styling (Win10/11 behavior varies).
    darkmode_allow_window(hwnd, self.dark_mode);
    try_enable_dark_titlebar(hwnd, self.dark_mode);

    // Applying Explorer / DarkMode_Explorer to the top-level window helps some Win10 builds
    // pick up correct non-client colors (caption buttons) and dark scrollbars/menus.
    try_set_explorer_theme(hwnd);

    EnumChildWindows(hwnd, enum_theme_children_proc, (LPARAM)&self);

    // Force a non-client refresh so caption buttons/titlebar repaint correctly (Win10 especially).

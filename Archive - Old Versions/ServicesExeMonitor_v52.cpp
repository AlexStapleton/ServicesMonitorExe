...
struct MonScratch {
    ...
    // Reused buffer for EnumServicesStatusExW results.
...
static void monitor_thread_main(std::stop_token st, App* self_ptr) {
    ...
    unique_sc_handle scm_enum(OpenSCManagerW(NULL, NULL, SC_MANAGER_ENUMERATE_SERVICE));
...
// If multiple profiles match, the first match (config order) wins.
{
    ...
    if (desired != cur) {
        ...
        if (ns && snap.svc_names && sc.svc_status) {
            ...
            bool ok = batch_query_service_states(scm_enum.get(), sc.svc_enum_buf, snap.svc_idx_sorted.data(), snap.svc_idx_sorted.size(), sc.svc_status, ns);

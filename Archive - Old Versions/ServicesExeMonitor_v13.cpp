...
static void log_linef(App& self, const wchar_t* fmt, ...) {
    ...
    activity_append(self.activity, line);
}
...
// Action worker queue
// ----------------------------
static void action_enqueue(App& self, int kind, const wchar_t* name, const wchar_t* reason, int wait_ms) {
                    ...
                    if (_wcsicmp(sc.svc_status[i], L"stopped") != 0) {
                        if ((mono_now - snap.svc_lastreq[i]) >= cd_ms) {
                            ...
                            if (found) {
                                ...
                                action_enqueue(self, KIND_SVC, snap.svc_names[i], reason, stop_wait_ms);
                            }
                        }
                    }
                }
                ...
                if (snap.exe_auto[i] && _wcsicmp(status, L"stopped") != 0) {
                    if ((mono_now - snap.exe_lastreq[i]) >= cd_ms) {
                        ...
                        if (found) {
                            ...
                            action_enqueue(self, KIND_EXE, snap.exe_names[i], reason, stop_wait_ms);
                        }
                    }
                }
            }
...
static bool prof_row_name_from_listbox_line(const std::wstring& line, std::wstring& out_name) {
...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowW(L"BUTTON", L"Close", WS_CHILD | WS_VISIBLE,
            660, 810, 80, 28, dlg, (HMENU)IDCANCEL, NULL, NULL);
        ...
        if (id == IDC_PCONT_SVC_ADD) {
            ...
            bool apply_now = false;
            {
                ...
                if (psel >= 0 && psel < (int)self.profiles.size()) {
                    ...
                    if (!dup) {
                        rows.push_back(ItemRow{ name, autostop });
        ...
        if (id == IDC_PCONT_EXE_ADD) {
            ...
            bool apply_now = false;
            {
                ...
                if (psel >= 0 && psel < (int)self.profiles.size()) {
                    ...
                    if (!dup) {
                        rows.push_back(ItemRow{ std::wstring(norm), autostop });
                        std::sort(rows.begin(), rows.end(), [](const ItemRow& a, const ItemRow& b) {
                            return _wcsicmp(a.name.c_str(), b.name.c_str()) < 0;
                        });
                        added = true;
...
static void build_ui(App& self, HWND hwnd) {
    ...
    CreateWindowW(L"BUTTON", L"Remove selected", WS_CHILD | WS_VISIBLE,
        10, 144, 220, 28, self.left_page_svc, (HMENU)IDC_SVC_REMOVE_BTN, NULL, NULL);
    ...
    CreateWindowW(L"BUTTON", L"Remove selected", WS_CHILD | WS_VISIBLE,
        10, 144, 220, 28, self.left_page_exe, (HMENU)IDC_EXE_REMOVE_BTN, NULL, NULL);
    ...
    self.il_svc = ImageList_Create(16, 16, ILC_COLOR32 | ILC_MASK, 4, 4);
    self.il_exe = ImageList_Create(16, 16, ILC_COLOR32 | ILC_MASK, 4, 4);
    if (self.il_svc) {
        HICON hSvc = (HICON)LoadImageW(NULL, IDI_INFORMATION, IMAGE_ICON, 16, 16, LR_SHARED);
        HICON hExe = (HICON)LoadImageW(NULL, IDI_APPLICATION, IMAGE_ICON, 16, 16, LR_SHARED);
...
int rightW = (self.activity_panel_w > 0) ? self.activity_panel_w : 360;
...
self.activity_panel_w = rightW;
    ...
    // Services page
    {
        ...
        int addW = 84;
        int gap = 8;
        int editW = std::max(60, lw - 2*inPad - addW - gap);
        ...
        int y2 = y1 + rowH + 14;
        int fullW = std::max(80, lw - 2*inPad);

        if (bStop) SetWindowPos(bStop, NULL, inPad, y2, fullW, btnH, SWP_NOZORDER);
        if (bRem)  SetWindowPos(bRem,  NULL, inPad, y2 + btnH + 10, fullW, btnH, SWP_NOZORDER);
    }
    ...
    // EXEs page
    {
        ...
        int addW = 72;
        int gap = 8;
        int editW = std::max(60, lw - 2*inPad - browseW - addW - gap*2);
    ...
    if (self.svc_search) SetWindowPos(self.svc_search, NULL, cPad, cPad, std::max(80, cw - 2*cPad), searchH, SWP_NOZORDER);
    if (self.exe_search) SetWindowPos(self.exe_search, NULL, cPad, cPad, std::max(80, cw - 2*cPad), searchH, SWP_NOZORDER);

    int lvY = cPad + searchH + gap;
    int lvH = std::max(60, ch - lvY - cPad);

    SetWindowPos(self.lv_svc, NULL, cPad, lvY, std::max(80, cw - 2*cPad), lvH, SWP_NOZORDER);
    SetWindowPos(self.lv_exe, NULL, cPad, lvY, std::max(80, cw - 2*cPad), lvH, SWP_NOZORDER);

    int rx = leftW + centerW + pad;

    if (self.splitter) {
        int sx = leftW + centerW - (SPLITTER_WIDTH / 2);
        SetWindowPos(self.splitter, NULL, sx, pad, SPLITTER_WIDTH, H - 2*pad, SWP_NOZORDER);
    }
    int labelH = 18;
    HWND label = self.activity_label;
    if (label) SetWindowPos(label, NULL, rx, pad, rightW - 2*pad, labelH, SWP_NOZORDER);

    SetWindowPos(self.activity, NULL, rx, pad + labelH + 6, rightW - 2*pad, H - (pad + labelH + 6) - pad, SWP_NOZORDER);
...
static void stop_selected(App& self, int kind) {
    ...
    action_enqueue(self, kind, name, L"manual stop", wait_ms);
}

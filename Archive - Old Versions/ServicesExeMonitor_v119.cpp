...
static void capture_columns_now(App& self, int kind) {
    ...
    for (int i = 0; i < ncol; i++) {
        self.colw[kind][i] = ListView_GetColumnWidth(lv, i);
    }
    int order[8]{0};
    if (ListView_GetColumnOrderArray(lv, ncol, order)) {
        for (int i = 0; i < ncol; i++) self.colorder[kind][i] = order[i];
    } else {
        for (int i = 0; i < ncol; i++) self.colorder[kind][i] = i;
    }
    self.cols_have[kind] = true;
...
static void apply_saved_columns(App& self, int kind) {
    ...
    int order[8]{0};
    for (int i = 0; i < ncol; i++) order[i] = self.colorder[kind][i];
    ListView_SetColumnOrderArray(lv, ncol, order);

    for (int i = 0; i < ncol; i++) {
        int w = self.colw[kind][i];
    ...
    switch (msg) {
        case WM_KEYDOWN:
            if (wParam == VK_ESCAPE) {
                ...
                if (kind != -1 && hwnd) apply_filter_now(*self, hwnd, kind);
                return 0;
            }
            break;
    }

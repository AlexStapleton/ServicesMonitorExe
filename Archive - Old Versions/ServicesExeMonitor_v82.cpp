...
static void theme_menu_set_owner_draw(HMENU menu, bool enable) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (enable) {
            if ((mi.fType & MFT_OWNERDRAW) == 0) {
                // Allocate and store label
                size_t len = wcslen(buf);
                wchar_t* heap = new wchar_t[len + 1];
                if (len) memcpy(heap, buf, len * sizeof(wchar_t));
                heap[len] = 0;

                mi.fMask = MIIM_FTYPE | MIIM_DATA;
                mi.fType |= MFT_OWNERDRAW;
                mi.dwItemData = (ULONG_PTR)heap;

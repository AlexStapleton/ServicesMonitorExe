...
struct Item {
    ...
    wchar_t  ui_cache_last_stopped_text[64]{}; // formatted local time string
...
static bool lv_set_row_with(App& self, HWND lv, Item* it, const wchar_t* status_raw, time_t wall_ts) {
    ...
    if (idx < 0) {
        ...
        if (it->kind == KIND_SVC && has_auto_disable_col) {
            ...
            ListView_SetItemText(lv, row, col_last, it->ui_cache_last_stopped_text);
        } else {
            ListView_SetItemText(lv, row, col_last, it->ui_cache_last_stopped_text);
        }
        ...
        if (it->kind == KIND_SVC && has_auto_disable_col) {
            ...
            ListView_SetItemText(lv, idx, col_last, it->ui_cache_last_stopped_text);
        } else {
            ListView_SetItemText(lv, idx, col_last, it->ui_cache_last_stopped_text);
        }
...
static bool lv_update_row_existing_with(App& self, HWND lv, int kind, Item* it, const wchar_t* status_raw, time_t wall_ts, int* out_row_idx) {
    ...
    // Update "Last Stopped" only on transition into a stopped/off state.
    auto is_stopped_status = [&](const wchar_t* st) -> bool {
        if (!st || !st[0]) return false;
        if (it->kind == KIND_SVC) return _wcsicmp(st, L"stopped") == 0;
        // EXEs may report various non-running strings depending on the code path.
        if (_wcsicmp(st, L"stopped") == 0) return true;
        if (_wcsicmp(st, L"not running") == 0) return true;
        if (_wcsicmp(st, L"off") == 0) return true;
        return false;
    };

    const bool prev_stopped = is_stopped_status(it->ui_last_status);
    const bool now_stopped  = is_stopped_status(status_raw);
    if (!prev_stopped && now_stopped && wall_ts) {
        it->ui_last_stopped_wall = wall_ts;
    }

    // Re-format the displayed stopped timestamp only when it changes.
    if (it->ui_last_update_wall != it->ui_last_stopped_wall) {
        if (it->ui_last_stopped_wall) {
            fmt_time_local(it->ui_cache_last_stopped_text, 64, it->ui_last_stopped_wall);
        } else {
            it->ui_cache_last_stopped_text[0] = 0;
        }
        it->ui_last_update_wall = it->ui_last_stopped_wall;
    }
...
self.left_tabs = make_tab(hwnd, IDC_LEFT_TABS);
    ...
    lv_set_columns5(self.lv_svc, L"Auto Stop", L"Service Name", L"Status", L"Auto Disable", L"Last Stopped");
    lv_set_columns(self.lv_exe, L"Auto Stop", L"Executable", L"Status", L"Last Stopped");
...
// to seed Status/Last Stopped for items that never change.

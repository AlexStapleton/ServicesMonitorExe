                ...
                if (kind == KIND_SVC) {
                    ...
                    if (!it->flap_flagged && it->flap_restarts >= 3) {
                        ...
                        if (self.auto_disable_flapping && !it->flap_auto_disable_attempted) {
                            ...
                            DWORD err = 0;
                            if (set_service_start_type(it->name.c_str(), SERVICE_DISABLED, &err)) {
                                log_linef(self, L"%s: auto-disabled (flapping detected)", it->name.c_str());
                                action_enqueue(self, KIND_SVC, ACTION_STOP, it->name.c_str(), L"Auto-disabled (flapping)", self.stop_wait_ms);
                            } else {
                                log_linef(self, L"%s: auto-disable failed (err=%lu) - run as admin?", it->name.c_str(), (unsigned long)err);
                            }
                        }
                    }
                }
...
// ----------------------------
static bool prefs_apply(App& self, HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close, HWND c_dark, HWND c_autodisable);
...
static bool prefs_apply(App& self, HWND dlg, HWND e_ui, HWND e_mon, HWND e_cd, HWND e_sw, HWND c_tray, HWND c_close, HWND c_dark, HWND c_autodisable) {
    ...
    bool dark_changed = false;
    {
        ...
        // Flapping policy
        BOOL ad_checked = (SendMessageW(c_autodisable, BM_GETCHECK, 0, 0) == BST_CHECKED);
        self.auto_disable_flapping = ad_checked ? true : false;
...
static LRESULT CALLBACK PrefsWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    static HWND c_tray, c_close, c_dark, c_autodisable;
    ...
    case WM_CREATE: {
        ...
        c_autodisable = CreateWindowW(L"BUTTON", L"Auto-disable services when flapping", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
            S(12), S(224), S(300), S(22), dlg, (HMENU)IDC_PREF_AUTO_DISABLE_FLAP, NULL, NULL);
        wchar_t buf[64];
        ...
        SendMessageW(c_autodisable, BM_SETCHECK, self.auto_disable_flapping ? BST_CHECKED : BST_UNCHECKED, 0);
...
if (id == IDC_TOP_QUIT && code == BN_CLICKED) {
    ...
    return 0;
        ...
        if (id == IDC_PREF_APPLY) { prefs_apply(self, dlg, e_ui, e_mon, e_cd, e_sw, c_tray, c_close, c_dark, c_autodisable); return 0; }
        if (id == IDOK) {
            if (prefs_apply(self, dlg, e_ui, e_mon, e_cd, e_sw, c_tray, c_close, c_dark, c_autodisable)) DestroyWindow(dlg);
            return 0;
        }
        return 0;
    }

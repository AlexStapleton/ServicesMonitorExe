...
static void theme_force_immediate_repaint(HWND hwnd) {
    if (!hwnd) return;
    ...
    // hide + show the window while preserving placement. This forces DWM to recreate
    // the non-client surface on stubborn systems.
...
void theme_apply_to_window(App& self, HWND hwnd) {
    ...
    txn.end();
    theme_force_immediate_repaint(hwnd);
}

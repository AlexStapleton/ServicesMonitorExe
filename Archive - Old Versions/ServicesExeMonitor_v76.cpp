...
static void theme_apply_to_control(App& self, HWND child) {
    ...
    if (lstrcmpiW(cls, L"Button") == 0) {
        ...
        // Clear themed background for owner-draw push buttons so Windows doesn't try to paint behind us.
        if (is_push && self.dark_mode) {
            try_set_explorer_theme_mode(child, true);
        } else {
            try_set_explorer_theme_mode(child, self.dark_mode);
        }
...
if (nh && nh->code == NM_CUSTOMDRAW) {
    ...
    case WM_COMMAND: {
        ...
        if (id == IDM_DARKMODE || id == IDM_TRAY_DARKMODE) {
            ...
            theme_update_resources(self);
            theme_apply_everywhere(self);
            // Menu owner-draw: enable in dark mode, disable in light mode.
            {
                HMENU mb2 = GetMenu(hwnd);
                if (mb2) theme_menu_set_owner_draw(mb2, self.dark_mode);
            }

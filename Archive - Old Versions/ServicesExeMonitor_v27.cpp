...
static void apply_profile_index(App& self, int idx) {
...

    {
        ...
        // Policy: For non-default profiles only, start EXE rows whose "Auto-stop" is OFF and are currently NOT running.
        // (If Auto-stop is ON, the monitor would immediately stop it anyway.)
        if (idx >= 0) {
            for (const auto& r : cfg->items[KIND_EXE]) {
                if (r.name.empty()) continue;
                if (r.auto_stop) continue;

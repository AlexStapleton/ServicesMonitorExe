...
static bool mons_calc_growth_capacity(size_t current_cap, size_t required, size_t* out_cap) {
    if (!out_cap) return false;
    if (required == 0) { *out_cap = current_cap; return true; }

    // Growth policy: ~1.5x (less memory overshoot than x2), but never below 16,
    // and always at least large enough for the current request.
    size_t grown = (current_cap == 0) ? 16 : (current_cap + (current_cap >> 1));
    if (grown < 16) grown = 16;
    if (grown < required) grown = required;

    // Round up to a small alignment bucket to avoid frequent tiny reallocations.
    constexpr size_t kAlign = 16;
    if (grown > (SIZE_MAX - (kAlign - 1))) return false;
    grown = (grown + (kAlign - 1)) & ~(kAlign - 1);
...
static bool mons_ensure_scratch(MonScratch* sc, size_t ns, size_t ne) {
    if (ns > sc->cap_s) {
        size_t nc = 0;
        if (!mons_calc_growth_capacity(sc->cap_s, ns, &nc)) return false;
    ...
    if (ne > sc->cap_e) {
        size_t nc = 0;
        if (!mons_calc_growth_capacity(sc->cap_e, ne, &nc)) return false;

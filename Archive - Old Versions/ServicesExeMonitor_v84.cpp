...
static bool theme_draw_owner_menu(App& self, const DRAWITEMSTRUCT* dis) {
    ...
    // Prefer itemData text (works for popup/menu-bar items that have itemID==0).
    const wchar_t* txtp = (const wchar_t*)dis->itemData;
    wchar_t tmp[256]{0};
    if (!txtp || !txtp[0]) {
        // Fallback: query by command id if available.
        if (dis->hwndItem && dis->itemID) {
            GetMenuStringW((HMENU)dis->hwndItem, dis->itemID, tmp, 255, MF_BYCOMMAND);
            txtp = tmp;
        } else {
            txtp = L"";
        }
    ...
    HFONT hfont = (HFONT)SendMessageW(self.hwnd, WM_GETFONT, 0, 0);
    HGDIOBJ oldf = NULL;
    if (hfont) oldf = SelectObject(hdc, hfont);

    RECT tr = rc;
    InflateRect(&tr, -8, 0);
    DrawTextW(hdc, txtp, -1, &tr, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
...
// Enable/disable owner-draw for menu items.
// We store a heap-copied label in dwItemData so WM_DRAWITEM/WM_MEASUREITEM can render reliably
// even for popup/menu-bar items that have itemID==0.
static void theme_menu_set_owner_draw(HMENU menu, bool enable) {
    ...
    for (int i = 0; i < n; ++i) {
        MENUITEMINFOW mi{};
        mi.cbSize = sizeof(mi);
        mi.fMask = MIIM_FTYPE | MIIM_SUBMENU | MIIM_DATA | MIIM_STRING | MIIM_ID;
        wchar_t buf[512]{0};
        mi.dwTypeData = buf;
        mi.cch = (UINT)(sizeof(buf)/sizeof(buf[0]) - 1);

        if (!GetMenuItemInfoW(menu, i, TRUE, &mi)) continue;

        // Recurse first
        if (mi.hSubMenu) theme_menu_set_owner_draw(mi.hSubMenu, enable);

        // Separators: always keep default drawing
        if (mi.fType & MFT_SEPARATOR) {
            if (mi.fType & MFT_OWNERDRAW) {
                // In case some earlier build left ownerdraw on a separator
                wchar_t* heap = (wchar_t*)mi.dwItemData;
                if (heap) delete[] heap;
        ...
        if (enable) {
            // Ensure owner-draw is ON and item text is available via dwItemData.
            wchar_t* heap = (wchar_t*)mi.dwItemData;

            // If already owner-draw but no cached text, or cached text mismatches, refresh it.
            bool need_refresh = (heap == nullptr);
            if (!need_refresh && buf[0]) {
                if (wcscmp(heap, buf) != 0) need_refresh = true;
            }

            if (need_refresh) {
                if (heap) delete[] heap;
                size_t len = wcslen(buf);
                wchar_t* new_heap = new wchar_t[len + 1];
                if (len) memcpy(new_heap, buf, len * sizeof(wchar_t));
                new_heap[len] = 0;

                mi.fMask = MIIM_FTYPE | MIIM_DATA;
                mi.fType |= MFT_OWNERDRAW;
                mi.dwItemData = (ULONG_PTR)new_heap;
                SetMenuItemInfoW(menu, i, TRUE, &mi);
            } else if ((mi.fType & MFT_OWNERDRAW) == 0) {
                // Has cached text but owner-draw flag missing; enable it.
                mi.fMask = MIIM_FTYPE;
                mi.fType |= MFT_OWNERDRAW;
                SetMenuItemInfoW(menu, i, TRUE, &mi);
            }
        } else {
            // Ensure owner-draw is OFF and free any cached label.
            wchar_t* heap = (wchar_t*)mi.dwItemData;
            if (heap) delete[] heap;
...
tray_sync(self, GetModuleHandleW(NULL));
    ...
    if (dark_changed) {
        ...
        // Bump theme generation (so future windows get themed)...
        self.theme_gen.fetch_add(1, std::memory_order_relaxed);
        // ...but apply instantly for good UX (no waiting for the next UI timer tick).
...
switch (msg) {
    case WM_CREATE: {
        ...
        // Menus: use owner-draw only in dark mode; restore default in light mode.
        theme_menu_set_owner_draw(menubar, self.dark_mode);
        DrawMenuBar(hwnd);
    ...
    case WM_MEASUREITEM: {
        ...
        if (self.dark_mode && (mis->CtlType == ODT_MENU || mis->CtlType == ODT_TAB)) {
            ...
            if (mis->CtlType == ODT_MENU) {
                const wchar_t* t = (const wchar_t*)mis->itemData;
                if (t && t[0]) {
                    wcsncpy(txt, t, 255);
                } else if (mis->itemID) {

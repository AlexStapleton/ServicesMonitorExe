...
static HWND st_profiles_hdr, lb_profiles, st_name_hdr, e_name;
static HWND b_add, b_rename, b_del, b_up, b_down;
static HWND tab_right;

static HWND st_watch_hdr, lb_watch, st_watch_add_hdr, e_watch, b_watch_add, b_watch_del;
static HWND st_start_hdr, lb_start_exe, st_start_add_hdr, e_start_exe, b_start_exe_add, b_start_exe_del;

static HWND st_contents_hdr, st_svc_hdr, st_exe_hdr;
static HWND lb_svc, lb_exe;
static HWND e_svc, chk_svc, b_svc_add, b_svc_toggle, b_svc_del;
static HWND e_exe, chk_exe, b_exe_add, b_exe_toggle, b_exe_del;

static HWND st_active;
static HWND st_note;
static HWND b_close;
    ...
    auto DW = [&](HWND h, int x, int y, int w, int hgt, BOOL repaint = TRUE) {
        if (!h) return;
        SetWindowPos(h, NULL, x, y, w, hgt, SWP_NOZORDER | (repaint ? 0 : SWP_NOREDRAW));
    };
    auto DWS = [&](HWND h, int x, int y, int w, int hgt) { DW(h, x, y, w, hgt, FALSE); };
    ...
    const bool show_contents   = (tab == 2);
    ...
    VIS(st_contents_hdr, show_contents);
    VIS(st_note, show_contents);
    VIS(st_svc_hdr, show_contents);
    VIS(lb_svc, show_contents);
    VIS(e_svc, show_contents);
    VIS(chk_svc, show_contents);
    VIS(b_svc_add, show_contents);
    VIS(b_svc_toggle, show_contents);
    VIS(b_svc_del, show_contents);

    VIS(st_exe_hdr, show_contents);
    VIS(lb_exe, show_contents);
    VIS(e_exe, show_contents);
    VIS(chk_exe, show_contents);
    VIS(b_exe_add, show_contents);
    VIS(b_exe_toggle, show_contents);
    VIS(b_exe_del, show_contents);
    ...
    int px = xR + pr.left;
    int py = yTop + pr.top;
    int pw = std::max(1, pr.right - pr.left);
    int ph = std::max(1, pr.bottom - pr.top);
    ...
    if (show_activation) {
        ...
        const int minList = std::max(dpi_scale(s_prof_dpi, 220), dpi_scale(s_prof_dpi, 16) * 8);
        const int addNeed = headerH + g2 + std::max(editH, btnH) + g2;
        int listH = std::max(minList, ph - (yy - py) - addNeed - g2);
        ...
        DWS(st_contents_hdr, px, yy, pw, bigHdrH); yy += bigHdrH + g2;
        DW(st_note, px, yy, pw, labelH * 3); yy += (labelH * 3) + g;

        // Remaining area for lists + controls
        int remainingH = std::max(1, ph - (yy - py));
        const int ctrlBlockH = editH + dpi_scale(s_prof_dpi, 22) + (btnH * 3) + (g2 * 6);
        int listH = std::max(dpi_scale(s_prof_dpi, 140), remainingH - ctrlBlockH);

        const bool side_by_side = (pw >= dpi_scale(s_prof_dpi, 740));
        if (side_by_side) {
            int colGap = g;
            int colW = (pw - colGap) / 2;
            int xSvc = px;
            int xExe = px + colW + colGap;
            ...
            DW(lb_svc, xSvc, yList, colW, listH);
            DW(lb_exe, xExe, yList, colW, listH);

            int yCtrl = yList + listH + g2;

            DW(e_svc, xSvc, yCtrl, colW, editH);
            DW(e_exe, xExe, yCtrl, colW, editH);

            yCtrl += editH + g2;

            const int chkH = dpi_scale(s_prof_dpi, 22);
            DW(chk_svc, xSvc, yCtrl, colW, chkH);
            DW(chk_exe, xExe, yCtrl, colW, chkH);

            yCtrl += chkH + g2;

            DW(b_svc_add, xSvc, yCtrl, colW, btnH);
            DW(b_exe_add, xExe, yCtrl, colW, btnH);
            yCtrl += btnH + g2;

            DW(b_svc_toggle, xSvc, yCtrl, colW, btnH);
            DW(b_exe_toggle, xExe, yCtrl, colW, btnH);
            yCtrl += btnH + g2;

            DW(b_svc_del, xSvc, yCtrl, colW, btnH);
            DW(b_exe_del, xExe, yCtrl, colW, btnH);
        } else {
            // Stack services then EXEs (better when narrow)
            int w = pw;

            // Services
            DWS(st_svc_hdr, px, yy, w, headerH);
            int yList = yy + headerH + g2;
            DW(lb_svc, px, yList, w, listH);

            int yCtrl = yList + listH + g2;
            DW(e_svc, px, yCtrl, w, editH); yCtrl += editH + g2;
            const int chkH = dpi_scale(s_prof_dpi, 22);
            DW(chk_svc, px, yCtrl, w, chkH); yCtrl += chkH + g2;
            DW(b_svc_add, px, yCtrl, w, btnH); yCtrl += btnH + g2;
            DW(b_svc_toggle, px, yCtrl, w, btnH); yCtrl += btnH + g2;
            DW(b_svc_del, px, yCtrl, w, btnH); yCtrl += btnH + g;

            // EXEs
            int yy2 = yCtrl;
            DWS(st_exe_hdr, px, yy2, w, headerH);
            yList = yy2 + headerH + g2;
            DW(lb_exe, px, yList, w, listH);

            yCtrl = yList + listH + g2;
            DW(e_exe, px, yCtrl, w, editH); yCtrl += editH + g2;
            DW(chk_exe, px, yCtrl, w, chkH); yCtrl += chkH + g2;
            DW(b_exe_add, px, yCtrl, w, btnH); yCtrl += btnH + g2;
            DW(b_exe_toggle, px, yCtrl, w, btnH); yCtrl += btnH + g2;
            DW(b_exe_del, px, yCtrl, w, btnH);
        }
    }
};
...
if (tab_right) {
    ...
    ti.pszText = (LPWSTR)L"Start EXEs";
    TabCtrl_InsertItem(tab_right, 1, &ti);
    ti.pszText = (LPWSTR)L"Contents";
    ...
    TabCtrl_SetCurSel(tab_right, 0);
        ...
        st_contents_hdr = CreateWindowW(L"STATIC", L"Profile contents\r\n(editable even when profile is NOT active)", WS_CHILD | WS_VISIBLE,
        ...
        st_start_hdr = CreateWindowW(L"STATIC",
            L"Start EXEs when this profile activates\r\n(stopped on deactivation):",
            WS_CHILD | WS_VISIBLE,
        ...
        b_close = b_close = CreateWindowW(L"BUTTON", L"Close", WS_CHILD | WS_VISIBLE,

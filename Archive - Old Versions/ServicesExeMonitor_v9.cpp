...
static void post_status(App& self, int kind, const wchar_t* name, const wchar_t* status, time_t wall_ts) {
...
static void apply_profile_index(App& self, int idx) {
    ...
    // Refresh listviews
    self.suppress_lv_notify = true;
    ListView_DeleteAllItems(self.lv_svc);
    ListView_DeleteAllItems(self.lv_exe);

    for (Item* it : svc_ptrs) lv_set_row(self, self.lv_svc, it);
    for (Item* it : exe_ptrs) lv_set_row(self, self.lv_exe, it);
...
static LRESULT CALLBACK ProfilesWndProc(HWND dlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        st_active = CreateWindowW(L"STATIC", L"", WS_CHILD | WS_VISIBLE,
            280, 356, 460, 18, dlg, (HMENU)IDC_PROF_ACTIVE_LABEL, NULL, NULL);
        ...
        CreateWindowW(L"STATIC",
            ...
            WS_CHILD | WS_VISIBLE,
            280, 378, 460, 40, dlg, NULL, NULL, NULL);

        CreateWindowW(L"BUTTON", L"Close", WS_CHILD | WS_VISIBLE,
            660, 392, 80, 28, dlg, (HMENU)IDCANCEL, NULL, NULL);
...
static void build_ui(App& self, HWND hwnd) {
    ...
    // Center: listviews
    self.lv_svc = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"",
        WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS,
        0, 0, 100, 100, self.center_page_svc, (HMENU)IDC_LV_SVC, GetModuleHandleW(NULL), NULL);

    self.lv_exe = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"",
        WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS,
    ...
    self.activity = CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"",
...
self.activity_panel_w = rightW;
    ...
    SetWindowPos(self.lv_svc, NULL, 0, 0, centerW - 2*pad, ch, SWP_NOZORDER);
    SetWindowPos(self.lv_exe, NULL, 0, 0, centerW - 2*pad, ch, SWP_NOZORDER);

...
static void theme_menu_make_owner_draw(HMENU menu) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (mi.hSubMenu) {
            theme_menu_make_owner_draw(mi.hSubMenu);
        }
        // Skip separators
...
switch (msg) {
    case WM_CREATE: {
        ...
        // Dark-mode: force owner-draw menus so the Settings menu can be painted dark reliably.
        if (self.dark_mode) {
            theme_menu_make_owner_draw(menubar);
            DrawMenuBar(hwnd);
        }
...
if (nh && nh->code == NM_CUSTOMDRAW) {
    ...
    case WM_COMMAND: {
        ...
        if (id == IDM_DARKMODE || id == IDM_TRAY_DARKMODE) {
            ...
            theme_update_resources(self);
            theme_apply_everywhere(self);
            // Re-apply owner-draw menu to match the new theme.
            if (self.dark_mode) {
                HMENU mb2 = GetMenu(hwnd);
                if (mb2) theme_menu_make_owner_draw(mb2);
            }
